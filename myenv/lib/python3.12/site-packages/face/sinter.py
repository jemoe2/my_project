...
def get_fb(f, drop_self=True):
    ...
    if not (inspect.isfunction(f) or inspect.ismethod(f) or inspect.isbuiltin(f)) and hasattr(f, "__call__"):
    ...
    if not all([isinstance(a, str) for a in ret.args]):  # pragma: no cover (2 only)
        raise TypeError("does not support anonymous tuple arguments" " or any other strange args for that matter.")
...
def build_chain_str(funcs, params, inner_name, params_sofar=None, level=0, func_aliaser=None, func_names=None):
    if not funcs:
    ...
    inner_args = ", ".join(["%s=%s" % kv for kv in inner_arg_items if kv[0] in params_sofar])
...
def make_chain(funcs, provides, final_func, preprovided, inner_name):
    ...
    reqs, opts = chain_argspec(funcs + [final_func], provides + [()], inner_name)

    unresolved = tuple(reqs - preprovided)
    args = reqs | (preprovided & opts)
    chain = compile_chain(funcs + [final_func], [args] + provides, inner_name)
