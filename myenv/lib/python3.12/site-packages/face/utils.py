...
def process_command_name(name):
    ...
    if name.endswith("-") or name.endswith("_"):
        raise ValueError("expected subcommand name without trailing dashes" " or underscores, not: %r" % name)

    name_match = VALID_FLAG_RE.match(name)
    if not name_match:
        raise ValueError(
            "valid subcommand name must begin with a letter, and"
...
def flag_to_identifier(flag):
    ...
    if flag.endswith("-") or flag.endswith("_"):
        raise ValueError("expected flag without trailing dashes" " or underscores, not: %r" % orig_flag)
    ...
    if not flag_match:
        raise ValueError(
            "valid flag names must begin with a letter, optionally"
    ...
    if keyword.iskeyword(flag_name):
        raise ValueError("valid flag names must not be Python keywords: %r" % orig_flag)
...
def get_cardinalized_args_label(name, min_count, max_count):
    ...
    if min_count == 1:
        return (
            name
            + " "
            + get_cardinalized_args_label(name, min_count=0, max_count=max_count - 1 if max_count is not None else None)
        )
...
def get_rdep_map(dep_map):
    ...
    for key in dep_map:
        ...
        while to_proc:
            ...
            if key in cur_rdeps:
                raise ValueError(
                    "dependency cycle: %r recursively depends" " on itself. full dep chain: %r" % (cur, cur_chain)
                )
