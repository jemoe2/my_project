...
def _validate_char(char):
    ...
    if len(char) > 1:
        raise ValueError(
            "char flags must be exactly one character, optionally" " prefixed by a dash, not: %r" % orig_char
        )
    if char not in _VALID_CHARS:
        raise ValueError(
            "expected valid flag character (ASCII letters, numbers,"
...
def _posargs_to_provides(posargspec, posargs):
    ...
    raise RuntimeError(
        "invalid posargspec/posargs configuration %r -- %r" % (posargspec, posargs)
    )  # pragma: no cover (shouldn't get here)
...
class Flag(object):
    ...
    def __init__(self, name, parse_as=str, missing=None, multi="error", char=None, doc=None, display=None):
        ...
        if missing is ERROR and not callable(parse_as):
            raise ValueError(
                "cannot make an argument-less flag required."
        ...
        if callable(multi):
            ...
            raise ValueError(
                "multi expected callable, bool, or one of %r, not: %r" % (list(_MULTI_SHORTCUTS.keys()), multi)
            )
    ...
    def set_display(self, display):
        ...
        if not isinstance(display, FlagDisplay):
            raise TypeError(
                "expected bool, text name, dict of display" " options, or FlagDisplay instance, not: %r" % display
            )
        self.display = display

    def __repr__(self):
        return format_nonexp_repr(
            self, ["name", "parse_as"], ["missing", "multi"], opt_key=lambda v: v not in (None, _multi_error)
        )
...
class PosArgSpec(object):
    ...
    def __init__(self, parse_as=str, min_count=None, max_count=None, display=None, provides=None, **kwargs):
        ...
        if self.max_count and self.min_count > self.max_count:
            raise ValueError("expected min_count > max_count, not: %r > %r" % (self.min_count, self.max_count))
        ...
        if not isinstance(display, PosArgDisplay):
            raise TypeError(
                "expected bool, text name, dict of display" " options, or PosArgDisplay instance, not: %r" % display
            )
    ...
    def parse(self, posargs):
        ...
        if len_posargs < min_count:
            raise ArgumentArityError("too few arguments, expected %s, got %s" % (arg_range_text, len_posargs))
        if max_count is not None and len_posargs > max_count:
            raise ArgumentArityError("too many arguments, expected %s, got %s" % (arg_range_text, len_posargs))
        ret = []
        for pa in posargs:
            try:
...
def _ensure_posargspec(posargs, posargs_name):
    ...
    if not isinstance(posargs, PosArgSpec):
        raise TypeError(
            "expected %s as True, False, number of args, text name of args,"
            " dict of PosArgSpec options, or instance of PosArgSpec, not: %r" % (posargs_name, posargs)
        )
...
class Parser(object):
    ...
    def __init__(self, name, doc=None, flags=None, posargs=None, post_posargs=None, flagfile=True):
        ...
        if flagfile is True:
            ...
            raise TypeError("expected True, False, or Flag instance for" " flagfile, not: %r" % flagfile)
    ...
    def get_flag_map(self, path, with_hidden=True):
        ...
        return OrderedDict([(k, f) for k, f in flag_map.items() if with_hidden or not f.display.hidden])
    ...
    def _add_subparser(self, subprs):
        ...
        if self.posargs.accepts_args:
            raise ValueError("commands accepting positional arguments" " cannot take subcommands")
    ...
    def add(self, *a, **kw):
        ...
        if isinstance(a[0], Flag):
            ...
            try:
                ...
                raise ValueError("expected Parser, Flag, or Flag parameters," " not: %r, %r (got %r)" % (a, kw, te))
    ...
    def _add_flag(self, flag):
        ...
        for subcmds, flag_map in self._path_flag_map.items():
            ...
            if flag.name in (conflict_flag.name, conflict_flag.char):
                raise ValueError("pre-existing flag %r conflicts with name of new flag %r" % (conflict_flag, flag.name))
            if flag.char and flag.char in (conflict_flag.name, conflict_flag.char):
                raise ValueError(
                    "pre-existing flag %r conflicts with short form for new flag %r" % (conflict_flag, flag)
                )
            ...
            try:
                ...
                if leftover_args:
                    raise ArgumentParseError(
                        'excessive flags or arguments for flag "%s",' " expected one flag per line" % flag.name
                    )
...
def parse_sv_line(line, sep=","):
    ...
    *sep*. Supports quoting.
    ...
    from csv import QUOTE_MINIMAL, Dialect, reader
