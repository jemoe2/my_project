...
from face.utils import (
    echo,
    format_flag_label,
    format_flag_post_doc,
    format_posargs_label,
)
...
def _wrap_stout_cmd_doc(indent, doc, max_width):
    ...
    parts = []
    paras = ["\n".join(para) for para in split(doc.splitlines(), lambda l: not l.lstrip()) if para]
    for para in paras:
        part = textwrap.fill(
            text=para, width=(max_width - len(indent)), initial_indent=indent, subsequent_indent=indent
        )
        parts.append(part)
    return "\n\n".join(parts)


def get_stout_layout(labels, indent, sep, width=None, max_width=DEFAULT_MAX_WIDTH, min_doc_width=40):
    ...
    return {"width": width, "label_width": max_label_width, "doc_width": max_doc_width, "doc_start": doc_start}
        ...
        if parser.doc:
            append(
                _wrap_stout_cmd_doc(
                    indent=ctx["section_indent"],
                    doc=parser.doc,
                    max_width=ctx["width"] or get_wrap_width(max_width=ctx["max_width"]),
                )
            )
...
class HelpHandler(object):
    ...
    def __init__(
        self, flag=DEFAULT_HELP_FLAG, subcmd=None, formatter=StoutHelpFormatter, func=None, **formatter_kwargs
    ):
        ...
        if isinstance(formatter, type):
            ...
            raise TypeError(
                "only accepts extra formatter kwargs (%r) if"
                " formatter argument is a Formatter type, not: %r" % (sorted(formatter_kwargs.keys()), formatter)
            )
        _has_get_help_text = callable(getattr(self.formatter, "get_help_text", None))
        if not _has_get_help_text:
            raise TypeError(
                "expected valid formatter, or other object with a" " get_help_text() method, not %r" % (self.formatter,)
            )
        return
