...
class InvalidSubcommand(ArgumentParseError):
    ...
    def from_parse(cls, prs, subcmd_name):
        ...
        msg = 'unknown subcommand "%s", choose from: %s' % (subcmd_name, ", ".join(valid_subcmds))
        return cls(msg)
...
class UnknownFlag(ArgumentParseError):
    ...
    def from_parse(cls, cmd_flag_map, flag_name):
        # TODO: add edit distance calculation
        valid_flags = unique(
            [face.utils.format_flag_label(flag) for flag in cmd_flag_map.values() if not flag.display.hidden]
        )
        msg = 'unknown flag "%s", choose from: %s' % (flag_name, ", ".join(valid_flags))
        return cls(msg)
...
class InvalidPositionalArgument(ArgumentParseError):
    ...
    def from_parse(cls, posargspec, arg, exc):
        ...
        return cls("positional argument failed to parse %s" " %s: %r (got error: %r)" % (prep, type_desc, arg, exc))
...
class MissingRequiredFlags(ArgumentParseError):
    ...
    def from_parse(cls, cmd_flag_map, parsed_flag_map, missing_flag_names):
        ...
        msg = "missing required arguments for flags: %s" % ", ".join(sorted(labels))
        return cls(msg)
...
class DuplicateFlag(ArgumentParseError):
    ...
    def from_parse(cls, flag, arg_val_list):
        ...
        if callable(flag.parse_as):
            msg = 'more than one value was passed for flag "%s": %s' % (flag.name, avl_text)
        else:
