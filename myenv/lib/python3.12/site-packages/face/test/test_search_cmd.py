...
def get_search_command(as_parser=False):
    ...
    rg_subcmd.add("--max-count", char="-m", parse_as=int, doc="Limit the number of matching lines per file.")
    ...
    ls_subcmd = Command(
        _ls, "ls", posargs={"display": "file_path", "provides": "file_paths"}, post_posargs={"provides": "diff_paths"}
    )
    cmd.add(ls_subcmd)
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Module shortcut test not supported on Windows (mostly on github ci)"
)
