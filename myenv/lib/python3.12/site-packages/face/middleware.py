...
from face.sinter import get_arg_names, get_callable_labels, get_fb, make_chain
...
def get_middleware_chain(middlewares, innermost, preprovided):
    ...
    if mw_unres:
        ...
        if avail_unres:
            msg += " (%r provided but not resolvable, check middleware order.)" % sorted(avail_unres)
        raise NameError(msg)
    return mw_chain
...
def check_middleware(func, provides=None):
    ...
    if not arg_names:
        raise TypeError(
            "middleware function %r must take at least one"
            ' argument "%s" as its first parameter' % (func_label, INNER_NAME)
        )
    if arg_names[0] != INNER_NAME:
        raise TypeError(
            "middleware function %r must take argument"
            ' "%s" as the first parameter, not "%s"' % (func_label, INNER_NAME, arg_names[0])
        )
    if fb.varargs:
        raise TypeError(
            "middleware function %r may only take explicitly" ' named arguments, not "*%s"' % (func_label, fb.varargs)
        )
    if fb.varkw:
        raise TypeError(
            "middleware function %r may only take explicitly" ' named arguments, not "**%s"' % (func_label, fb.varkw)
        )
    ...
    if conflict_args:
        raise TypeError(
            "middleware function %r provides conflict with" " reserved face builtins: %r" % (func_label, conflict_args)
        )
