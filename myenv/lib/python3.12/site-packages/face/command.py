...
from face.parser import Flag, Parser
from face.utils import echo, get_rdep_map, unwrap_text
...
class Command(Parser):
    ...
    def __init__(self, func, name=None, doc=None, **kwargs):
        ...
        if not func and not _help:
            raise ValueError("Command requires a handler function or help handler" " to be set, not: %r" % func)
    ...
    def get_flag_map(self, path=(), with_hidden=True):
        ...
        return OrderedDict(
            [
                ...
                if f.name in dep_names or f is self.flagfile_flag or f is self.help_handler.flag
            ]
        )
    ...
    def prepare(self, paths=None):
        ...
        for path in paths:
            ...
            mws = [mw for mw in all_mws if not mw._face_optional or [p for p in mw._face_provides if p in deps]]
    ...
    def run(self, argv=None, extras=None, print_error=None):
        ...
        if print_error is None or print_error is True:
            ...
            raise TypeError("expected callable for print_error, not %r" % print_error)
