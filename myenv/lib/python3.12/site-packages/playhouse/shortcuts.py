...
from peewee import *
from peewee import Alias, CompoundSelectQuery, Metadata, __deprecated__, callable_
    ...
    if manytomany:
        for name, m2m in model._meta.manytomany.items():
            ...
            for rel_obj in getattr(model, name):
                accum.append(
                    model_to_dict(
                        rel_obj, recurse=recurse, backrefs=backrefs, only=only, exclude=exclude, max_depth=max_depth - 1
                    )
                )
            data[name] = accum
