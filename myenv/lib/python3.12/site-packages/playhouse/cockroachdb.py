...
from peewee import EnclosedNodeList, Entity, IndexMetadata, NodeList, _atomic, _manual
from playhouse.pool import _PooledPostgresqlDatabase

try:
    from playhouse.postgres_ext import ArrayField, BinaryJSONField, IntervalField
    ...
    def conflict_update(self, oc, query):
        ...
        if action in ("ignore", "nothing"):
            ...
            if oc._conflict_target:
                parts.append(
                    EnclosedNodeList(
                        [Entity(col) if isinstance(col, basestring) else col for col in oc._conflict_target]
                    )
                )
            ...
            raise ValueError(
                "CockroachDB does not support the usage of a " "constraint name. Use the column(s) instead."
            )
