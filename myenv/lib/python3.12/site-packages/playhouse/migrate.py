...
class CockroachDBMigrator(PostgresqlMigrator):
    ...
    @operation
    def drop_index(self, table, index_name):
        return self.make_context().literal("DROP INDEX ").sql(Entity(index_name)).literal(" CASCADE")
...
class MySQLMigrator(SchemaMigrator):
    ...
    def get_foreign_key_constraint(self, table, column_name):
        ...
        if not result:
            raise AttributeError(
                "Unable to find foreign key constraint for " '"%s" on table "%s".' % (table, column_name)
            )
        return result[0]
    ...
    def add_not_null(self, table, column):
        ...
        add_not_null = (
            self._alter_table(self.make_context(), table).literal(" MODIFY ").sql(column_def.sql(is_null=False))
        )
        ...
        return (
            ...
            add_not_null,
            self.add_foreign_key_constraint(table, column, fk_metadata.dest_table, fk_metadata.dest_column),
        )
    ...
    def rename_column(self, table, old_name, new_name):
        ...
        if is_foreign_key:
            ...
            return [
                ...
                rename_ctx,
                self.add_foreign_key_constraint(table, new_name, fk_metadata.dest_table, fk_metadata.dest_column),
            ]
        else:
            return rename_ctx
...
class SqliteMigrator(SchemaMigrator):
    ...
    def _get_create_table(self, table):
        res = self.database.execute_sql(
            ("select name, sql from sqlite_master " "where type=? and LOWER(name)=?"), ["table", table.lower()]
        )
        return res.fetchone()
