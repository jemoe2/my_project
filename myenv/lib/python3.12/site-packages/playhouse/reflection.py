...
class PostgresqlMetadata(Metadata):
    ...
    def get_column_types(self, table, schema):
        ...
        cursor = self.execute(
            "SELECT attname, atttypid FROM pg_catalog.pg_attribute " "WHERE attrelid = %s::regclass AND attnum > %s",
            identifier,
            0,
        )
    ...
    def multi_column_indexes(self, table):
        ...
        for index in self.indexes[table]:
            if len(index.columns) > 1:
                field_names = [
                    self.columns[table][column].name for column in index.columns if column in self.columns[table]
                ]
    ...
    def introspect(self, table_names=None, literal_column_names=False, include_views=False, snake_case=True):
        ...
        for table in tables:
            for foreign_key in foreign_keys[table]:
                ...
                src.set_foreign_key(
                    foreign_key=foreign_key, model_names=model_names, dest=dest, related_name=related_names.get(src)
                )

        return DatabaseMetadata(columns, primary_keys, foreign_keys, model_names, indexes)

    def generate_models(
        self, skip_invalid=False, table_names=None, literal_column_names=False, bare_fields=False, include_views=False
    ):
