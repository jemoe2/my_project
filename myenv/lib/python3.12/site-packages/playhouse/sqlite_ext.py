...
class FTS5Model(BaseFTSModel):
    ...
    * snippet(tbl, col_idx, prefix, suffix, ?, max_tokens)
    ...
    _error_messages = {
        "field_type": "Besides the implicit `rowid` column, all columns must " "be instances of SearchField",
    ...
    def _fts_cmd_sql(cls, cmd, **extra_params):
        ...
        return NodeList(
            (SQL("INSERT INTO"), cls._meta.entity, EnclosedNodeList(columns), SQL("VALUES"), EnclosedNodeList(values))
        )
        ...
        **kwargs,
        ...
        if c_extensions and not CYTHON_SQLITE_EXTENSIONS:
            raise ImproperlyConfigured(
                "SqliteExtDatabase initialized with " "C extensions, but shared library was " "not found!"
            )
