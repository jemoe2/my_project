...
class DataSet(object):
    def __init__(self, url, include_views=False, **kwargs):
        ...
        self._models = self._introspector.generate_models(
            skip_invalid=True, literal_column_names=True, include_views=self._include_views, **kwargs
        )
    ...
    def update_cache(self, table=None):
        if table:
            ...
            if table in self._models:
                ...
                dependencies.extend([related._meta.table_name for _, related, _ in model_class._meta.model_graph()])
            else:
        ...
        updated = self._introspector.generate_models(
            skip_invalid=True, table_names=dependencies, literal_column_names=True, include_views=self._include_views
        )
    ...
    def thaw(self, table, format="csv", filename=None, file_obj=None, strict=False, encoding="utf8", **kwargs):
        self._check_arguments(filename, file_obj, format, self._export_formats)
        if filename:
