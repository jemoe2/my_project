...
from semgrep.error import SemgrepError, error_type_string
...
class Metrics:
    ...
    def add_num_diff_scanned(self, scanned: List[Path], rules: List[Rule]) -> None:
        ...
        num_scanned = []
        for lang in langs:
            filtered = [path for path in scanned if any(str(path).endswith(ext) for ext in lang.definition.exts)]
            if filtered:
    ...
    def add_rules(self, rules: Sequence[Rule], profile: Optional[out.Profile]) -> None:
        ...
        if profile:
            ...
            self.payload.performance.ruleStats = [
                ...
                if _rule_match_times[rule.id2] > 0.0 and _rule_bytes_scanned[rule.id2] > 0
            ]
    ...
    def add_findings(self, findings: FilteredMatches) -> None:
        ...
        self.payload.value.numFindingsByProduct = [
            (USER_FRIENDLY_PRODUCT_NAMES[p], n_findings) for p, n_findings in _num_findings_by_product.items()
        ]
        ...
        if profile:
            self.payload.performance.fileStats = [
                FileStats(
                    ...
                    numTimesScanned=mock_int(len([x for x in target_times.match_times if x > 0.0])),
                    # TODO: we just have a single parse_time in target_times.parse_times
                    parseTime=mock_float(max(time for time in target_times.parse_times)),
                    matchTime=mock_float(sum(time for time in target_times.match_times)),
            ...
            self.payload.performance.fileStats = sorted(self.payload.performance.fileStats, key=lambda fs: fs.size)
    ...
    def add_errors(self, errors: List[SemgrepError]) -> None:
        self.payload.errors.errors = [met.Error(error_type_string(e.type_())) for e in errors]

    @suppress_errors
    def add_profiling(self, profiler: ProfileManager) -> None:
        self.payload.performance.profilingTimes = [(k, v) for k, v in profiler.dump_stats().items()]
