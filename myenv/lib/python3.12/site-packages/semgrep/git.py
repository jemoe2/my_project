...
class BaselineHandler:
    ...
    def print_git_log(self) -> None:
        ...
        if head_commit_sha != merge_base_sha:
            logger.info("  Will report findings introduced by these commits (may be incomplete for shallow checkouts):")
            log = git_check_output(["git", "log", "--oneline", "--graph", f"{merge_base_sha}..HEAD"])
            logger.info(indent(log, "    "))

        if merge_base_sha != base_commit_sha:
            logger.warning("  The current branch is missing these commits from the baseline branch:")
            ...
            logger.info("  Any finding these commits fixed will look like a new finding in the current branch.")
            logger.info(
