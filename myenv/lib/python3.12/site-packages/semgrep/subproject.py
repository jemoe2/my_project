    ...
    def to_stats_output(self) -> List[out.DependencySourceFile]:
        return [
            out.DependencySourceFile(
                kind=out.DependencySourceFileKind(value=out.Manifest_(value=self.manifest.kind)),
...
class LockfileOnlyDependencySource(DependencySource):
    ...
    def to_stats_output(self) -> List[out.DependencySourceFile]:
        return [
            out.DependencySourceFile(
                kind=out.DependencySourceFileKind(value=out.Lockfile_(value=self.lockfile.kind)),
...
class ManifestLockfileDependencySource(DependencySource):
    ...
    def to_semgrep_output(self) -> out.DependencySource:
        return out.DependencySource(out.ManifestLockfileDependencySource((self.manifest, self.lockfile)))
...
class MultiLockfileDependencySource(DependencySource):
    ...
    sources: Tuple[Union[LockfileOnlyDependencySource, ManifestLockfileDependencySource], ...]
    ...
    def get_all_source_files(self) -> List[Path]:
        return [source_file for source in self.sources for source_file in source.get_all_source_files()]
...
class ResolvedDependencies:
    ...
    _dependencies_by_package_version_pair: Dict[out.DependencyChild, List[FoundDependency]]

    @classmethod
    def from_resolved_interfaces(cls, resolved: out.ResolutionOk) -> "ResolvedDependencies":
        return cls.from_found_dependencies(resolved.value[0])

    @classmethod
    def from_found_dependencies(cls, found_deps: List[FoundDependency]) -> "ResolvedDependencies":
    ...
    def pretty_print(self) -> None:
        ...
        def pretty_print_dependency(
            ...
            if dep.children is not None:
                for child in dep.children:
                    ...
                    already_printed |= pretty_print_dependency(child_dep, indent + 2, already_printed)

            return already_printed

        print("Direct dependencies (plus children if available)")
        directs = [dep for dep in self.iter_found_dependencies() if dep.transitivity == out.Transitivity(out.Direct())]
        ...
        transitives = [
            ...
            for dep in dep_group
            if dep.transitivity == out.Transitivity(out.Transitive()) and package_version_pair not in printed_in_graph
        ]
        unknown = [
            dep
            for package_version_pair, dep_group in self._dependencies_by_package_version_pair.items()
            for dep in dep_group
            if dep.transitivity == out.Transitivity(out.Unknown()) and package_version_pair not in printed_in_graph
        ]
...
class Subproject:
    ...
    def to_stats_output(self) -> out.SubprojectStats:
        ...
        normalized_paths = sorted(str(path).strip() for path in self.dependency_source.get_display_paths())
        subproject_id = hashlib.sha256("".join(normalized_paths).encode("utf-8")).hexdigest()
...
class UnresolvedSubproject(Subproject):
    ...
    def from_subproject(
        ...
        base: Subproject,
        unresolved_reason: UnresolvedReason,
        resolution_errors: Sequence[Union[DependencyParserError, DependencyResolutionError]],
...
class ResolvedSubproject(Subproject):
    ...
    def from_unresolved(
        ...
        return cls(
            ...
            ecosystem=ecosystem,
            resolution_errors=list(resolution_errors),
            found_dependencies=ResolvedDependencies.from_found_dependencies(found_dependencies),
...
def find_closest_subproject(path: Path, ecosystem: Ecosystem, candidates: List[S]) -> Optional[S]:
    """
    ...
    sorted_candidates = sorted(candidates, key=lambda x: len(x.root_dir.parts), reverse=True)
