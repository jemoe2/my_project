...
from semgrep.constants import NOSEM_INLINE_COMMENT_RE, RuleScanSource
...
from semgrep.util import get_lines_from_file, get_lines_from_git_blob
...
class RuleMatch:
    ...
    ordering_key: Tuple[str, Position, Position, str, str] = field(init=False, repr=False)
    ...
    def get_lines(self) -> List[str]:
        ...
        """
        if self.git_blob:
            return get_lines_from_git_blob(self.git_blob, self.start.line, self.end.line)
    ...
    def get_path_changed_ci_unique_key(self, rename_dict: Dict[str, Path]) -> Tuple[str, str, str, int]:
        """
    ...
    def is_validation_state_blocking(self) -> bool:
        ...
        return self.metadata.get("dev.semgrep.validation_state.actions", {}).get(validation_state) == "block"
    ...
    def is_blocking(self) -> bool:
        ...
        if "sca_info" in self.extra:
            if (self.is_always_reachable_sca_match and self.is_sca_match_in_transitive_dependency) or (
                not self.exposure_type == "reachable"
            ):
                return False
            else:
                return blocking
    ...
    def to_app_finding_format(
        ...
        ret = out.Finding(
            ...
            dataflow_trace=remove_content(self.dataflow_trace) if remove_dataflow_content else self.dataflow_trace,
...
def remove_content(
    ...
    if isinstance(x, out.MatchDataflowTrace):
        ...
        intermediate_vars = (
            [remove_content_int_var(v) for v in x.intermediate_vars] if isinstance(x.intermediate_vars, list) else None
        )
