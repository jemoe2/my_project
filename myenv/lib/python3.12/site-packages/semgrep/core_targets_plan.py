...
class Plan:
        ...
        for task in self.target_mappings:
            result[task.language_label].append(
                ...
                if product is None
                    ...
                    products=(product,),
                    rule_nums=tuple(num for num in task.rule_nums if self.rules[num].product == product),
                )
            )
        return result
    ...
    def counts_by_ecosystem(
        ...
        if self.sca_subprojects is not None:
            unused_ecosystems = {ecosystem for ecosystem in result if not self.sca_subprojects.get(ecosystem)}
    ...
    def to_targets(self) -> out.Targets:
        ...
        return out.Targets(out.Targets_([task.to_target() for task in self.target_mappings]))
    ...
    def table_by_language(self, with_tables_for: Optional[out.Product] = None, use_color: bool = True) -> Table:
        table = Table(box=box.SIMPLE_HEAD, show_edge=False)
        table.add_column("Language", header_style=Style(color=None, bold=use_color))
        table.add_column("Rules", justify="right", header_style=Style(color=None, bold=use_color))
        table.add_column("Files", justify="right", header_style=Style(color=None, bold=use_color))
    ...
    def table_by_origin(self, with_tables_for: Optional[out.Product] = None, use_color: bool = True) -> Table:
        table = Table(box=box.SIMPLE_HEAD, show_edge=False)
