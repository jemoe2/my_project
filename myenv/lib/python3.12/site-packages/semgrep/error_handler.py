...
from semgrep.error import FINDINGS_EXIT_CODE, OK_EXIT_CODE
...
class ErrorHandler:
    ...
    def send(self, exit_code: int) -> int:
        ...
        if not self.is_enabled or exit_code == OK_EXIT_CODE or exit_code == FINDINGS_EXIT_CODE:
            return exit_code
        ...
        logger.debug(f"Sending to fail-open endpoint {url} since fail-open is configured to {self.suppress_errors}")
