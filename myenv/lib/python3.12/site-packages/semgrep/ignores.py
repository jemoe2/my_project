...
from typing import FrozenSet, Iterable, Iterator, Set, TextIO
...
MULTI_CHAR_REGEX = re.compile(r"(?!<\\)\[.*(?!<\\)\]")  # Matches anything in unescaped brackets
...
class FileIgnore:
    ...
    def _survives(self, path: Path) -> bool:
        ...
        matchable_path = str(path.relative_to(self.base_path)) if path_is_relative_to_base else str(path)
    ...
    def _filter(self, path: Path) -> bool:
        ...
        return path.exists() and (self._survives(absolute_path) or absolute_path.samefile(self.base_path))
...
class Parser:
    ...
    def remove_comments(line: str) -> Iterator[str]:
        ...
        if m:
            yield m.groupdict().get("ignore_pattern", "")  # return empty string if entire line is a comment
        else:
            yield line.rstrip()
    ...
    def expand_directives(self, line: str) -> Iterable[str]:
        ...
        if line.startswith(":include "):
            ...
            if include_path.is_file():
                ...
                logger.debug(f"Skipping `:include {include_path}` directive, file not found")
                return []
        elif line == ":":
            return []
        elif CONTROL_REGEX.match(line):
            raise SemgrepError(f"While parsing .semgrepignore: unknown ignore directive in {self.file_path}: '{line}'")
        else:
...
class Processor:
    ...
    def process(self, pre: Iterable[str]) -> Set[str]:
        ...
        patterns = {
            pattern for pat in pre for unescaped in self.unescape(pat) for pattern in self.to_fnmatch(unescaped)
        }
        return patterns
