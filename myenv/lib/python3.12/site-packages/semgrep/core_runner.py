...
from semgrep.constants import PLEASE_FILE_ISSUE_TEXT, Colors
from semgrep.core_output import (
    core_error_to_semgrep_error,
    core_matches_to_rule_matches,
)
from semgrep.core_targets_plan import Plan, Task
from semgrep.engine import EngineType
from semgrep.error import SemgrepCoreError, SemgrepError, with_color
from semgrep.output_extra import OutputExtra
from semgrep.parsing_data import ParsingData
from semgrep.rule import Rule
from semgrep.rule_match import OrderedRuleMatchList, RuleMatchMap
...
def setrlimits_preexec_fn() -> None:
    ...
    potential_soft_limits = [
        int(hard_limit / 3),  # Larger fractions cause "current limit exceeds maximum limit" for unknown reason
        int(hard_limit / 4),
        old_soft_limit * 100,
        old_soft_limit * 10,
        old_soft_limit * 5,
...
class StreamingSemgrepCore:
    ...
    def __init__(self, cmd: List[str], total: int, engine_type: EngineType, capture_stderr: bool) -> None:
        """
    ...
    def execute(self) -> int:
        ...
        with Progress(
            ...
            TimeElapsedColumn(),
            console=console,
            disable=(not sys.stderr.isatty() or self._total <= 1 or terminal.is_quiet or terminal.is_debug),
        ) as progress_bar:
...
class CoreRunner:
    ...
    def _extract_core_output(
        ...
        if returncode != 0:
            ...
            if "errors" in output_json:
                ...
                fail_msg = "non-zero exit status with one or more errors in json response"
                semgrep_errors = None
                if len(errors) < 1:
                    fail_msg = "non-zero exit status errors array is empty in json response"
                else:
        ...
        logger.debug(f"--- semgrep-core stderr ---\n" f"{core_stderr}" f"--- end semgrep-core stderr ---")
    ...
    def _parse_core_output(
        ...
        try:
            ...
            if returncode == -11 or returncode == -9:
                ...
                soft_limit, _hard_limit = (-1, -1) if IS_WINDOWS else resource.getrlimit(resource.RLIMIT_STACK)
                tip = f"""
    ...
    def plan_core_run(
        ...
        *,
        ...
        target_info: Dict[Tuple[Path, Language], Tuple[List[int], Set[str]]] = collections.defaultdict(
            lambda: (list(), set())
        )
        ...
        for rule_num, rule in enumerate(rules):
            ...
            for language in rule.languages:
                targets = list(
                    target_manager.get_files_for_rule(language, rule.includes, rule.excludes, rule.id, rule.product)
                )
    ...
    def _run_rules_direct_to_semgrep_core_helper(
        ...
        rule_file = exit_stack.enter_context(
            ...
            else tempfile.NamedTemporaryFile("w+", suffix=".json", delete=(not IS_WINDOWS))
        )
...
Could not find the semgrep-core executable. Your Semgrep install is likely corrupted. Please uninstall Semgrep and try again.
            ...
            total = plan.num_targets * 3 if show_progress else 0  # Multiply by 3 for Pro Engine
            ...
            for err in core_output.errors:
                if isinstance(err.error_type.value, out.Timeout):
                    ...
                    if (
                        ...
                        and file_timeouts[Path(err.location.path.value)] >= self._timeout_threshold
                    ):
...
Exception raised: `{e}`
    ...
    def validate_configs(self, configs: Tuple[str, ...]) -> Sequence[SemgrepError]:
        ...
        metachecks = Config.from_config_list(["p/semgrep-rule-lints"], None, force_jsonschema=True)[0].get_rules(True)

        parsed_errors = []

        with tempfile.NamedTemporaryFile("w", suffix=".yaml", delete=(not IS_WINDOWS)) as rule_file:
            yaml = YAML()
            ...
            runner = StreamingSemgrepCore(cmd, total=total, engine_type=self._engine_type, capture_stderr=True)
            returncode = runner.execute()
