...
from semgrep.config_resolver import ConfigLoader, get_config
...
from semgrep.error import (
    ...
    InvalidScanningRootError,
    SemgrepError,
    select_real_errors,
)
from semgrep.exclude_rules import filter_exclude_rule
from semgrep.git import BaselineHandler, get_project_url
from semgrep.ignores import IGNORE_FILE_NAME, FileIgnore, Parser
from semgrep.nosemgrep import filter_ignored
from semgrep.output import DEFAULT_SHOWN_SEVERITIES, OutputHandler, OutputSettings
...
from semgrep.subproject import ResolvedSubproject, UnresolvedSubproject
from semgrep.target_manager import (
    SAST_PRODUCT,
    SCA_PRODUCT,
    SECRETS_PRODUCT,
    FileTargetingLog,
...
def file_ignore_to_ignore_profiles(
    ...
    return {
        ...
        SECRETS_PRODUCT: FileIgnore(file_ignore.base_path, frozenset(), max_log_list_entries=0),
    }
...
def run_rules(
    ...
    *,
    ...
    dependency_aware_rules: List[Rule] = [r for r in rest_of_the_rules if r.project_depends_on]
    ...
    if len(dependency_aware_rules) > 0:
        ...
        for subproject in all_subprojects:
            dependency_parser_errors.extend(
                [e for e in subproject.resolution_errors if isinstance(e, DependencyParserError)]
            )
            output_handler.handle_semgrep_errors(
                [e for e in subproject.resolution_errors if isinstance(e, DependencyResolutionError)]
            )

        # Filter rules that match the dependencies
        filtered_dependency_aware_rules = filter_dependency_aware_rules(dependency_aware_rules, resolved_subprojects)
        ...
        for rule in dependency_aware_rules:
            if rule.should_run_on_semgrep_core:
                ...
                (
                    dep_rule_matches,
                    dep_rule_errors,
                ) = generate_unreachable_sca_findings(rule, lambda p, d: False, resolved_subprojects)
                rule_matches_by_rule[rule] = dep_rule_matches
                output_handler.handle_semgrep_errors(dep_rule_errors)
        ...
        for ecosystem in resolved_subprojects:
            for proj in resolved_subprojects[ecosystem]:
                ...
                for dep in unknown_lockfile_deps:
                    if str(proj.root_dir.joinpath(Path("unknown_lockfile"))) not in deps_by_lockfile:
                        deps_by_lockfile[str(proj.root_dir.joinpath(Path("unknown_lockfile")))] = []
                    deps_by_lockfile[str(proj.root_dir.joinpath(Path("unknown_lockfile")))].append(dep)
...
def list_targets_and_exit(target_manager: TargetManager, product: out.Product, long_format: bool = False) -> None:
...
def run_scan(
    *,
    ...
    progress_msg = "Loading rules from registry..." if includes_remote_config else "Loading rules..."
    ...
    if dump_n_rule_partitions:
        ...
        args = out.DumpRulePartitionsParams(out.RawJson(rules), dump_n_rule_partitions, out.Fpath(output_dir))
    ...
    if not pattern:
        ...
        invalid_msg = f"({unit_str(len(config_errors), 'config error')})" if len(config_errors) else ""
        logger.verbose(
...
""",
    ...
    if baseline_commit:
        try:
            ...
            raise SemgrepError(f"Exception in BaselineHandler initialization: {exception_with_trace}")
    ...
    try:
        target_manager = TargetManager(
            ...
            ignore_profiles=file_ignore_to_ignore_profiles(get_file_ignore(too_many_entries)),
            respect_semgrepignore=respect_semgrepignore,
        )
        # Debugging option --x-ls
        if x_ls or x_ls_long:
            list_targets_and_exit(target_manager, out.Product(out.SAST()), long_format=x_ls_long)
    ...
    findings_count = sum(
        len([match for match in matches if not match.from_transient_scan]) for matches in rule_matches_by_rule.values()
    )
                ...
                with baseline_handler.baseline_context():
                    ...
                    if target_mode_config.is_pro_diff_scan:
                        ...
                        baseline_target_mode_config = TargetModeConfig.pro_diff_scan(
                            frozenset(t for t in scanned if t.exists() and not t.is_symlink()),
                            0,  # scanning the same set of files in the second run
                        )
                    else:
                        baseline_target_strings = frozenset(
                            Path(t) for t in baseline_targets if t.exists() and not t.is_symlink()
                        )
    ...
    filtered_matches_by_rule = filter_ignored(rule_matches_by_rule, keep_ignored=keep_ignored)
    ...
    if metrics.is_enabled:
        ...
        if engine_type.is_pro and baseline_handler:
            metrics.add_num_diff_scanned({Path(t.value) for t in output_extra.core.paths.scanned}, filtered_rules)
