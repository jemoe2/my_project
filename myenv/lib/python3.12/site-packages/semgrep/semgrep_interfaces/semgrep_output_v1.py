...
def _atd_missing_json_field(type_name: str, json_field_name: str) -> NoReturn:
    raise ValueError(f"missing field '{json_field_name}'" f" in JSON object of type '{type_name}'")
...
def _atd_bad_json(expected_type: str, json_value: Any) -> NoReturn:
    ...
    raise ValueError(f"incompatible JSON value where" f" type '{expected_type}' was expected: '{value_str}'")
...
def _atd_bad_python(expected_type: str, json_value: Any) -> NoReturn:
    ...
    raise ValueError(f"incompatible Python value where" f" type '{expected_type}' was expected: '{value_str}'")
...
def _atd_read_assoc_object_into_dict(read_value: Callable[[Any], Any]) -> Callable[[Dict[str, Any]], Dict[str, Any]]:
...
def _atd_read_nullable(read_elt: Callable[[Any], Any]) -> Callable[[Optional[Any]], Optional[Any]]:
...
def _atd_read_option(read_elt: Callable[[Any], Any]) -> Callable[[Optional[Any]], Optional[Any]]:
...
def _atd_write_list(write_elt: Callable[[Any], Any]) -> Callable[[List[Any]], List[Any]]:
...
def _atd_write_assoc_dict_to_object(write_value: Callable[[Any], Any]) -> Callable[[Dict[str, Any]], Dict[str, Any]]:
...
def _atd_write_nullable(write_elt: Callable[[Any], Any]) -> Callable[[Optional[Any]], Optional[Any]]:
...
def _atd_write_option(write_elt: Callable[[Any], Any]) -> Callable[[Optional[Any]], Optional[Any]]:
...
class DependencyChild:
    ...
    def from_json(cls, x: Any) -> "DependencyChild":
        if isinstance(x, dict):
            return cls(
                package=(
                    _atd_read_string(x["package"])
                    if "package" in x
                    else _atd_missing_json_field("DependencyChild", "package")
                ),
                version=(
                    _atd_read_string(x["version"])
                    if "version" in x
                    else _atd_missing_json_field("DependencyChild", "version")
                ),
            )
        else:
...
class LocAndContent:
    ...
    def from_json(cls, x: Any) -> "LocAndContent":
        return cls(
            (
                lambda x: (
                    (Location.from_json(x[0]), _atd_read_string(x[1]))
                    if isinstance(x, list) and len(x) == 2
    ...
    def to_json(self) -> Any:
        return (
            lambda x: (
                [(lambda x: x.to_json())(x[0]), _atd_write_string(x[1])]
...
class MatchIntermediateVar:
    ...
    def from_json(cls, x: Any) -> "MatchIntermediateVar":
        if isinstance(x, dict):
            return cls(
                location=(
                    Location.from_json(x["location"])
                    if "location" in x
                    else _atd_missing_json_field("MatchIntermediateVar", "location")
                ),
                content=(
                    _atd_read_string(x["content"])
                    if "content" in x
                    else _atd_missing_json_field("MatchIntermediateVar", "content")
                ),
            )
        else:
...
class ProFeature:
    ...
    def from_json(cls, x: Any) -> "ProFeature":
        if isinstance(x, dict):
            return cls(
                interproc_taint=(
                    _atd_read_bool(x["interproc_taint"])
                    if "interproc_taint" in x
                    else _atd_missing_json_field("ProFeature", "interproc_taint")
                ),
                interfile_taint=(
                    _atd_read_bool(x["interfile_taint"])
                    if "interfile_taint" in x
                    else _atd_missing_json_field("ProFeature", "interfile_taint")
                ),
                proprietary_language=(
                    _atd_read_bool(x["proprietary_language"])
                    if "proprietary_language" in x
                    else _atd_missing_json_field("ProFeature", "proprietary_language")
                ),
            )
        else:
...
class ScaPattern:
    ...
    def from_json(cls, x: Any) -> "ScaPattern":
        if isinstance(x, dict):
            return cls(
                ecosystem=(
                    Ecosystem.from_json(x["ecosystem"])
                    if "ecosystem" in x
                    else _atd_missing_json_field("ScaPattern", "ecosystem")
                ),
                package=(
                    _atd_read_string(x["package"])
                    if "package" in x
                    else _atd_missing_json_field("ScaPattern", "package")
                ),
                semver_range=(
                    _atd_read_string(x["semver_range"])
                    if "semver_range" in x
                    else _atd_missing_json_field("ScaPattern", "semver_range")
                ),
            )
        else:
...
class HistoricalInfo:
    ...
    def from_json(cls, x: Any) -> "HistoricalInfo":
        if isinstance(x, dict):
            return cls(
                git_commit=(
                    Sha1.from_json(x["git_commit"])
                    if "git_commit" in x
                    else _atd_missing_json_field("HistoricalInfo", "git_commit")
                ),
                git_commit_timestamp=(
                    Datetime.from_json(x["git_commit_timestamp"])
                    if "git_commit_timestamp" in x
                    else _atd_missing_json_field("HistoricalInfo", "git_commit_timestamp")
                ),
        ...
        if isinstance(x, dict):
            return cls(
                svalue_abstract_content=(
                    _atd_read_string(x["svalue_abstract_content"])
...
class MetavarValue:
    ...
    def from_json(cls, x: Any) -> "MetavarValue":
        if isinstance(x, dict):
            return cls(
                start=(
                    Position.from_json(x["start"]) if "start" in x else _atd_missing_json_field("MetavarValue", "start")
                ),
                end=Position.from_json(x["end"]) if "end" in x else _atd_missing_json_field("MetavarValue", "end"),
                abstract_content=(
                    _atd_read_string(x["abstract_content"])
        ...
        if isinstance(x, dict):
            return cls(
                package=(
                    _atd_read_string(x["package"])
                    if "package" in x
                    else _atd_missing_json_field("FoundDependency", "package")
                ),
                version=(
                    _atd_read_string(x["version"])
                    if "version" in x
                    else _atd_missing_json_field("FoundDependency", "version")
                ),
                ecosystem=(
                    Ecosystem.from_json(x["ecosystem"])
                    if "ecosystem" in x
                    else _atd_missing_json_field("FoundDependency", "ecosystem")
                ),
                allowed_hashes=(
                    _atd_read_assoc_object_into_dict(_atd_read_list(_atd_read_string))(x["allowed_hashes"])
                    if "allowed_hashes" in x
                    else _atd_missing_json_field("FoundDependency", "allowed_hashes")
                ),
                transitivity=(
                    Transitivity.from_json(x["transitivity"])
                    if "transitivity" in x
                    else _atd_missing_json_field("FoundDependency", "transitivity")
                ),
...
class DependencyMatch:
    ...
    def from_json(cls, x: Any) -> "DependencyMatch":
        if isinstance(x, dict):
            return cls(
                dependency_pattern=(
                    ScaPattern.from_json(x["dependency_pattern"])
                    if "dependency_pattern" in x
                    else _atd_missing_json_field("DependencyMatch", "dependency_pattern")
                ),
                found_dependency=(
                    FoundDependency.from_json(x["found_dependency"])
                    if "found_dependency" in x
                    else _atd_missing_json_field("DependencyMatch", "found_dependency")
                ),
                lockfile=(
                    Fpath.from_json(x["lockfile"])
                    if "lockfile" in x
                    else _atd_missing_json_field("DependencyMatch", "lockfile")
                ),
            )
        else:
...
class ScaMatch:
    ...
    def from_json(cls, x: Any) -> "ScaMatch":
        if isinstance(x, dict):
            return cls(
                reachable=(
                    _atd_read_bool(x["reachable"])
                    if "reachable" in x
                    else _atd_missing_json_field("ScaMatch", "reachable")
                ),
                reachability_rule=(
                    _atd_read_bool(x["reachability_rule"])
                    if "reachability_rule" in x
                    else _atd_missing_json_field("ScaMatch", "reachability_rule")
                ),
                sca_finding_schema=(
                    _atd_read_int(x["sca_finding_schema"])
                    if "sca_finding_schema" in x
                    else _atd_missing_json_field("ScaMatch", "sca_finding_schema")
                ),
                dependency_match=(
                    DependencyMatch.from_json(x["dependency_match"])
                    if "dependency_match" in x
                    else _atd_missing_json_field("ScaMatch", "dependency_match")
                ),
            )
        else:
                ...
                return cls(
                    CliCall(
                        (
                            lambda x: (
                                (
                                    LocAndContent.from_json(x[0]),
                                    _atd_read_list(MatchIntermediateVar.from_json)(x[1]),
                                    MatchCallTrace.from_json(x[2]),
                                )
    ...
    def from_json(cls, x: Any) -> "MatchDataflowTrace":
        if isinstance(x, dict):
            return cls(
                ...
                intermediate_vars=(
                    _atd_read_list(MatchIntermediateVar.from_json)(x["intermediate_vars"])
                    if "intermediate_vars" in x
                    else None
                ),
...
class CoreMatchExtra:
    ...
    def from_json(cls, x: Any) -> "CoreMatchExtra":
        if isinstance(x, dict):
            return cls(
                metavars=(
                    Metavars.from_json(x["metavars"])
                    if "metavars" in x
                    else _atd_missing_json_field("CoreMatchExtra", "metavars")
                ),
                engine_kind=(
                    EngineOfFinding.from_json(x["engine_kind"])
                    if "engine_kind" in x
                    else _atd_missing_json_field("CoreMatchExtra", "engine_kind")
                ),
                is_ignored=(
                    _atd_read_bool(x["is_ignored"])
                    if "is_ignored" in x
                    else _atd_missing_json_field("CoreMatchExtra", "is_ignored")
                ),
...
class CoreMatch:
    ...
    def from_json(cls, x: Any) -> "CoreMatch":
        if isinstance(x, dict):
            return cls(
                check_id=(
                    RuleId.from_json(x["check_id"])
                    if "check_id" in x
                    else _atd_missing_json_field("CoreMatch", "check_id")
                ),
                ...
                extra=(
                    CoreMatchExtra.from_json(x["extra"])
                    if "extra" in x
                    else _atd_missing_json_field("CoreMatch", "extra")
                ),
            )
        else:
...
class MatchingExplanationExtra:
        ...
        if isinstance(x, dict):
            return cls(
                before_negation_matches=(
                    _atd_read_option(_atd_read_list(CoreMatch.from_json))(x["before_negation_matches"])
                    if "before_negation_matches" in x
                    else _atd_missing_json_field("MatchingExplanationExtra", "before_negation_matches")
                ),
                before_filter_matches=(
                    _atd_read_option(_atd_read_list(CoreMatch.from_json))(x["before_filter_matches"])
                    if "before_filter_matches" in x
                    else _atd_missing_json_field("MatchingExplanationExtra", "before_filter_matches")
                ),
            )
        else:
    ...
    def to_json(self) -> Any:
        res: Dict[str, Any] = {}
        res["before_negation_matches"] = _atd_write_option(_atd_write_list((lambda x: x.to_json())))(
            self.before_negation_matches
        )
        res["before_filter_matches"] = _atd_write_option(_atd_write_list((lambda x: x.to_json())))(
            self.before_filter_matches
        )
        return res
...
class MatchingExplanation:
    ...
    def from_json(cls, x: Any) -> "MatchingExplanation":
        if isinstance(x, dict):
            return cls(
                op=(
                    MatchingOperation.from_json(x["op"])
                    if "op" in x
                    else _atd_missing_json_field("MatchingExplanation", "op")
                ),
                children=(
                    _atd_read_list(MatchingExplanation.from_json)(x["children"])
                    if "children" in x
                    else _atd_missing_json_field("MatchingExplanation", "children")
                ),
                matches=(
                    _atd_read_list(CoreMatch.from_json)(x["matches"])
                    if "matches" in x
                    else _atd_missing_json_field("MatchingExplanation", "matches")
                ),
                loc=(
                    Location.from_json(x["loc"])
                    if "loc" in x
                    else _atd_missing_json_field("MatchingExplanation", "loc")
                ),
...
class KillingParent:
    ...
    def from_json(cls, x: Any) -> "KillingParent":
        if isinstance(x, dict):
            return cls(
                killing_parent_kind=(
                    KillingParentKind.from_json(x["killing_parent_kind"])
                    if "killing_parent_kind" in x
                    else _atd_missing_json_field("KillingParent", "killing_parent_kind")
                ),
                snippet=(
                    Snippet.from_json(x["snippet"])
                    if "snippet" in x
                    else _atd_missing_json_field("KillingParent", "snippet")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                line=(
                    ...
                    if "line" in x
                    else _atd_missing_json_field("UnexpectedNoMatchDiagnosis", "line")
                ),
                kind=(
                    UnexpectedNoMatchDiagnosisKind.from_json(x["kind"])
                    if "kind" in x
                    else _atd_missing_json_field("UnexpectedNoMatchDiagnosis", "kind")
                ),
            )
        else:
...
class UnexpectedMatchDiagnosis:
    ...
    def from_json(cls, x: Any) -> "UnexpectedMatchDiagnosis":
        if isinstance(x, dict):
            return cls(
                matched_text=(
                    Snippet.from_json(x["matched_text"])
                    if "matched_text" in x
                    else _atd_missing_json_field("UnexpectedMatchDiagnosis", "matched_text")
                ),
                originating_kind=(
                    OriginatingNodeKind.from_json(x["originating_kind"])
                    if "originating_kind" in x
                    else _atd_missing_json_field("UnexpectedMatchDiagnosis", "originating_kind")
                ),
                originating_text=(
                    Snippet.from_json(x["originating_text"])
                    if "originating_text" in x
                    else _atd_missing_json_field("UnexpectedMatchDiagnosis", "originating_text")
                ),
                killing_parents=(
                    _atd_read_list(KillingParent.from_json)(x["killing_parents"])
                    if "killing_parents" in x
                    else _atd_missing_json_field("UnexpectedMatchDiagnosis", "killing_parents")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                triage_ignored_syntactic_ids=(
                    _atd_read_list(_atd_read_string)(x["triage_ignored_syntactic_ids"])
                    if "triage_ignored_syntactic_ids" in x
                    else []
                ),
                triage_ignored_match_based_ids=(
                    _atd_read_list(_atd_read_string)(x["triage_ignored_match_based_ids"])
                    if "triage_ignored_match_based_ids" in x
                    else []
                ),
            )
        else:
...
class MatchingDiagnosis:
    ...
    def from_json(cls, x: Any) -> "MatchingDiagnosis":
        if isinstance(x, dict):
            return cls(
                target=(
                    Fpath.from_json(x["target"])
                    if "target" in x
                    else _atd_missing_json_field("MatchingDiagnosis", "target")
                ),
                unexpected_match_diagnoses=(
                    _atd_read_list(UnexpectedMatchDiagnosis.from_json)(x["unexpected_match_diagnoses"])
                    if "unexpected_match_diagnoses" in x
                    else _atd_missing_json_field("MatchingDiagnosis", "unexpected_match_diagnoses")
                ),
                unexpected_no_match_diagnoses=(
                    _atd_read_list(UnexpectedNoMatchDiagnosis.from_json)(x["unexpected_no_match_diagnoses"])
                    if "unexpected_no_match_diagnoses" in x
                    else _atd_missing_json_field("MatchingDiagnosis", "unexpected_no_match_diagnoses")
                ),
            )
        else:
    ...
    def to_json(self) -> Any:
        ...
        res["unexpected_no_match_diagnoses"] = _atd_write_list((lambda x: x.to_json()))(
            self.unexpected_no_match_diagnoses
        )
        return res
...
class ExpectedReported:
    ...
    def from_json(cls, x: Any) -> "ExpectedReported":
        if isinstance(x, dict):
            return cls(
                expected_lines=(
                    _atd_read_list(_atd_read_int)(x["expected_lines"])
                    if "expected_lines" in x
                    else _atd_missing_json_field("ExpectedReported", "expected_lines")
                ),
                reported_lines=(
                    _atd_read_list(_atd_read_int)(x["reported_lines"])
                    if "reported_lines" in x
                    else _atd_missing_json_field("ExpectedReported", "reported_lines")
                ),
            )
        else:
...
class RuleResult:
    ...
    def from_json(cls, x: Any) -> "RuleResult":
        if isinstance(x, dict):
            return cls(
                passed=(
                    _atd_read_bool(x["passed"]) if "passed" in x else _atd_missing_json_field("RuleResult", "passed")
                ),
                matches=(
                    _atd_read_assoc_object_into_list(ExpectedReported.from_json)(x["matches"])
                    if "matches" in x
                    else _atd_missing_json_field("RuleResult", "matches")
                ),
                errors=(
                    _atd_read_list(Todo.from_json)(x["errors"])
                    if "errors" in x
                    else _atd_missing_json_field("RuleResult", "errors")
                ),
...
class FixtestResult:
    ...
    def from_json(cls, x: Any) -> "FixtestResult":
        if isinstance(x, dict):
            return cls(
                passed=(
                    _atd_read_bool(x["passed"]) if "passed" in x else _atd_missing_json_field("FixtestResult", "passed")
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "ConfigError":
        if isinstance(x, dict):
            return cls(
                ...
                reason=(
                    ConfigErrorReason.from_json(x["reason"])
                    if "reason" in x
                    else _atd_missing_json_field("ConfigError", "reason")
                ),
            )
        else:
...
class Checks:
    ...
    def from_json(cls, x: Any) -> "Checks":
        if isinstance(x, dict):
            return cls(
                checks=(
                    _atd_read_assoc_object_into_list(RuleResult.from_json)(x["checks"])
                    if "checks" in x
                    else _atd_missing_json_field("Checks", "checks")
                ),
            )
        else:
...
class TestsResult:
    ...
    def from_json(cls, x: Any) -> "TestsResult":
        if isinstance(x, dict):
            return cls(
                results=(
                    _atd_read_assoc_object_into_list(Checks.from_json)(x["results"])
                    if "results" in x
                    else _atd_missing_json_field("TestsResult", "results")
                ),
                fixtest_results=(
                    _atd_read_assoc_object_into_list(FixtestResult.from_json)(x["fixtest_results"])
                    if "fixtest_results" in x
                    else _atd_missing_json_field("TestsResult", "fixtest_results")
                ),
                config_missing_tests=(
                    _atd_read_list(Fpath.from_json)(x["config_missing_tests"])
                    if "config_missing_tests" in x
                    else _atd_missing_json_field("TestsResult", "config_missing_tests")
                ),
                config_missing_fixtests=(
                    _atd_read_list(Fpath.from_json)(x["config_missing_fixtests"])
                    if "config_missing_fixtests" in x
                    else _atd_missing_json_field("TestsResult", "config_missing_fixtests")
                ),
                config_with_errors=(
                    _atd_read_list(ConfigError.from_json)(x["config_with_errors"])
                    if "config_with_errors" in x
                    else _atd_missing_json_field("TestsResult", "config_with_errors")
                ),
            )
        else:
...
class TargetingConf:
    ...
    def from_json(cls, x: Any) -> "TargetingConf":
        if isinstance(x, dict):
            return cls(
                exclude=(
                    _atd_read_list(_atd_read_string)(x["exclude"])
                    if "exclude" in x
                    else _atd_missing_json_field("TargetingConf", "exclude")
                ),
                max_target_bytes=(
                    _atd_read_int(x["max_target_bytes"])
                    if "max_target_bytes" in x
                    else _atd_missing_json_field("TargetingConf", "max_target_bytes")
                ),
                respect_gitignore=(
                    _atd_read_bool(x["respect_gitignore"])
                    if "respect_gitignore" in x
                    else _atd_missing_json_field("TargetingConf", "respect_gitignore")
                ),
                respect_semgrepignore_files=(
                    _atd_read_bool(x["respect_semgrepignore_files"])
                    if "respect_semgrepignore_files" in x
                    else _atd_missing_json_field("TargetingConf", "respect_semgrepignore_files")
                ),
                always_select_explicit_targets=(
                    _atd_read_bool(x["always_select_explicit_targets"])
                    if "always_select_explicit_targets" in x
                    else _atd_missing_json_field("TargetingConf", "always_select_explicit_targets")
                ),
                explicit_targets=(
                    _atd_read_list(_atd_read_string)(x["explicit_targets"])
                    if "explicit_targets" in x
                    else _atd_missing_json_field("TargetingConf", "explicit_targets")
                ),
                force_novcs_project=(
                    _atd_read_bool(x["force_novcs_project"])
                    if "force_novcs_project" in x
                    else _atd_missing_json_field("TargetingConf", "force_novcs_project")
                ),
                exclude_minified_files=(
                    _atd_read_bool(x["exclude_minified_files"])
                    if "exclude_minified_files" in x
                    else _atd_missing_json_field("TargetingConf", "exclude_minified_files")
                ),
                diff_depth=(
                    _atd_read_int(x["diff_depth"])
                    if "diff_depth" in x
                    else _atd_missing_json_field("TargetingConf", "diff_depth")
                ),
                include_=_atd_read_list(_atd_read_string)(x["include_"]) if "include_" in x else None,
                force_project_root=(
                    ProjectRoot.from_json(x["force_project_root"]) if "force_project_root" in x else None
                ),
    ...
    def from_json(cls, x: Any) -> "CodeTarget":
        if isinstance(x, dict):
            return cls(
                ...
                analyzer=(
                    Analyzer.from_json(x["analyzer"])
                    if "analyzer" in x
                    else _atd_missing_json_field("CodeTarget", "analyzer")
                ),
                products=(
                    _atd_read_list(Product.from_json)(x["products"])
                    if "products" in x
                    else _atd_missing_json_field("CodeTarget", "products")
                ),
...
class ScanningRoots:
    ...
    def from_json(cls, x: Any) -> "ScanningRoots":
        if isinstance(x, dict):
            return cls(
                root_paths=(
                    _atd_read_list(Fpath.from_json)(x["root_paths"])
                    if "root_paths" in x
                    else _atd_missing_json_field("ScanningRoots", "root_paths")
                ),
                targeting_conf=(
                    TargetingConf.from_json(x["targeting_conf"])
                    if "targeting_conf" in x
                    else _atd_missing_json_field("ScanningRoots", "targeting_conf")
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "TargetTimes":
        if isinstance(x, dict):
            return cls(
                ...
                num_bytes=(
                    _atd_read_int(x["num_bytes"])
                    if "num_bytes" in x
                    else _atd_missing_json_field("TargetTimes", "num_bytes")
                ),
                match_times=(
                    _atd_read_list(_atd_read_float)(x["match_times"])
                    if "match_times" in x
                    else _atd_missing_json_field("TargetTimes", "match_times")
                ),
                parse_times=(
                    _atd_read_list(_atd_read_float)(x["parse_times"])
                    if "parse_times" in x
                    else _atd_missing_json_field("TargetTimes", "parse_times")
                ),
                run_time=(
                    _atd_read_float(x["run_time"])
                    if "run_time" in x
                    else _atd_missing_json_field("TargetTimes", "run_time")
                ),
            )
        else:
...
class DependencySourceFile:
    ...
    def from_json(cls, x: Any) -> "DependencySourceFile":
        if isinstance(x, dict):
            return cls(
                kind=(
                    DependencySourceFileKind.from_json(x["kind"])
                    if "kind" in x
                    else _atd_missing_json_field("DependencySourceFile", "kind")
                ),
                path=(
                    Fpath.from_json(x["path"])
                    if "path" in x
                    else _atd_missing_json_field("DependencySourceFile", "path")
                ),
            )
        else:
...
class DependencyResolutionStats:
    ...
    def from_json(cls, x: Any) -> "DependencyResolutionStats":
        if isinstance(x, dict):
            return cls(
                resolution_method=(
                    ResolutionMethod.from_json(x["resolution_method"])
                    if "resolution_method" in x
                    else _atd_missing_json_field("DependencyResolutionStats", "resolution_method")
                ),
                dependency_count=(
                    _atd_read_int(x["dependency_count"])
                    if "dependency_count" in x
                    else _atd_missing_json_field("DependencyResolutionStats", "dependency_count")
                ),
                ecosystem=(
                    Ecosystem.from_json(x["ecosystem"])
                    if "ecosystem" in x
                    else _atd_missing_json_field("DependencyResolutionStats", "ecosystem")
                ),
            )
        else:
...
class SubprojectStats:
    ...
    def from_json(cls, x: Any) -> "SubprojectStats":
        if isinstance(x, dict):
            return cls(
                subproject_id=(
                    _atd_read_string(x["subproject_id"])
                    if "subproject_id" in x
                    else _atd_missing_json_field("SubprojectStats", "subproject_id")
                ),
                dependency_sources=(
                    _atd_read_list(DependencySourceFile.from_json)(x["dependency_sources"])
                    if "dependency_sources" in x
                    else _atd_missing_json_field("SubprojectStats", "dependency_sources")
                ),
                resolved_stats=(
                    DependencyResolutionStats.from_json(x["resolved_stats"]) if "resolved_stats" in x else None
                ),
            )
        else:
...
class SupplyChainStats:
    ...
    def from_json(cls, x: Any) -> "SupplyChainStats":
        if isinstance(x, dict):
            return cls(
                subprojects_stats=(
                    _atd_read_list(SubprojectStats.from_json)(x["subprojects_stats"])
                    if "subprojects_stats" in x
                    else _atd_missing_json_field("SupplyChainStats", "subprojects_stats")
                ),
            )
        else:
...
class SkipReason:
    ...
    value: Union[
        AlwaysSkipped,
        SemgrepignorePatternsMatch,
        CliIncludeFlagsDoNotMatch,
        CliExcludeFlagsMatch,
        ExceededSizeLimit,
        AnalysisFailedParserOrInternalError,
        ExcludedByConfig,
        WrongLanguage,
        TooBig,
        Minified,
...
class SkippedRule:
    ...
    def from_json(cls, x: Any) -> "SkippedRule":
        if isinstance(x, dict):
            return cls(
                rule_id=(
                    RuleId.from_json(x["rule_id"])
                    if "rule_id" in x
                    else _atd_missing_json_field("SkippedRule", "rule_id")
                ),
                details=(
                    _atd_read_string(x["details"])
                    if "details" in x
...
class ScannedAndSkipped:
    ...
    def from_json(cls, x: Any) -> "ScannedAndSkipped":
        if isinstance(x, dict):
            return cls(
                scanned=(
                    _atd_read_list(Fpath.from_json)(x["scanned"])
                    if "scanned" in x
                    else _atd_missing_json_field("ScannedAndSkipped", "scanned")
                ),
...
class ScanInfo:
    ...
    def from_json(cls, x: Any) -> "ScanInfo":
        if isinstance(x, dict):
            return cls(
                enabled_products=(
                    _atd_read_list(Product.from_json)(x["enabled_products"])
                    if "enabled_products" in x
                    else _atd_missing_json_field("ScanInfo", "enabled_products")
                ),
                deployment_id=(
                    _atd_read_int(x["deployment_id"])
                    if "deployment_id" in x
                    else _atd_missing_json_field("ScanInfo", "deployment_id")
                ),
                deployment_name=(
                    _atd_read_string(x["deployment_name"])
                    if "deployment_name" in x
                    else _atd_missing_json_field("ScanInfo", "deployment_name")
                ),
...
class ScanConfiguration:
    ...
    def from_json(cls, x: Any) -> "ScanConfiguration":
        if isinstance(x, dict):
            return cls(
                rules=(
                    RawJson.from_json(x["rules"])
                    if "rules" in x
                    else _atd_missing_json_field("ScanConfiguration", "rules")
                ),
                triage_ignored_syntactic_ids=(
                    _atd_read_list(_atd_read_string)(x["triage_ignored_syntactic_ids"])
                    if "triage_ignored_syntactic_ids" in x
...
class ProductIgnoredFiles:
    ...
    def to_json(self) -> Any:
        return _atd_write_assoc_dict_to_array((lambda x: x.to_json()), _atd_write_list((lambda x: x.to_json())))(
            self.value
        )
...
class HistoricalConfiguration:
    ...
    def from_json(cls, x: Any) -> "HistoricalConfiguration":
        if isinstance(x, dict):
            return cls(
                enabled=(
                    _atd_read_bool(x["enabled"])
                    if "enabled" in x
                    else _atd_missing_json_field("HistoricalConfiguration", "enabled")
                ),
        ...
        if isinstance(x, dict):
            return cls(
                ...
                path_to_transitivity=(
                    _atd_read_bool(x["path_to_transitivity"]) if "path_to_transitivity" in x else False
                ),
                scan_all_deps_in_diff_scan=(
                    _atd_read_bool(x["scan_all_deps_in_diff_scan"]) if "scan_all_deps_in_diff_scan" in x else False
                ),
                ignored_files=_atd_read_list(_atd_read_string)(x["ignored_files"]) if "ignored_files" in x else [],
                product_ignored_files=(
                    ProductIgnoredFiles.from_json(x["product_ignored_files"]) if "product_ignored_files" in x else None
                ),
                generic_slow_rollout=(
                    _atd_read_bool(x["generic_slow_rollout"]) if "generic_slow_rollout" in x else False
                ),
                historical_config=(
                    HistoricalConfiguration.from_json(x["historical_config"]) if "historical_config" in x else None
                ),
                always_suppress_errors=(
                    _atd_read_bool(x["always_suppress_errors"]) if "always_suppress_errors" in x else False
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "ScanResponse":
        if isinstance(x, dict):
            return cls(
                ...
                config=(
                    ScanConfiguration.from_json(x["config"])
                    if "config" in x
                    else _atd_missing_json_field("ScanResponse", "config")
                ),
                engine_params=(
                    EngineConfiguration.from_json(x["engine_params"])
                    if "engine_params" in x
                    else _atd_missing_json_field("ScanResponse", "engine_params")
                ),
            )
        else:
...
class ScanMetadata:
    ...
    def from_json(cls, x: Any) -> "ScanMetadata":
        if isinstance(x, dict):
            return cls(
                cli_version=(
                    Version.from_json(x["cli_version"])
                    if "cli_version" in x
                    else _atd_missing_json_field("ScanMetadata", "cli_version")
                ),
                unique_id=(
                    Uuid.from_json(x["unique_id"])
                    if "unique_id" in x
                    else _atd_missing_json_field("ScanMetadata", "unique_id")
                ),
                requested_products=(
                    _atd_read_list(Product.from_json)(x["requested_products"])
                    if "requested_products" in x
                    else _atd_missing_json_field("ScanMetadata", "requested_products")
                ),
...
class ProjectMetadata:
    ...
    def from_json(cls, x: Any) -> "ProjectMetadata":
        if isinstance(x, dict):
            return cls(
                scan_environment=(
                    _atd_read_string(x["scan_environment"])
                    if "scan_environment" in x
                    else _atd_missing_json_field("ProjectMetadata", "scan_environment")
                ),
                repository=(
                    _atd_read_string(x["repository"])
                    if "repository" in x
                    else _atd_missing_json_field("ProjectMetadata", "repository")
                ),
                repo_url=(
                    _atd_read_nullable(Uri.from_json)(x["repo_url"])
                    if "repo_url" in x
                    else _atd_missing_json_field("ProjectMetadata", "repo_url")
                ),
                branch=(
                    _atd_read_nullable(_atd_read_string)(x["branch"])
                    if "branch" in x
                    else _atd_missing_json_field("ProjectMetadata", "branch")
                ),
                commit=(
                    _atd_read_nullable(Sha1.from_json)(x["commit"])
                    if "commit" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit")
                ),
                commit_title=(
                    _atd_read_nullable(_atd_read_string)(x["commit_title"])
                    if "commit_title" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit_title")
                ),
                commit_author_email=(
                    _atd_read_nullable(_atd_read_string)(x["commit_author_email"])
                    if "commit_author_email" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit_author_email")
                ),
                commit_author_name=(
                    _atd_read_nullable(_atd_read_string)(x["commit_author_name"])
                    if "commit_author_name" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit_author_name")
                ),
                commit_author_username=(
                    _atd_read_nullable(_atd_read_string)(x["commit_author_username"])
                    if "commit_author_username" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit_author_username")
                ),
                commit_author_image_url=(
                    _atd_read_nullable(Uri.from_json)(x["commit_author_image_url"])
                    if "commit_author_image_url" in x
                    else _atd_missing_json_field("ProjectMetadata", "commit_author_image_url")
                ),
                ci_job_url=(
                    _atd_read_nullable(Uri.from_json)(x["ci_job_url"])
                    if "ci_job_url" in x
                    else _atd_missing_json_field("ProjectMetadata", "ci_job_url")
                ),
                on=_atd_read_string(x["on"]) if "on" in x else _atd_missing_json_field("ProjectMetadata", "on"),
                pull_request_author_username=(
                    _atd_read_nullable(_atd_read_string)(x["pull_request_author_username"])
                    if "pull_request_author_username" in x
                    else _atd_missing_json_field("ProjectMetadata", "pull_request_author_username")
                ),
                pull_request_author_image_url=(
                    _atd_read_nullable(Uri.from_json)(x["pull_request_author_image_url"])
                    if "pull_request_author_image_url" in x
                    else _atd_missing_json_field("ProjectMetadata", "pull_request_author_image_url")
                ),
                pull_request_id=(
                    _atd_read_nullable(_atd_read_string)(x["pull_request_id"])
                    if "pull_request_id" in x
                    else _atd_missing_json_field("ProjectMetadata", "pull_request_id")
                ),
                pull_request_title=(
                    _atd_read_nullable(_atd_read_string)(x["pull_request_title"])
                    if "pull_request_title" in x
                    else _atd_missing_json_field("ProjectMetadata", "pull_request_title")
                ),
                is_full_scan=(
                    _atd_read_bool(x["is_full_scan"])
                    if "is_full_scan" in x
                    else _atd_missing_json_field("ProjectMetadata", "is_full_scan")
                ),
    ...
    def to_json(self) -> Any:
        ...
        res["pull_request_author_image_url"] = _atd_write_nullable((lambda x: x.to_json()))(
            self.pull_request_author_image_url
        )
...
class ScanRequest:
    ...
    def from_json(cls, x: Any) -> "ScanRequest":
        if isinstance(x, dict):
            return cls(
                project_metadata=(
                    ProjectMetadata.from_json(x["project_metadata"])
                    if "project_metadata" in x
                    else _atd_missing_json_field("ScanRequest", "project_metadata")
                ),
                scan_metadata=(
                    ScanMetadata.from_json(x["scan_metadata"])
                    if "scan_metadata" in x
                    else _atd_missing_json_field("ScanRequest", "scan_metadata")
                ),
        ...
        if isinstance(x, dict):
            return cls(
                ...
                enabled_products=(
                    _atd_read_list(Product.from_json)(x["enabled_products"])
                    if "enabled_products" in x
                    else _atd_missing_json_field("CiConfig", "enabled_products")
                ),
                ignored_files=(
                    _atd_read_list(_atd_read_string)(x["ignored_files"])
                    if "ignored_files" in x
                    else _atd_missing_json_field("CiConfig", "ignored_files")
                ),
                autofix=_atd_read_bool(x["autofix"]) if "autofix" in x else False,
                deepsemgrep=_atd_read_bool(x["deepsemgrep"]) if "deepsemgrep" in x else False,
                dependency_query=_atd_read_bool(x["dependency_query"]) if "dependency_query" in x else False,
...
class CiConfigFromCloud:
    ...
    def from_json(cls, x: Any) -> "CiConfigFromCloud":
        if isinstance(x, dict):
            return cls(
                repo_config=(
                    CiConfig.from_json(x["repo_config"])
                    if "repo_config" in x
                    else _atd_missing_json_field("CiConfigFromCloud", "repo_config")
                ),
                ...
                dirs_config=(
                    _atd_read_list(
                        (
                            lambda x: (
                                (Fpath.from_json(x[0]), CiConfig.from_json(x[1]))
                                if isinstance(x, list) and len(x) == 2
                                else _atd_bad_json("array of length 2", x)
                            )
                        )
    ...
    def to_json(self) -> Any:
        ...
        if self.dirs_config is not None:
            res["dirs_config"] = _atd_write_list(
                (
                    lambda x: (
        ...
        if isinstance(x, dict):
            return cls(
                deployment_id=(
                    _atd_read_int(x["deployment_id"])
                    if "deployment_id" in x
                    else _atd_missing_json_field("ScanConfig", "deployment_id")
                ),
                deployment_name=(
                    _atd_read_string(x["deployment_name"])
                    if "deployment_name" in x
                    else _atd_missing_json_field("ScanConfig", "deployment_name")
                ),
                policy_names=(
                    _atd_read_list(_atd_read_string)(x["policy_names"])
                    if "policy_names" in x
                    else _atd_missing_json_field("ScanConfig", "policy_names")
                ),
                rule_config=(
                    _atd_read_string(x["rule_config"])
                    if "rule_config" in x
                    else _atd_missing_json_field("ScanConfig", "rule_config")
                ),
                autofix=_atd_read_bool(x["autofix"]) if "autofix" in x else False,
                deepsemgrep=_atd_read_bool(x["deepsemgrep"]) if "deepsemgrep" in x else False,
                dependency_query=_atd_read_bool(x["dependency_query"]) if "dependency_query" in x else False,
                ...
                ignored_files=_atd_read_list(_atd_read_string)(x["ignored_files"]) if "ignored_files" in x else [],
                enabled_products=(
                    _atd_read_list(Product.from_json)(x["enabled_products"]) if "enabled_products" in x else None
                ),
                actions=_atd_read_list(Action.from_json)(x["actions"]) if "actions" in x else [],
                ci_config_from_cloud=(
                    CiConfigFromCloud.from_json(x["ci_config_from_cloud"]) if "ci_config_from_cloud" in x else None
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "SarifFormat":
        if isinstance(x, dict):
            return cls(
                ...
                is_pro=(
                    _atd_read_bool(x["is_pro"]) if "is_pro" in x else _atd_missing_json_field("SarifFormat", "is_pro")
                ),
                show_dataflow_traces=(
                    _atd_read_bool(x["show_dataflow_traces"])
                    if "show_dataflow_traces" in x
                    else _atd_missing_json_field("SarifFormat", "show_dataflow_traces")
                ),
            )
        else:
...
class RuleIdAndEngineKind:
    ...
    def from_json(cls, x: Any) -> "RuleIdAndEngineKind":
        return cls(
            (
                lambda x: (
                    (RuleId.from_json(x[0]), EngineKind.from_json(x[1]))
                    if isinstance(x, list) and len(x) == 2
                    else _atd_bad_json("array of length 2", x)
                )
            )(x)
...
class ResolutionCmdFailed:
    ...
    def from_json(cls, x: Any) -> "ResolutionCmdFailed":
        if isinstance(x, dict):
            return cls(
                command=(
                    _atd_read_string(x["command"])
                    if "command" in x
                    else _atd_missing_json_field("ResolutionCmdFailed", "command")
                ),
                message=(
                    _atd_read_string(x["message"])
                    if "message" in x
                    else _atd_missing_json_field("ResolutionCmdFailed", "message")
                ),
            )
        else:
...
class ResolutionOk:
    ...
    def to_json(self) -> Any:
        return [
            ...
            (
                lambda x: (
                    [_atd_write_list((lambda x: x.to_json()))(x[0]), _atd_write_list((lambda x: x.to_json()))(x[1])]
                    if isinstance(x, tuple) and len(x) == 2
                    else _atd_bad_python("tuple of length 2", x)
                )
            )(self.value),
                ...
                return cls(
                    ResolutionOk(
                        (
                            lambda x: (
                                (
                                    _atd_read_list(FoundDependency.from_json)(x[0]),
                                    _atd_read_list(ResolutionError.from_json)(x[1]),
                                )
                                if isinstance(x, list) and len(x) == 2
                                else _atd_bad_json("array of length 2", x)
                            )
...
class Profile:
    ...
    def from_json(cls, x: Any) -> "Profile":
        if isinstance(x, dict):
            return cls(
                rules=(
                    _atd_read_list(RuleId.from_json)(x["rules"])
                    if "rules" in x
                    else _atd_missing_json_field("Profile", "rules")
                ),
                rules_parse_time=(
                    _atd_read_float(x["rules_parse_time"])
                    if "rules_parse_time" in x
                    else _atd_missing_json_field("Profile", "rules_parse_time")
                ),
                profiling_times=(
                    _atd_read_assoc_object_into_dict(_atd_read_float)(x["profiling_times"])
                    if "profiling_times" in x
                    else _atd_missing_json_field("Profile", "profiling_times")
                ),
                targets=(
                    _atd_read_list(TargetTimes.from_json)(x["targets"])
                    if "targets" in x
                    else _atd_missing_json_field("Profile", "targets")
                ),
                total_bytes=(
                    _atd_read_int(x["total_bytes"])
                    if "total_bytes" in x
                    else _atd_missing_json_field("Profile", "total_bytes")
                ),
...
class ParsingStats:
    ...
    def from_json(cls, x: Any) -> "ParsingStats":
        if isinstance(x, dict):
            return cls(
                targets_parsed=(
                    _atd_read_int(x["targets_parsed"])
                    if "targets_parsed" in x
                    else _atd_missing_json_field("ParsingStats", "targets_parsed")
                ),
                num_targets=(
                    _atd_read_int(x["num_targets"])
                    if "num_targets" in x
                    else _atd_missing_json_field("ParsingStats", "num_targets")
                ),
                bytes_parsed=(
                    _atd_read_int(x["bytes_parsed"])
                    if "bytes_parsed" in x
                    else _atd_missing_json_field("ParsingStats", "bytes_parsed")
                ),
                num_bytes=(
                    _atd_read_int(x["num_bytes"])
                    if "num_bytes" in x
                    else _atd_missing_json_field("ParsingStats", "num_bytes")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                rule_id=(
                    RuleId.from_json(x["rule_id"])
                    if "rule_id" in x
                    else _atd_missing_json_field("IncompatibleRule", "rule_id")
                ),
                this_version=(
                    Version.from_json(x["this_version"])
                    if "this_version" in x
                    else _atd_missing_json_field("IncompatibleRule", "this_version")
                ),
...
class FindingHashes:
    ...
    def from_json(cls, x: Any) -> "FindingHashes":
        if isinstance(x, dict):
            return cls(
                start_line_hash=(
                    _atd_read_string(x["start_line_hash"])
                    if "start_line_hash" in x
                    else _atd_missing_json_field("FindingHashes", "start_line_hash")
                ),
                end_line_hash=(
                    _atd_read_string(x["end_line_hash"])
                    if "end_line_hash" in x
                    else _atd_missing_json_field("FindingHashes", "end_line_hash")
                ),
                code_hash=(
                    _atd_read_string(x["code_hash"])
                    if "code_hash" in x
                    else _atd_missing_json_field("FindingHashes", "code_hash")
                ),
                pattern_hash=(
                    _atd_read_string(x["pattern_hash"])
                    if "pattern_hash" in x
                    else _atd_missing_json_field("FindingHashes", "pattern_hash")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                check_id=(
                    RuleId.from_json(x["check_id"])
                    if "check_id" in x
                    else _atd_missing_json_field("Finding", "check_id")
                ),
                ...
                end_line=(
                    _atd_read_int(x["end_line"]) if "end_line" in x else _atd_missing_json_field("Finding", "end_line")
                ),
                end_column=(
                    _atd_read_int(x["end_column"])
                    if "end_column" in x
                    else _atd_missing_json_field("Finding", "end_column")
                ),
                message=(
                    _atd_read_string(x["message"]) if "message" in x else _atd_missing_json_field("Finding", "message")
                ),
                severity=(
                    (lambda x: x)(x["severity"]) if "severity" in x else _atd_missing_json_field("Finding", "severity")
                ),
                index=_atd_read_int(x["index"]) if "index" in x else _atd_missing_json_field("Finding", "index"),
                commit_date=(
                    _atd_read_string(x["commit_date"])
                    if "commit_date" in x
                    else _atd_missing_json_field("Finding", "commit_date")
                ),
                syntactic_id=(
                    _atd_read_string(x["syntactic_id"])
                    if "syntactic_id" in x
                    else _atd_missing_json_field("Finding", "syntactic_id")
                ),
                metadata=(
                    RawJson.from_json(x["metadata"])
                    if "metadata" in x
                    else _atd_missing_json_field("Finding", "metadata")
                ),
                is_blocking=(
                    _atd_read_bool(x["is_blocking"])
                    if "is_blocking" in x
                    else _atd_missing_json_field("Finding", "is_blocking")
                ),
    ...
    def from_json(cls, x: Any) -> "ErrorSpan":
        if isinstance(x, dict):
            return cls(
                ...
                config_path=(
                    _atd_read_nullable(_atd_read_list(_atd_read_string))(x["config_path"])
                    if "config_path" in x
                    else None
                ),
                context_start=(
                    _atd_read_nullable(Position.from_json)(x["context_start"]) if "context_start" in x else None
                ),
        ...
        if isinstance(x, dict):
            return cls(
                path=(
                    Fpath.from_json(x["path"])
                    if "path" in x
                    else _atd_missing_json_field("DependencyParserError", "path")
                ),
                parser=(
                    ScaParserName.from_json(x["parser"])
                    if "parser" in x
                    else _atd_missing_json_field("DependencyParserError", "parser")
                ),
                reason=(
                    _atd_read_string(x["reason"])
                    if "reason" in x
                    else _atd_missing_json_field("DependencyParserError", "reason")
                ),
...
class Contributor:
    ...
    def from_json(cls, x: Any) -> "Contributor":
        if isinstance(x, dict):
            return cls(
                commit_author_name=(
                    _atd_read_string(x["commit_author_name"])
                    if "commit_author_name" in x
                    else _atd_missing_json_field("Contributor", "commit_author_name")
                ),
                commit_author_email=(
                    _atd_read_string(x["commit_author_email"])
                    if "commit_author_email" in x
                    else _atd_missing_json_field("Contributor", "commit_author_email")
                ),
            )
        else:
...
class Contribution:
    ...
    def from_json(cls, x: Any) -> "Contribution":
        if isinstance(x, dict):
            return cls(
                commit_hash=(
                    _atd_read_string(x["commit_hash"])
                    if "commit_hash" in x
                    else _atd_missing_json_field("Contribution", "commit_hash")
                ),
                commit_timestamp=(
                    Datetime.from_json(x["commit_timestamp"])
                    if "commit_timestamp" in x
                    else _atd_missing_json_field("Contribution", "commit_timestamp")
                ),
                contributor=(
                    Contributor.from_json(x["contributor"])
                    if "contributor" in x
                    else _atd_missing_json_field("Contribution", "contributor")
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "CliError":
        if isinstance(x, dict):
            return cls(
                ...
                level=(
                    ErrorSeverity.from_json(x["level"])
                    if "level" in x
                    else _atd_missing_json_field("CliError", "level")
                ),
...
class CiScanResults:
    ...
    def from_json(cls, x: Any) -> "CiScanResults":
        if isinstance(x, dict):
            return cls(
                findings=(
                    _atd_read_list(Finding.from_json)(x["findings"])
                    if "findings" in x
                    else _atd_missing_json_field("CiScanResults", "findings")
                ),
                ignores=(
                    _atd_read_list(Finding.from_json)(x["ignores"])
                    if "ignores" in x
                    else _atd_missing_json_field("CiScanResults", "ignores")
                ),
                token=(
                    _atd_read_nullable(_atd_read_string)(x["token"])
                    if "token" in x
                    else _atd_missing_json_field("CiScanResults", "token")
                ),
                searched_paths=(
                    _atd_read_list(Fpath.from_json)(x["searched_paths"])
                    if "searched_paths" in x
                    else _atd_missing_json_field("CiScanResults", "searched_paths")
                ),
                renamed_paths=(
                    _atd_read_list(Fpath.from_json)(x["renamed_paths"])
                    if "renamed_paths" in x
                    else _atd_missing_json_field("CiScanResults", "renamed_paths")
                ),
                rule_ids=(
                    _atd_read_list(RuleId.from_json)(x["rule_ids"])
                    if "rule_ids" in x
                    else _atd_missing_json_field("CiScanResults", "rule_ids")
                ),
...
class CiScanFailure:
    ...
    def from_json(cls, x: Any) -> "CiScanFailure":
        if isinstance(x, dict):
            return cls(
                exit_code=(
                    _atd_read_int(x["exit_code"])
                    if "exit_code" in x
                    else _atd_missing_json_field("CiScanFailure", "exit_code")
                ),
                stderr=(
                    _atd_read_string(x["stderr"])
                    if "stderr" in x
                    else _atd_missing_json_field("CiScanFailure", "stderr")
                ),
            )
        else:
...
class CiScanCompleteStats:
    ...
    def from_json(cls, x: Any) -> "CiScanCompleteStats":
        if isinstance(x, dict):
            return cls(
                findings=(
                    _atd_read_int(x["findings"])
                    if "findings" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "findings")
                ),
                errors=(
                    _atd_read_list(CliError.from_json)(x["errors"])
                    if "errors" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "errors")
                ),
                total_time=(
                    _atd_read_float(x["total_time"])
                    if "total_time" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "total_time")
                ),
                unsupported_exts=(
                    _atd_read_assoc_object_into_dict(_atd_read_int)(x["unsupported_exts"])
                    if "unsupported_exts" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "unsupported_exts")
                ),
                lockfile_scan_info=(
                    _atd_read_assoc_object_into_dict(_atd_read_int)(x["lockfile_scan_info"])
                    if "lockfile_scan_info" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "lockfile_scan_info")
                ),
                parse_rate=(
                    _atd_read_assoc_object_into_dict(ParsingStats.from_json)(x["parse_rate"])
                    if "parse_rate" in x
                    else _atd_missing_json_field("CiScanCompleteStats", "parse_rate")
                ),
                engine_requested=_atd_read_string(x["engine_requested"]) if "engine_requested" in x else None,
                findings_by_product=(
                    _atd_read_assoc_object_into_dict(_atd_read_int)(x["findings_by_product"])
                    if "findings_by_product" in x
                    else None
                ),
                supply_chain_stats=(
                    SupplyChainStats.from_json(x["supply_chain_stats"]) if "supply_chain_stats" in x else None
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                exit_code=(
                    _atd_read_int(x["exit_code"])
                    if "exit_code" in x
                    else _atd_missing_json_field("CiScanComplete", "exit_code")
                ),
                stats=(
                    CiScanCompleteStats.from_json(x["stats"])
                    if "stats" in x
                    else _atd_missing_json_field("CiScanComplete", "stats")
                ),
                dependencies=CiScanDependencies.from_json(x["dependencies"]) if "dependencies" in x else None,
                dependency_parser_errors=(
                    _atd_read_list(DependencyParserError.from_json)(x["dependency_parser_errors"])
                    if "dependency_parser_errors" in x
                    else None
                ),
            ...
            if cons == "PartialScanOk":
                return cls(
                    PartialScanOk(
                        (
                            lambda x: (
                                (CiScanResults.from_json(x[0]), CiScanComplete.from_json(x[1]))
                                if isinstance(x, list) and len(x) == 2
                                else _atd_bad_json("array of length 2", x)
                            )
                        )(x[1])
        ...
        if isinstance(x, dict):
            return cls(
                ...
                has_triage_via_comment=(
                    _atd_read_bool(x["has_triage_via_comment"]) if "has_triage_via_comment" in x else False
                ),
                has_dependency_query=(
                    _atd_read_bool(x["has_dependency_query"]) if "has_dependency_query" in x else False
                ),
            )
        else:
            ...
            if cons == "ManifestLockfileDependencySource":
                return cls(
                    ManifestLockfileDependencySource(
                        (
                            lambda x: (
                                (Manifest.from_json(x[0]), Lockfile.from_json(x[1]))
                                if isinstance(x, list) and len(x) == 2
                                else _atd_bad_json("array of length 2", x)
                            )
                        )(x[1])
...
class ApplyFixesReturn:
    ...
    def from_json(cls, x: Any) -> "ApplyFixesReturn":
        if isinstance(x, dict):
            return cls(
                modified_file_count=(
                    _atd_read_int(x["modified_file_count"])
                    if "modified_file_count" in x
                    else _atd_missing_json_field("ApplyFixesReturn", "modified_file_count")
                ),
                fixed_lines=(
                    _atd_read_list(
                        (
                            lambda x: (
                                (_atd_read_int(x[0]), _atd_read_list(_atd_read_string)(x[1]))
                                if isinstance(x, list) and len(x) == 2
                                else _atd_bad_json("array of length 2", x)
                            )
                        )
                    ...
                    if "fixed_lines" in x
                    else _atd_missing_json_field("ApplyFixesReturn", "fixed_lines")
                ),
            )
        else:
    ...
    def to_json(self) -> Any:
        ...
        res["fixed_lines"] = _atd_write_list(
            (
                lambda x: (
                    [_atd_write_int(x[0]), _atd_write_list(_atd_write_string)(x[1])]
                    if isinstance(x, tuple) and len(x) == 2
                    else _atd_bad_python("tuple of length 2", x)
                )
            )
                ...
                return cls(
                    RetResolveDependencies(
                        _atd_read_list(
                            (
                                lambda x: (
                                    (DependencySource.from_json(x[0]), ResolutionResult.from_json(x[1]))
                                    if isinstance(x, list) and len(x) == 2
                                    else _atd_bad_json("array of length 2", x)
                                )
                            )
...
class FormatContext:
    ...
    def from_json(cls, x: Any) -> "FormatContext":
        if isinstance(x, dict):
            return cls(
                is_ci_invocation=(
                    _atd_read_bool(x["is_ci_invocation"])
                    if "is_ci_invocation" in x
                    else _atd_missing_json_field("FormatContext", "is_ci_invocation")
                ),
                is_logged_in=(
                    _atd_read_bool(x["is_logged_in"])
                    if "is_logged_in" in x
                    else _atd_missing_json_field("FormatContext", "is_logged_in")
                ),
                is_using_registry=(
                    _atd_read_bool(x["is_using_registry"])
                    if "is_using_registry" in x
                    else _atd_missing_json_field("FormatContext", "is_using_registry")
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "Edit":
        if isinstance(x, dict):
            return cls(
                ...
                start_offset=(
                    _atd_read_int(x["start_offset"])
                    if "start_offset" in x
                    else _atd_missing_json_field("Edit", "start_offset")
                ),
                end_offset=(
                    _atd_read_int(x["end_offset"])
                    if "end_offset" in x
                    else _atd_missing_json_field("Edit", "end_offset")
                ),
                replacement_text=(
                    _atd_read_string(x["replacement_text"])
                    if "replacement_text" in x
                    else _atd_missing_json_field("Edit", "replacement_text")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                rules=(
                    RawJson.from_json(x["rules"])
                    if "rules" in x
                    else _atd_missing_json_field("DumpRulePartitionsParams", "rules")
                ),
                n_partitions=(
                    _atd_read_int(x["n_partitions"])
                    if "n_partitions" in x
                    else _atd_missing_json_field("DumpRulePartitionsParams", "n_partitions")
                ),
                output_dir=(
                    Fpath.from_json(x["output_dir"])
                    if "output_dir" in x
                    else _atd_missing_json_field("DumpRulePartitionsParams", "output_dir")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                message=(
                    _atd_read_string(x["message"])
                    if "message" in x
                    else _atd_missing_json_field("CliMatchExtra", "message")
                ),
                metadata=(
                    RawJson.from_json(x["metadata"])
                    if "metadata" in x
                    else _atd_missing_json_field("CliMatchExtra", "metadata")
                ),
                severity=(
                    MatchSeverity.from_json(x["severity"])
                    if "severity" in x
                    else _atd_missing_json_field("CliMatchExtra", "severity")
                ),
                fingerprint=(
                    _atd_read_string(x["fingerprint"])
                    if "fingerprint" in x
                    else _atd_missing_json_field("CliMatchExtra", "fingerprint")
                ),
                lines=(
                    _atd_read_string(x["lines"]) if "lines" in x else _atd_missing_json_field("CliMatchExtra", "lines")
                ),
        ...
        if isinstance(x, dict):
            return cls(
                check_id=(
                    RuleId.from_json(x["check_id"])
                    if "check_id" in x
                    else _atd_missing_json_field("CliMatch", "check_id")
                ),
                ...
                extra=(
                    CliMatchExtra.from_json(x["extra"])
                    if "extra" in x
                    else _atd_missing_json_field("CliMatch", "extra")
                ),
            )
        else:
...
class CliOutput:
    ...
    def from_json(cls, x: Any) -> "CliOutput":
        if isinstance(x, dict):
            return cls(
                results=(
                    _atd_read_list(CliMatch.from_json)(x["results"])
                    if "results" in x
                    else _atd_missing_json_field("CliOutput", "results")
                ),
                errors=(
                    _atd_read_list(CliError.from_json)(x["errors"])
                    if "errors" in x
                    else _atd_missing_json_field("CliOutput", "errors")
                ),
                paths=(
                    ScannedAndSkipped.from_json(x["paths"])
                    if "paths" in x
                    else _atd_missing_json_field("CliOutput", "paths")
                ),
                version=Version.from_json(x["version"]) if "version" in x else None,
                time=Profile.from_json(x["time"]) if "time" in x else None,
                explanations=(
                    _atd_read_list(MatchingExplanation.from_json)(x["explanations"]) if "explanations" in x else None
                ),
                rules_by_engine=(
                    _atd_read_list(RuleIdAndEngineKind.from_json)(x["rules_by_engine"])
                    if "rules_by_engine" in x
                    else None
                ),
                engine_requested=EngineKind.from_json(x["engine_requested"]) if "engine_requested" in x else None,
                interfile_languages_used=(
                    _atd_read_list(_atd_read_string)(x["interfile_languages_used"])
                    if "interfile_languages_used" in x
                    else None
                ),
...
class ApplyFixesParams:
    ...
    def from_json(cls, x: Any) -> "ApplyFixesParams":
        if isinstance(x, dict):
            return cls(
                dryrun=(
                    _atd_read_bool(x["dryrun"])
                    if "dryrun" in x
                    else _atd_missing_json_field("ApplyFixesParams", "dryrun")
                ),
                edits=(
                    _atd_read_list(Edit.from_json)(x["edits"])
                    if "edits" in x
                    else _atd_missing_json_field("ApplyFixesParams", "edits")
                ),
            )
        else:
...
class CallFormatter:
    ...
    def to_json(self) -> Any:
        return [
            ...
            (
                lambda x: (
                    [(lambda x: x.to_json())(x[0]), (lambda x: x.to_json())(x[1]), (lambda x: x.to_json())(x[2])]
                    if isinstance(x, tuple) and len(x) == 3
                    else _atd_bad_python("tuple of length 3", x)
                )
            )(self.value),
            ...
            if cons == "CallFormatter":
                return cls(
                    CallFormatter(
                        (
                            lambda x: (
                                (OutputFormat.from_json(x[0]), FormatContext.from_json(x[1]), CliOutput.from_json(x[2]))
                                if isinstance(x, list) and len(x) == 3
                                else _atd_bad_json("array of length 3", x)
                            )
                        )(x[1])
            ...
            if cons == "CallSarifFormat":
                return cls(
                    CallSarifFormat(
                        (
                            lambda x: (
                                (SarifFormat.from_json(x[0]), FormatContext.from_json(x[1]), CliOutput.from_json(x[2]))
                                if isinstance(x, list) and len(x) == 3
                                else _atd_bad_json("array of length 3", x)
                            )
                        )(x[1])
...
class DiffFile:
    ...
    def from_json(cls, x: Any) -> "DiffFile":
        if isinstance(x, dict):
            return cls(
                filename=(
                    Fpath.from_json(x["filename"])
                    if "filename" in x
                    else _atd_missing_json_field("DiffFile", "filename")
                ),
                diffs=(
                    _atd_read_list(_atd_read_string)(x["diffs"])
                    if "diffs" in x
                    else _atd_missing_json_field("DiffFile", "diffs")
                ),
...
class DiffFiles:
    ...
    def from_json(cls, x: Any) -> "DiffFiles":
        if isinstance(x, dict):
            return cls(
                cve_diffs=(
                    _atd_read_list(DiffFile.from_json)(x["cve_diffs"])
                    if "cve_diffs" in x
                    else _atd_missing_json_field("DiffFiles", "cve_diffs")
                ),
            )
        else:
    ...
    def from_json(cls, x: Any) -> "DeploymentConfig":
        if isinstance(x, dict):
            return cls(
                ...
                name=(
                    _atd_read_string(x["name"]) if "name" in x else _atd_missing_json_field("DeploymentConfig", "name")
                ),
...
class DeploymentResponse:
    ...
    def from_json(cls, x: Any) -> "DeploymentResponse":
        if isinstance(x, dict):
            return cls(
                deployment=(
                    DeploymentConfig.from_json(x["deployment"])
                    if "deployment" in x
                    else _atd_missing_json_field("DeploymentResponse", "deployment")
                ),
            )
        else:
...
class CoreError:
    ...
    def from_json(cls, x: Any) -> "CoreError":
        if isinstance(x, dict):
            return cls(
                error_type=(
                    ErrorType.from_json(x["error_type"])
                    if "error_type" in x
                    else _atd_missing_json_field("CoreError", "error_type")
                ),
                severity=(
                    ErrorSeverity.from_json(x["severity"])
                    if "severity" in x
                    else _atd_missing_json_field("CoreError", "severity")
                ),
                message=(
                    _atd_read_string(x["message"])
                    if "message" in x
                    else _atd_missing_json_field("CoreError", "message")
                ),
                details=_atd_read_string(x["details"]) if "details" in x else None,
...
class CoreOutput:
    ...
    def from_json(cls, x: Any) -> "CoreOutput":
        if isinstance(x, dict):
            return cls(
                version=(
                    Version.from_json(x["version"])
                    if "version" in x
                    else _atd_missing_json_field("CoreOutput", "version")
                ),
                results=(
                    _atd_read_list(CoreMatch.from_json)(x["results"])
                    if "results" in x
                    else _atd_missing_json_field("CoreOutput", "results")
                ),
                errors=(
                    _atd_read_list(CoreError.from_json)(x["errors"])
                    if "errors" in x
                    else _atd_missing_json_field("CoreOutput", "errors")
                ),
                paths=(
                    ScannedAndSkipped.from_json(x["paths"])
                    if "paths" in x
                    else _atd_missing_json_field("CoreOutput", "paths")
                ),
                time=Profile.from_json(x["time"]) if "time" in x else None,
                explanations=(
                    _atd_read_list(MatchingExplanation.from_json)(x["explanations"]) if "explanations" in x else None
        ...
        if isinstance(x, dict):
            return cls(
                paths=(
                    ScannedAndSkipped.from_json(x["paths"])
                    if "paths" in x
                    else _atd_missing_json_field("CliOutputExtra", "paths")
                ),
                time=Profile.from_json(x["time"]) if "time" in x else None,
                explanations=(
                    _atd_read_list(MatchingExplanation.from_json)(x["explanations"]) if "explanations" in x else None
        ...
        if isinstance(x, dict):
            return cls(
                message=(
                    _atd_read_string(x["message"])
                    if "message" in x
                    else _atd_missing_json_field("CiScanResultsResponseError", "message")
                ),
            )
        else:
...
class CiScanResultsResponse:
    ...
    def from_json(cls, x: Any) -> "CiScanResultsResponse":
        if isinstance(x, dict):
            return cls(
                errors=(
                    _atd_read_list(CiScanResultsResponseError.from_json)(x["errors"])
                    if "errors" in x
                    else _atd_missing_json_field("CiScanResultsResponse", "errors")
                ),
                task_id=_atd_read_string(x["task_id"]) if "task_id" in x else None,
            )
        else:
...
class CiScanCompleteResponse:
    ...
    def from_json(cls, x: Any) -> "CiScanCompleteResponse":
        if isinstance(x, dict):
            return cls(
                success=(
                    _atd_read_bool(x["success"])
                    if "success" in x
                    else _atd_missing_json_field("CiScanCompleteResponse", "success")
                ),
                ...
                app_blocking_match_based_ids=(
                    _atd_read_list(MatchBasedId.from_json)(x["app_blocking_match_based_ids"])
                    if "app_blocking_match_based_ids" in x
                    else []
                ),
            )
        else:
    ...
    def to_json(self) -> Any:
        ...
        res["app_blocking_match_based_ids"] = _atd_write_list((lambda x: x.to_json()))(
            self.app_blocking_match_based_ids
        )
        return res
