...
class SecretsConfig:
    ...
    def from_json(cls, x: Any) -> "SecretsConfig":
        if isinstance(x, dict):
            return cls(
                permitted_origins=(
                    SecretsOrigin.from_json(x["permitted_origins"])
                    if "permitted_origins" in x
                    else _atd_missing_json_field("SecretsConfig", "permitted_origins")
                ),
            )
        else:
...
class ProFeatures:
        ...
        if isinstance(x, dict):
            return cls(
                ...
                numInterfileDiffScanned=(
                    _atd_read_assoc_object_into_list(_atd_read_int)(x["numInterfileDiffScanned"])
                    if "numInterfileDiffScanned" in x
                    else None
                ),
            )
        else:
    ...
    def to_json(self) -> Any:
        ...
        if self.numInterfileDiffScanned is not None:
            res["numInterfileDiffScanned"] = _atd_write_assoc_list_to_object(_atd_write_int)(
                self.numInterfileDiffScanned
            )
        return res
...
class EngineConfig:
    ...
    def from_json(cls, x: Any) -> "EngineConfig":
        if isinstance(x, dict):
            return cls(
                analysis_type=(
                    AnalysisType.from_json(x["analysis_type"])
                    if "analysis_type" in x
                    else _atd_missing_json_field("EngineConfig", "analysis_type")
                ),
                pro_langs=(
                    _atd_read_bool(x["pro_langs"])
                    if "pro_langs" in x
                    else _atd_missing_json_field("EngineConfig", "pro_langs")
                ),
                ...
                supply_chain_config=(
                    SupplyChainConfig.from_json(x["supply_chain_config"]) if "supply_chain_config" in x else None
                ),
            )
        else:
...
class Value:
    ...
    def from_json(cls, x: Any) -> "Value":
        if isinstance(x, dict):
            return cls(
                features=(
                    _atd_read_list(_atd_read_string)(x["features"])
                    if "features" in x
                    else _atd_missing_json_field("Value", "features")
                ),
                ...
                numFindingsByProduct=(
                    _atd_read_assoc_object_into_list(_atd_read_int)(x["numFindingsByProduct"])
                    if "numFindingsByProduct" in x
                    else None
                ),
                numIgnored=_atd_read_int(x["numIgnored"]) if "numIgnored" in x else None,
                ruleHashesWithFindings=(
                    _atd_read_assoc_object_into_list(_atd_read_int)(x["ruleHashesWithFindings"])
                    if "ruleHashesWithFindings" in x
                    else None
                ),
                ...
                interfileLanguagesUsed=(
                    _atd_read_list(_atd_read_string)(x["interfileLanguagesUsed"])
                    if "interfileLanguagesUsed" in x
                    else None
                ),
            )
        else:
...
class RuleStats:
    ...
    def from_json(cls, x: Any) -> "RuleStats":
        if isinstance(x, dict):
            return cls(
                ruleHash=(
                    _atd_read_string(x["ruleHash"])
                    if "ruleHash" in x
                    else _atd_missing_json_field("RuleStats", "ruleHash")
                ),
                bytesScanned=(
                    _atd_read_int(x["bytesScanned"])
                    if "bytesScanned" in x
                    else _atd_missing_json_field("RuleStats", "bytesScanned")
                ),
    ...
    def from_json(cls, x: Any) -> "FileStats":
        if isinstance(x, dict):
            return cls(
                ...
                numTimesScanned=(
                    _atd_read_int(x["numTimesScanned"])
                    if "numTimesScanned" in x
                    else _atd_missing_json_field("FileStats", "numTimesScanned")
                ),
    ...
    def from_json(cls, x: Any) -> "Performance":
        if isinstance(x, dict):
            return cls(
                ...
                profilingTimes=(
                    _atd_read_assoc_object_into_list(_atd_read_float)(x["profilingTimes"])
                    if "profilingTimes" in x
                    else None
                ),
        ...
        if isinstance(x, dict):
            return cls(
                targets_parsed=(
                    ...
                    else _atd_missing_json_field("ParseStat", "targets_parsed")
                ),
                num_targets=(
                    _atd_read_int(x["num_targets"])
                    if "num_targets" in x
                    else _atd_missing_json_field("ParseStat", "num_targets")
                ),
                bytes_parsed=(
                    _atd_read_int(x["bytes_parsed"])
                    if "bytes_parsed" in x
                    else _atd_missing_json_field("ParseStat", "bytes_parsed")
                ),
                num_bytes=(
                    _atd_read_int(x["num_bytes"])
                    if "num_bytes" in x
                    else _atd_missing_json_field("ParseStat", "num_bytes")
                ),
            )
        else:
        ...
        if isinstance(x, dict):
            return cls(
                version=(
                    ...
                    if "version" in x
                    else _atd_missing_json_field("Environment", "version")
                ),
                os=_atd_read_string(x["os"]) if "os" in x else _atd_missing_json_field("Environment", "os"),
                isTranspiledJS=(
                    _atd_read_bool(x["isTranspiledJS"])
                    if "isTranspiledJS" in x
                    else _atd_missing_json_field("Environment", "isTranspiledJS")
                ),
                projectHash=(
                    _atd_read_nullable(Sha256.from_json)(x["projectHash"])
                    if "projectHash" in x
                    else _atd_missing_json_field("Environment", "projectHash")
                ),
                configNamesHash=(
                    Sha256.from_json(x["configNamesHash"])
                    if "configNamesHash" in x
                    else _atd_missing_json_field("Environment", "configNamesHash")
                ),
                ci=(
                    _atd_read_nullable(_atd_read_string)(x["ci"])
                    if "ci" in x
                    else _atd_missing_json_field("Environment", "ci")
                ),
...
class Payload:
    ...
    def from_json(cls, x: Any) -> "Payload":
        if isinstance(x, dict):
            return cls(
                event_id=(
                    Uuid.from_json(x["event_id"]) if "event_id" in x else _atd_missing_json_field("Payload", "event_id")
                ),
                started_at=(
                    Datetime.from_json(x["started_at"])
                    if "started_at" in x
                    else _atd_missing_json_field("Payload", "started_at")
                ),
                sent_at=(
                    Datetime.from_json(x["sent_at"])
                    if "sent_at" in x
                    else _atd_missing_json_field("Payload", "sent_at")
                ),
                anonymous_user_id=(
                    _atd_read_string(x["anonymous_user_id"])
                    if "anonymous_user_id" in x
                    else _atd_missing_json_field("Payload", "anonymous_user_id")
                ),
                environment=(
                    Environment.from_json(x["environment"])
                    if "environment" in x
                    else _atd_missing_json_field("Payload", "environment")
                ),
                performance=(
                    Performance.from_json(x["performance"])
                    if "performance" in x
                    else _atd_missing_json_field("Payload", "performance")
                ),
                ...
                extension=(
                    Extension.from_json(x["extension"])
                    if "extension" in x
                    else _atd_missing_json_field("Payload", "extension")
                ),
                parse_rate=(
                    _atd_read_assoc_object_into_list(ParseStat.from_json)(x["parse_rate"]) if "parse_rate" in x else []
                ),
            )
        else:
