...
from semgrep.util import unit_str, with_color, with_feature_status
...
def _print_scan_plan_header(
    ...
    if target_mode_config.is_pro_diff_scan:
        ...
        summary_line = f"Inter-file Differential Scanning {unit_str(diff_file_count, 'file')}"
    else:
...
def _print_degenerate_table(plan: Plan, *, rule_count: int) -> None:
    ...
    if not rule_count or not plan.target_mappings:
        ...
        # e.g. 1 rule, 4 files
        file_count = len([task for task in plan.target_mappings if plan.product in task.products])
    ...
    *,
    ...
    sast_plan = CoreRunner.plan_core_run(
        [
            ...
            for rule in rules
            if (
                (isinstance(rule.product.value, out.SAST) or isinstance(rule.product.value, out.Secrets))
        ...
        (
            ...
            else evolve(target_manager, target_strings=target_mode_config.get_diff_targets())
        ),
        product=out.Product(out.SAST()),  # code-smell since secrets and sast are within the same plan
    ...
    sca_plan = CoreRunner.plan_core_run(
        [
            ...
            and any(len(sca_subprojects.get(ecosystem, [])) > 0 for ecosystem in rule.ecosystems)
        ],
        target_manager,
