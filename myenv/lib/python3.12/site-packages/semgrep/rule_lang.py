...
from ruamel.yaml import YAML, MappingNode, Node, RoundTripConstructor
from semgrep import __VERSION__, tracing
from semgrep.constants import PLEASE_FILE_ISSUE_TEXT
from semgrep.error import OK_EXIT_CODE, SemgrepCoreError, SemgrepError
from semgrep.error_location import SourceTracker, Span
...
class YamlTree(Generic[T]):
    ...
    def wrap(cls, value: YamlValue, span: Span) -> "YamlTree":
        ...
        if isinstance(value, list):
            ...
            return YamlTree(
                value=YamlMap({YamlTree.wrap(k, span): YamlTree.wrap(v, span) for k, v in value.items()}),
                span=span,
            )
        ...
        def construct_object(self, node: Node, deep: bool = False) -> YamlTree:
            ...
            if isinstance(node, MappingNode):
                ...
                if len(uniq_key_names) < len(kv_pairs):
                    raise InvalidRuleSchemaError(
                        ...
                        spans=[
                            Span.from_node(node, source_hash=source_hash, filename=filename).with_context(
                                before=1, after=1
                            )
                        ],
                    )
            ...
            if r is None and not allow_null:
                ...
                raise InvalidRuleSchemaError(
                    ...
                    spans=[
                        Span.from_node(node, source_hash=source_hash, filename=filename).with_context(before=1, after=1)
                    ],
                )
...
def validate_yaml(
    ...
    errors = remove_incompatible_rules_based_on_version(data, filename, no_rewrite_rule_ids=no_rewrite_rule_ids)
    ...
    try:
        if skip_rpc_validation:
            logger.debug("Skipping semgrep-core validation to proceed directly to jsonschema validation")
        else:
            ...
            try:
                if not rules_tmp_path or not Path.exists(Path(rules_tmp_path)):
                    raise NotImplementedError("Cannot execute RPC validation without a rules_tmp_path")
