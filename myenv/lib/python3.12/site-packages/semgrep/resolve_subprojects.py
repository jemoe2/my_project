...
def find_subprojects(dependency_source_files: FrozenSet[Path], matchers: List[SubprojectMatcher]) -> List[Subproject]:
    """
    ...
    for matcher in matchers:
        ...
        new_subprojects, new_used_files = matcher.make_subprojects(dependency_source_files - used_files)
...
def filter_changed_subprojects(
    ...
    all_dependency_source_targets = target_manager.get_all_dependency_source_files(ignore_baseline_handler=False)
    ...
    for language, ecosystems in ecosystems_by_language.items():
        for code_file in target_manager.get_files_for_language(language, out.Product).kept:
            ...
            for ecosystem in ecosystems:
                ...
                closest_subproject = find_closest_subproject(code_file, ecosystem, subprojects)
    ...
    unresolved_subprojects = [
        UnresolvedSubproject.from_subproject(s, UnresolvedReason.SKIPPED, []) for s in ordered_irrelevant
    ]
...
def resolve_subprojects(
...
) -> Tuple[List[UnresolvedSubproject], Dict[Ecosystem, List[ResolvedSubproject]], List[Path]]:
    """
    ...
    dependency_source_files = target_manager.get_all_dependency_source_files(ignore_baseline_handler=True)
    ...
    for to_resolve in relevant_subprojects:
        if to_resolve.ecosystem is None:
            ...
            unresolved.append(UnresolvedSubproject.from_subproject(to_resolve, UnresolvedReason.UNSUPPORTED, []))
            continue
        ...
        if resolved_info is not None:
            ...
            unresolved.append(UnresolvedSubproject.from_subproject(to_resolve, UnresolvedReason.FAILED, errors))
