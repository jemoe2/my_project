...
from semgrep.constants import CLI_RULE_ID, Colors
from semgrep.error import SemgrepCoreError, SemgrepError
...
AUTOFIX_TEXT_WIDTH = BASE_WIDTH - (BASE_INDENT + 4 + 13)  # 4 for line number, 13 for autofix tag
...
def format_lines(
    ...
    indent_len = (
        len(lines[0].rstrip()) - len(dedented_lines[0].rstrip()) if len(dedented_lines) > 0 and len(lines) > 0 else 0
    )
    ...
    for i, line in enumerate(dedented_lines):
        ...
        if i == 0 and is_different_file:
            yield Text.from_ansi(f" " * (BASE_INDENT + 1) + f"{with_color(Colors.cyan, f'{path}', bold=False)}")
    ...
    if trimmed > 0:
        ...
        longest_line_len = min(per_line_max_chars_limit, max(len(line) for line in lines))
        ...
        yield Text.assemble(f" " * (FINDINGS_INDENT_DEPTH - 4) + f"⋮┆" + f"-" * seperator_fill_count)
...
def call_trace_to_lines(
    ...
    if isinstance(trace, out.CliLoc):
        ...
        if intermediate_vars and len(intermediate_vars) > 0:
            ...
            yield Text.assemble(BASE_INDENT * " " + "Taint flows through these intermediate variables:")
            prev_path = ref_path
            for var in intermediate_vars:
                loc = var.location
                path = Path(loc.path.value)
                try:
                    lines = get_lines_from_file(Path(loc.path.value), loc.start.line, loc.end.line)
                except FileNotFoundError:
                    lines = [var.content]
...
def dataflow_trace_to_lines(
    ...
    if dataflow_trace:
        ...
        if sink:
            yield Text.assemble("")
            yield Text.assemble(FINDINGS_INDENT_DEPTH * " " + "This is how taint reaches the sink:")
            yield from call_trace_to_lines(
                rule_match_path,
                sink,
                color_output,
...
def print_time_summary(time_data: out.Profile, error_output: Sequence[SemgrepError]) -> None:
    items_to_show = 5
    col_lim = 50
    ...
    file_parsing_time = sum(sum(t for t in target.parse_times if t >= 0) for target in targets)
    file_timings = {
        target.path.value: (
    ...
    semgrep_core_errors = [err for err in error_output if isinstance(err, SemgrepCoreError)]
    errors = {
        (
    ...
    lang_bytes: Mapping[str, int] = {lang: sum(info[1][0] for info in lang_info[lang]) for lang in langs}
    lang_times: Mapping[str, float] = {lang: sum(info[1][1] for info in lang_info[lang]) for lang in langs}
    ...
    console.print(f"Total time: {total_time:.4f}s Config time: {config_time:.4f}s Core time: {core_time:.4f}s")
    ...
    slowest_file_times = sorted(file_timings.items(), key=lambda x: x[1], reverse=True)[:items_to_show]
    ...
    slowest_rule_times = sorted(rule_match_timings.items(), reverse=True)[:items_to_show]
    for rule_id, match_time in slowest_rule_times:
        rule_id = truncate(rule_id, col_lim) + ":"
        console.print(f"{with_color(Colors.yellow, f'{rule_id:<59}')} {match_time:.3f}s")
...
def print_text_output(
    ...
    for rule_index, rule_match in enumerate(sorted_rule_matches):
        current_file = f"{rule_match.path}@{rule_match.git_commit.value}" if rule_match.git_commit else rule_match.path
        ...
        if last_file is None or last_file != current_file:
            ...
            console.print(
                ...
                + (f" with lockfile {with_color(Colors.cyan, f'{lockfile}')}" if lockfile else "")
            )
            last_rule_id = None
            last_message = None
        ...
        if (
            ...
            rule_title = rule_match.title  # Title of the rule that we need to bold later
            wrapped_text = textwrap.fill(
                rule_title,
            ...
            if last_file == current_file and last_rule_id != rule_match.rule_id:
                console.print(" ")  # add a blank line between different rules in the same file
            ...
            severity = (
                ...
                if "sca_info" in rule_match.extra and "sca-severity" in rule_match.metadata
                else ""
            )
        ...
        if fix is not None:
            ...
            wrapped_fix = (
                textwrap.fill(
                    textwrap.dedent(" ".join(l.strip() for l in fix.splitlines(keepends=True))),
            ...
            dep_name = rule_match.extra["sca_info"].dependency_match.found_dependency.package
            fixed_versions = sorted(
                {version for fix_obj in fixes for name, version in fix_obj.items() if name == dep_name}
            )
        ...
        next_rule_match = sorted_rule_matches[rule_index + 1] if rule_index != len(sorted_rule_matches) - 1 else None
        is_same_file = next_rule_match.path == rule_match.path if next_rule_match else False
        is_same_rule = next_rule_match.rule_id == rule_match.rule_id if next_rule_match else False
        show_separator = is_same_file and is_same_rule and not (dataflow_traces and rule_match.dataflow_trace)
...
class TextFormatter(base.BaseFormatter):
    def format(
        ...
        with force_quiet_off(console), console.capture() as captured_output:
            ...
            code_blocking_rules = {
                match.match.check_id.value for match in grouped_matches[out.Product(out.SAST()), "blocking"]
            }
            ...
            rules_by_engine = cli_output_extra.rules_by_engine if cli_output_extra.rules_by_engine else []
