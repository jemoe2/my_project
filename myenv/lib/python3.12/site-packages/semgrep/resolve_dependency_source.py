...
from semdep.parsers.util import SemgrepParser, to_parser
...
PARSERS_BY_LOCKFILE_KIND: Dict[out.LockfileKind, Union[SemgrepParser, None]] = {
    ...
    out.LockfileKind(out.CargoLock()): to_parser(parse_cargo, ScaParserName(CargoParser())),
...
def _resolve_dependencies_rpc(
    dependency_source: Union[ManifestOnlyDependencySource, ManifestLockfileDependencySource],
) -> Tuple[
    ...
    if response is None:
        ...
        return None, [], []
    if len(response) > 1:
        logger.warning(f"Too many responses from dependency resolution RPC. Expected 1, got {len(response)}")
    result = response[0][1]
...
def _handle_multi_lockfile_source(
    ...
    for lockfile_source in dep_source.sources:
        new_resolved_info, new_errors, new_targets = resolve_dependency_source(lockfile_source)
...
def _handle_lockfile_source(
    ...
    if ptt_enabled and isinstance(dep_source, ManifestLockfileDependencySource):
        ...
        use_dynamic_resolution = (
            ...
            and (dep_source.manifest.kind, dep_source.lockfile.kind) in PTT_DYNAMIC_RESOLUTION_SUBPROJECT_KINDS
        )
        if use_nondynamic_ocaml_parsing or use_dynamic_resolution:
            ...
            if new_deps is not None:
                ...
                return (
                    (
                        ResolutionMethod.LOCKFILE_PARSING if use_nondynamic_ocaml_parsing else ResolutionMethod.DYNAMIC,
                        new_deps,
                    ),
                    new_errors,
                    new_targets,
    ...
    manifest_path = (
        Path(dep_source.manifest.path.value) if isinstance(dep_source, ManifestLockfileDependencySource) else None
    )
...
def resolve_dependency_source(
    ...
    """
    if isinstance(dep_source, LockfileOnlyDependencySource) or isinstance(dep_source, ManifestLockfileDependencySource):
