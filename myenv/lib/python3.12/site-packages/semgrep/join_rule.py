...
from semgrep.config_resolver import Config, resolve_config
from semgrep.error import ERROR_MAP, FATAL_EXIT_CODE, SemgrepError
...
def model_factory(model_name: str, columns: List[str]) -> Type[BaseModel]:
    ...
    return type(
        model_name,
        (BaseModel,),
        dict([("raw", pw.BlobField())] + [(column, pw.TextField(null=True)) for column in columns]),
    )
...
def match_on_conditions(  # type: ignore
    ...
    query = (
        ...
        .distinct()
        .where(*list(map(lambda terms: evaluate_condition(*terms), condition_terms)))  # type: ignore
    )
    return query
        ...
        for finding in findings:
            model_map[name].create(
                ...
                **{
                    ...
                    for metavar, content in finding.get("extra", {}).get("metavars", {}).items()
                },
            )
...
def json_to_rule_match(join_rule: Dict[str, Any], match: Dict[str, Any]) -> RuleMatch:
    ...
    extra = out.CoreMatchExtra(
        ...
        engine_kind=cli_match_extra.engine_kind if cli_match_extra.engine_kind else out.EngineOfFinding(out.OSS()),
        is_ignored=False,
    )
    return RuleMatch(
        message=join_rule.get("message", match.get("extra", {}).get("message", "[empty]")),
        metadata=join_rule.get("metadata", match.get("extra", {}).get("metadata", {})),
        severity=out.MatchSeverity.from_json(join_rule.get("severity", match.get("severity", "INFO"))),
        match=out.CoreMatch(
...
def run_join_rule(
    ...
    join_rule_refs: List[Ref] = [
        Ref(
            ...
            renames={rename.get("from"): rename.get("to") for rename in ref.get("renames", [])},
            alias=ref.get("as"),
        )
    ...
    collection_set_unaliased = {
        alias_lookup[collection] for collection in create_collection_set_from_conditions(conditions)
    }
    ...
    matched_on_conditions = match_on_conditions(
        model_map,
        alias_lookup,
        [Condition.parse(condition_string) for condition_string in join_contents.get("on", [])],
    )
