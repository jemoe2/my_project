...
from typing import Any, Callable, FrozenSet, List, Optional, Sequence, TypeVar, Union
from urllib.parse import urlparse

import click
from semgrep.constants import FIXTEST_SUFFIX, YML_SUFFIXES, YML_TEST_SUFFIXES, Colors
...
def path_has_permissions(path: Path, permissions: int, follow_symlinks: bool = True) -> bool:
    return (
        # path.stat(follow_symlinks=follow_symlinks): requires python >= 3.10
        path_exists(path, follow_symlinks=follow_symlinks)
        and (path.stat() if follow_symlinks else path.lstat()).st_mode & permissions == permissions
    )
...
def has_color() -> bool:
    ...
    no_color = is_truthy(os.environ.get("NO_COLOR")) or is_truthy(os.environ.get("SEMGREP_FORCE_NO_COLOR"))
...
def final_suffix_matches(path: Path, path2: Path) -> bool:
    return (is_config_test_suffix(path) and is_config_test_suffix(path2)) or (path.suffixes[-1] == path2.suffixes[-1])
