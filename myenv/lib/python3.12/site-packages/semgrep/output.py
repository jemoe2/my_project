...
from semgrep.constants import Colors, OutputFormat
from semgrep.engine import EngineType
from semgrep.error import FINDINGS_EXIT_CODE, SemgrepCoreError, SemgrepError
...
from semgrep.state import DesignTreatment, get_state
from semgrep.target_manager import FileTargetingLog, TargetManager
from semgrep.util import is_url, terminal_wrap, unit_str, with_color
    ...
    def normalize(self) -> NormalizedOutputSettings:
        ...
        if self.output_format is None:
            ...
            if self.output_destination in normalized_outputs:
                raise RuntimeError("Invalid output configuration: same output destination with multiple formats.")
...
class OutputHandler:
    ...
    def handle_semgrep_errors(self, errors: Sequence[SemgrepError]) -> None:
        ...
        for err in errors:
            if isinstance(err, SemgrepCoreError) and err.is_timeout() and err not in self.error_set:
                assert err.core.location
                self.semgrep_structured_errors.append(err)
                self.error_set.add(err)

                if not err.core.rule_id:
                    timeout_errors[Path(err.core.location.path.value)].append("<unknown rule_id>")
                else:
                    timeout_errors[Path(err.core.location.path.value)].append(err.core.rule_id.value)
            elif isinstance(err, SemgrepCoreError) and err.is_missing_plugin() and err not in self.error_set:
    ...
    def _handle_semgrep_timeout_errors(self, errors: Dict[Path, List[str]]) -> None:
        ...
        for path in errors.keys():
            ...
            print_threshold_hint = print_threshold_hint or (num_errs > 5 and not self.settings.timeout_threshold)
    ...
    def _handle_semgrep_error(self, error: SemgrepError) -> None:
        ...
        if error not in self.error_set:
            ...
            if self.settings.has_text_output() and (
                not (isinstance(error.level.value, out.Warning_)) or self.settings.verbose_errors
            ):
    ...
    def _make_failed_to_analyze(
        ...
        def update_failed_to_analyze(
            ...
            if err.spans is None or so_far[0] is None:
                ...
                num_lines = so_far[0] + sum(s.end.line - s.start.line + 1 for s in err.spans)
            rule_ids = so_far[1]
    ...
    def keep_ignores(self) -> bool:
        return bool(sum(1 for dest in self.settings.outputs if self._formatters[dest].keep_ignores()))
    ...
    def output(
        ...
        *,
        ...
        self.rule_matches = [
            match for matches_of_one_rule in rule_matches_by_rule.values() for match in matches_of_one_rule
        ]
        ...
        if self.final_error:
            ...
            semgrep_core_errors = [err for err in self.semgrep_structured_errors if isinstance(err, SemgrepCoreError)]

            failed_to_analyze_lines_by_path = self._make_failed_to_analyze(semgrep_core_errors)
                ...
                if output_destination:
                    ...
                    if output:
                        try:
                            ...
                            raise Exception("Received output encoding error, please set PYTHONIOENCODING=utf-8") from ex
        ...
        if self.filtered_rules:
            ...
            if print_summary:
                ...
                if missed_rule_count and state.get_cli_ux_flavor() != DesignTreatment.LEGACY:
                    missed_count_line = (
                        f"💎 Missed out on {unit_str(missed_rule_count, 'pro rule')} since you aren't logged in!"
                    )
                    learn_more_url = with_color(Colors.cyan, "https://sg.run/rules", underline=True)
    ...
    def _post_output(self, output_url: str, output: str) -> None:
        ...
        try:
            ...
            logger.verbose(f"posted to {output_url} and got status_code:{r.status_code}")
    ...
    def _build_output(
        ...
        if self.settings.verbose_errors:
            ...
            skipped = sorted(self.ignore_log.yield_json_objects(), key=lambda x: Path(x["path"]))
        ...
        if output_format == OutputFormat.TEXT:
            extra["color_output"] = (
                (output_destination is None and sys.stdout.isatty()) or os.environ.get("SEMGREP_FORCE_COLOR")
            ) and not os.environ.get("NO_COLOR")
            extra["per_finding_max_lines_limit"] = self.settings.output_per_finding_max_lines_limit
            extra["per_line_max_chars_limit"] = self.settings.output_per_line_max_chars_limit
        ...
        cli_output_extra = out.CliOutputExtra(
            ...
            interfile_languages_used=self.extra.core.interfile_languages_used if self.extra else None,
        ...
        output = formatter.output(  # the rules are used only by the SARIF formatter
            ...
            out.FormatContext(
                ...
                is_using_registry=state.metrics.is_using_registry or state.env.mock_using_registry,
            ),
        )
