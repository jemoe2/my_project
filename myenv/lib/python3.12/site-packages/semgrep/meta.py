...
class GitMeta:
    ...
    def commit_timestamp(self) -> out.Datetime:
        ...
        """
        return out.Datetime(datetime.fromtimestamp(int(self.commit_datetime)).isoformat())
...
class GithubMeta(GitMeta):
    ...
    def head_branch_hash(self) -> str:
        ...
        logger.debug(f"head branch ({head_branch_name}) has latest commit {commit}, fetching that commit now.")
        git_check_output(
            [
                "git",
                "fetch",
    ...
    def _find_branchoff_point(self, attempt_count: int = 0) -> str:
        ...
        logger.debug(f"Attempting to find merge base, attempt_count={attempt_count}, fetch_depth={fetch_depth}")

        if attempt_count:
            logger.debug(f"Trying to fetch branch {self._base_branch_ref} from origin as base branch")
            git_check_output(
                [
                    "git",
                    "fetch",
        ...
        # repositories.
        if attempt_count == 0 and self.gh_token is not None and self.api_url is not None and self.repo_name is not None:
            ...
            try:
                ...
                if req.status_code == 200:
                    ...
                    base = glom(compare_json, T["merge_base_commit"]["sha"], default=None)
                    if type(base) == str:
                ...
                logger.debug(f"Encountered error while getting merge base using GitHub API: {repr(e)}")
        ...
        try:  # check if both branches connect to the yet-unknown branch-off point now
            ...
            logger.info(f"Using {merge_base} as the merge-base of {self.base_branch_hash} and {self.head_branch_hash}")
            return merge_base
    ...
    def to_project_metadata(self) -> out.ProjectMetadata:
        ...
        res.commit_author_image_url = uri_opt(self.glom_event(T["sender"]["avatar_url"]))
        res.pull_request_author_username = self.glom_event(T["pull_request"]["user"]["login"])
        res.pull_request_author_image_url = uri_opt(self.glom_event(T["pull_request"]["user"]["avatar_url"]))
...
class GitlabMeta(GitMeta):
    ...
    def _fetch_branch_get_merge_base(branch_name: str, head_sha: str) -> str:
        ...
        parts = parts._replace(netloc=f"gitlab-ci-token:{os.environ['CI_JOB_TOKEN']}@{parts.netloc}")
        url = urllib.parse.urlunsplit(parts)
        git_check_output(["git", "fetch", url, branch_name])

        base_sha = git_check_output(["git", "merge-base", "--all", head_sha, "FETCH_HEAD"])
        return base_sha
...
class AzurePipelinesMeta(GitMeta):
    ...
    def repo_url(self) -> Optional[str]:
        ...
        url = os.getenv("SYSTEM_PULLREQUEST_SOURCEREPOSITORYURI") or os.getenv("BUILD_REPOSITORY_URI")
        return url if url else super().repo_url
    ...
    def commit_sha(self) -> Optional[str]:
        ...
        return os.getenv("SYSTEM_PULLREQUEST_SOURCECOMMITID") or os.getenv("BUILD_SOURCEVERSION")
...
class BuildkiteMeta(GitMeta):
    ...
    def ci_job_url(self) -> Optional[str]:
        ...
        return "{}#{}".format(os.getenv("BUILDKITE_BUILD_URL"), os.getenv("BUILDKITE_JOB_ID"))
...
def generate_meta_from_environment(baseline_ref: Optional[str], subdir: Optional[Path]) -> GitMeta:
