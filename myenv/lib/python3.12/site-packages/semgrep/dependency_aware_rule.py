...
from semgrep.subproject import ResolvedSubproject, find_closest_subproject
...
def parse_depends_on_yaml(entries: List[Dict[str, str]]) -> Iterator[out.ScaPattern]:
    ...
    for entry in entries:
        ...
        yield out.ScaPattern(ecosystem=ecosystem, package=package, semver_range=semver_range)
        ...
        for subproject in resolved_deps.get(ecosystem, []):
            ...
            for dep_pat, found_dep in dependency_matches:
                if found_dep.lockfile_path is None:
                    ...
                    logger.warning(f"Found a dependency ({found_dep.package}) without a lockfile path. Skipping...")
                    continue
                ...
                match = evolve(match, match_based_index=match_based_keys[match.match_based_key])
...
def transitive_dep_is_also_direct(package: str, deps: Tuple[Tuple[str, Transitivity], ...]) -> bool:
    """
...
def generate_reachable_sca_findings(
    ...
    rule: Rule,
    resolved_deps: Dict[Ecosystem, List[ResolvedSubproject]],
) -> Tuple[List[RuleMatch], List[SemgrepError], Callable[[Path, FoundDependency], bool]]:
    depends_on_keys = rule.project_depends_on
    dep_rule_errors: List[SemgrepError] = []
    ...
    for ecosystem in ecosystems:
        for match in matches:
            try:
                subproject = find_closest_subproject(match.path, ecosystem, resolved_deps.get(ecosystem, []))
                ...
                for dep_pat, found_dep in dependency_matches:
                    ...
                    if found_dep.transitivity == Transitivity(Transitive()) and transitive_dep_is_also_direct(
