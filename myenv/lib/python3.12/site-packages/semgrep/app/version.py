...
from typing import Dict, List, Literal, Mapping, Optional, Set, cast
...
def _fetch_latest_version() -> Optional[JsonObject]:
    ...
    try:
        resp = state.app_session.get(state.env.version_check_url, timeout=state.env.version_check_timeout)
    except Exception as e:
        logger.debug(f"Fetching latest version failed to connect: {e}")
        return None

    if resp.status_code != requests.codes.OK:
        logger.debug(f"Fetching latest version received HTTP error code: {resp.status_code}")
        return None
    try:
        res = resp.json()
    except ValueError:
...
def _get_version_filtered_banners() -> List[JsonObject]:
    ...
    for b in banners:
        try:
            ...
            logger.debug(f"Invalid version string: {e}")
            continue
        if (not show or current_version >= show) and (not hide or current_version < hide):
...
def get_too_many_findings_msg() -> Optional[str]:
    ...
    """
    banners = _get_version_filtered_banners()
    too_many_findings_banner = next((b for b in banners if (b.get("identifier") or "") == "too_many_findings"), None)
    if not too_many_findings_banner:
        return None
    message = too_many_findings_banner.get("message") or ""
    identifier = cast(ValidIdentifers, too_many_findings_banner.get("identifier", "") or "")
    icon = IDENTIFIER_LOOKUP.get(identifier) or "⏫"
