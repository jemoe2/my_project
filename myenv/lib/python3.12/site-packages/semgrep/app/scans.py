...
from semgrep.error import INVALID_API_KEY_EXIT_CODE, SemgrepError
...
class ScanHandler:
    def __init__(self, dry_run: bool = False, partial_output: Optional[Path] = None) -> None:
        """
        ...
        if self.scan_response:
            ...
            if self.scan_response.engine_params.ignored_files:
                return out.ProductIgnoredFiles(
                    value={
                        product: [out.Glob(pattern) for pattern in self.scan_response.engine_params.ignored_files]
        ...
        if self.partial_output:
            self.partial_output.write_text(
                out.PartialScanResult(
                    out.PartialScanError(out.CiScanFailure(exit_code=exit_code, stderr=""))
                ).to_json_string()
            )
    ...
    def report_findings(
        ...
        complete = out.CiScanComplete(
            ...
            stats=out.CiScanCompleteStats(
                findings=len([match for match in new_matches if not match.from_transient_scan]),
