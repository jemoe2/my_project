...
SettingsKeys = Literal["has_shown_metrics_notification", "api_token", "anonymous_user_id"]


def generate_anonymous_user_id(api_token: Optional[str]) -> str:
    return str(uuid.uuid4()) if api_token is None else str(uuid.uuid5(uuid.UUID("0" * 32), api_token))
...
class Settings:
    ...
    def get_default_contents(self) -> SettingsSchema:
        ...
        if not isinstance(yaml_contents, Mapping):
            logger.warning(f"Bad settings format; {self.path} will be overridden. Contents:\n{yaml_contents}")
            return default_settings
