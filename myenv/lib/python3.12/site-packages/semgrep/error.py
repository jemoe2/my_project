...
from semgrep.error_location import Position, SourceTracker, Span
...
class SemgrepCoreError(SemgrepError):
    ...
    def adjust_CliError(self, base: out.CliError) -> out.CliError:
        ...
        if self.core.location:
            ...
            # confusing to add
            if not isinstance(self.core.error_type.value, out.RuleParseError) and not isinstance(
    ...
    def is_special_interfile_analysis_error(self) -> bool:
        ...
        """
        return isinstance(self.core.error_type.value, out.OutOfMemoryDuringInterfile) or isinstance(
            self.core.error_type.value, out.TimeoutDuringInterfile
        )
    ...
    def format_for_terminal(self) -> str:
        ...
        for span in self.spans:
            ...
            if span.context_start:
                snippet += self._format_code_segment(span.context_start, previous_line(span.start), source, span)
            ...
            if span.start.line == span.end.line:
                ...
                snippet.append(self._format_line_number(span, None) + " " * (span.start.col - 1) + error)
            if span.context_end:
                snippet += self._format_code_segment(next_line(span.end), span.context_end, source, span)
