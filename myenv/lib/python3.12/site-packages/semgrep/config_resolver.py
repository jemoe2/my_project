...
from semgrep.util import is_config_suffix, is_rules, is_url, with_color
...
class ConfigLoader:
    ...
    def _fetch_semgrep_cloud_platform_scan_config(self) -> ConfigFile:
        ...
        request = out.ScanRequest(
            ...
            project_metadata=self._project_metadata_for_standalone_scan(require_repo_name),
        )
        ...
        try:
            if not config_id:  # registry rules don't have config ids
                ...
                config_id = (
                    ...
                    and (remote_rule_netloc.endswith(".semgrep.dev") or remote_rule_netloc == "semgrep.dev")
...
def resolve_config(
    ...
    config, errors = parse_config_files(config_loader.load_config(), force_jsonschema=force_jsonschema)
    if config:
...
class Config:
    ...
    def from_config_list(
        ...
        for i, config in enumerate(configs):
            try:
                ...
                resolved_config, config_errors = resolve_config(config, project_url, force_jsonschema=force_jsonschema)
                errors.extend(config_errors)
    ...
    @staticmethod
    def _validate(config_dict: Mapping[str, YamlTree]) -> Tuple[Mapping[str, Sequence[Rule]], List[SemgrepError], int]:
        """
...
def get_config(
    ...
    *,
    ...
    if pattern:
        ...
        config, errors = Config.from_config_list(config_strs, project_url, force_jsonschema=force_jsonschema)
