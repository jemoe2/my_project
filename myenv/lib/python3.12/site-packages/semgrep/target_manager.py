...
from semgrep.util import path_has_permissions, sub_check_output, with_color
...
ALL_EXTENSIONS: Collection[FileExtension] = {
    ext for definition in LANGUAGE.definition_by_id.values() for ext in definition.exts if ext != FileExtension("")
}
...
class FileTargetingLog:
    ...
    by_language: Dict[Union[Language, Literal["dependency_source_files"]], Set[Path]] = Factory(
    ...
    def __str__(self) -> str:
        ...
        if self.insufficient_permissions:
            ...
            skip_fragments.append(f"{len(self.insufficient_permissions)} files without read access")
        if self.size_limit:
    ...
    def yield_verbose_lines(self, too_many_entries: int) -> Iterator[Tuple[Literal[0, 1, 2], str]]:
...
class Target:
    ...
    def _is_valid_file_or_dir(self, path: Path) -> bool:
        ...
        return path_has_permissions(path, stat.S_IRUSR, follow_symlinks=False) and not path.is_symlink()
    ...
    def _parse_git_output_nulsep(self, output: str) -> FrozenSet[Path]:
        ...
        if output:
            files = frozenset(
                p for p in (self.path / elem for elem in output.split("\x00") if elem) if self._is_valid_file(p)
            )
        return files
    ...
    def files_from_filesystem(self) -> Tuple[FrozenSet[Path], FrozenSet[Path]]:
        ...
        insufficient_permissions = frozenset(match for match in all if not os.access(match, os.R_OK))
        access_ok = all - insufficient_permissions
        regular_files = frozenset(match for match in access_ok if match.is_file() and not match.is_symlink())
        return (regular_files, insufficient_permissions)

    @lru_cache(maxsize=None)
    def _files(self, ignore_baseline_handler: bool = False) -> Tuple[FrozenSet[Path], FrozenSet[Path]]:
        """
    ...
    def files(self, ignore_baseline_handler: bool = False) -> FrozenSet[Path]:
        selected, _insufficient_permissions = self._files(ignore_baseline_handler=ignore_baseline_handler)
        return selected

    # cached (see _files())
    def paths_with_insufficient_permissions(self, ignore_baseline_handler: bool = False) -> FrozenSet[Path]:
        _selected, insufficient_permissions = self._files(ignore_baseline_handler=ignore_baseline_handler)
...
class TargetManager:
    ...
    def filter_by_size(max_target_bytes: int, *, candidates: FrozenSet[Path]) -> FilteredFiles:
        ...
        kept, removed = partition(
            candidates,
            lambda path: os.path.isfile(path) and os.path.getsize(path) <= max_target_bytes,
        )
    ...
    def get_all_files(self, ignore_baseline_handler: bool = False) -> FrozenSet[Path]:
        return frozenset(f for target in self.targets for f in target.files(ignore_baseline_handler))

    @lru_cache(maxsize=None)
    def get_paths_with_insufficient_permissions(self, ignore_baseline_handler: bool = False) -> FrozenSet[Path]:
        """
        ...
        """
        return frozenset(
            f for target in self.targets for f in target.paths_with_insufficient_permissions(ignore_baseline_handler)
        )
    ...
    def get_files_for_language(
        ...
        files = self.filter_excludes(self.excludes.get(product, []), candidates=files.kept)
        ...
        paths_with_insufficient_permissions = self.get_paths_with_insufficient_permissions(ignore_baseline_handler)
        ...
        self.ignore_log.insufficient_permissions.update(set(files.removed | paths_with_insufficient_permissions))
