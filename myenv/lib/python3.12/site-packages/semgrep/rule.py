import hashlib
import json
from typing import Any, AnyStr, Dict, List, Optional, Sequence, Set, Union, cast

import semgrep.semgrep_interfaces.semgrep_output_v1 as out
from semgrep.constants import RuleScanSource
from semgrep.error import InvalidRuleSchemaError
from semgrep.error_location import Span
from semgrep.rule_lang import EmptySpan, RuleValidation, YamlMap, YamlTree
from semgrep.semgrep_interfaces.semgrep_output_v1 import Ecosystem
from semgrep.semgrep_types import JOIN_MODE, LANGUAGE, SEARCH_MODE, Language
...
class Rule:
    def __init__(self, raw: Dict[str, Any], yaml: Optional[YamlTree[YamlMap]] = None) -> None:
        ...
        lang_span = yaml.value["languages"].span if yaml and "languages" in yaml.value else None

        def resolve_language_string(language_str: str) -> Language:
            # Replace "generic" in the "languages" list by the engine specified
            # in the options section.
            analyzer_str = (
                self.options_dict.get("generic_engine", "spacegrep") if language_str == "generic" else language_str
            )
            return LANGUAGE.resolve(analyzer_str, lang_span)

        rule_languages: Set[Language] = {resolve_language_string(l) for l in self._raw.get("languages", [])}
        ...
        if any(language == LANGUAGE.resolve("javascript") for language in rule_languages):
    ...
    def _validate_none_language_rule(self) -> None:
        ...
        def _recursive_contains(obj: Union[Dict[str, Any], List[Any], str], search_key: str) -> bool:
            """
    ...
    def is_blocking(self) -> bool:
        ...
        validation_state_metadata = self.metadata.get("dev.semgrep.validation_state.actions")
    ...
    def ecosystems(self) -> Set[Ecosystem]:
        if "r2c-internal-project-depends-on" in self._raw:
            ...
            if "depends-on-either" in depends_on:
                dependencies: List[Dict[str, str]] = depends_on["depends-on-either"]
                return {Ecosystem.from_json(d["namespace"].lower()) for d in dependencies}
            else:
    ...
    def formula_string(self) -> str:
        ...
        def get_subrules(raw: Union[AnyStr, Dict, List]) -> str:
            ...
            if isinstance(raw, str):
                ...
                raise ValueError(f"This rule contains an unexpected pattern key: {self.id}:\n {str(raw)}")
