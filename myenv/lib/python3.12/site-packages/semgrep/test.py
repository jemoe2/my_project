...
from semgrep.util import (
    final_suffix_matches,
    is_config_fixtest_suffix,
    is_config_suffix,
    is_config_test_suffix,
)
...
def check_rule_id_mismatch(reported_lines: Dict[str, Dict[str, List[int]]], test_lines: Dict[str, Set]) -> None:
    """
...
def get_expected_and_reported_lines(json_out: Dict[str, Any], test_files: List[Path]) -> Dict[str, Dict[str, Any]]:
    """
    ...
    """
    ruleid_lines: Dict[str, Dict[str, List[int]]] = collections.defaultdict(lambda: collections.defaultdict(list))
    ok_lines: Dict[str, Dict[str, List[int]]] = collections.defaultdict(lambda: collections.defaultdict(list))
    reported_lines: Dict[str, Dict[str, List[int]]] = collections.defaultdict(lambda: collections.defaultdict(list))
    todo_ok_lines: Dict[str, Dict[str, List[int]]] = collections.defaultdict(lambda: collections.defaultdict(list))
    todo_ruleid_lines: Dict[str, Dict[str, List[int]]] = collections.defaultdict(lambda: collections.defaultdict(list))
    ...
    for test_file in test_files:
        ...
        for i, line in enumerate(all_lines):
            ...
            has_parseable_rule_id = (rule_in_line or todo_rule_in_line or ok_in_line or todo_ok_in_line) and ":" in line
            ...
            try:
                ...
                if todo_rule_in_line or rule_in_line:
                    _add_line_to_dict_of_ruleids(rule_ids, ruleid_lines, effective_line_num, test_file_resolved)
                if todo_rule_in_line or ok_in_line:
                    _add_line_to_dict_of_ruleids(rule_ids, ok_lines, effective_line_num, test_file_resolved)
                if todo_ok_in_line:
                    _add_line_to_dict_of_ruleids(rule_ids, todo_ok_lines, effective_line_num, test_file_resolved)
...
def _generate_check_output_line(check_id: str, check_results: Mapping[str, Any]) -> str:
    ...
    def _generate_missed_vs_incorrect_lines(matches: Mapping[str, Any]) -> str:
        ...
        incorrect = f"incorrect lines: {list(set(reported_lines) - set(expected_lines))}"
        return f"{missed}, {incorrect}"

    missed_vs_incorrect_lines = [
        _generate_missed_vs_incorrect_lines(matches) for _, matches in check_results["matches"].items()
    ]
    all_errors = "\n\t".join(json_error_report + missed_vs_incorrect_lines)

    test_file_names = " ".join(test_file for test_file, _ in check_results["matches"].items())

    return f"\t✖ {check_id}\n\t{all_errors}\n\ttest file path: {test_file_names}\n\n"


def _generate_fixcheck_output_line(filename: Path, diff: List[str], fixtest: Path) -> str:
    diff_lines = "\n\t".join(diff)
    return f"\t✖ {fixtest} <> autofix applied to {filename} \n\n\t{diff_lines}\n\n\n"


def invoke_semgrep_multi(config: Path, targets: List[Path], **kwargs: Any) -> Tuple[Path, Optional[str], Any]:
    try:
...
def relatively_eq(parent_target: Path, target: Path, parent_config: Path, config: Path) -> bool:
    ...
    if len(rel1) < s:
        return False
    s -= 1
    return rel1[:s] == rel2[:s] and remove_all_suffixes(rel1[s]) == remove_all_suffixes(rel2[s])
...
def get_config_filenames(original_config: Path) -> List[Path]:
    ...
    return [
        ...
        if is_config_suffix(config) and not config.name.startswith(".") and not config.parent.name.startswith(".")
    ]
...
def get_config_test_filenames(
    ...
    def target_matches_config(target: Path, config: Path) -> bool:
        ...
        return (
            (original_target_is_file_not_directory or relatively_eq(original_target, target, original_config, config))
            and target.is_file()
            and correct_suffix
        )

    return {config: [target for target in targets if target_matches_config(target, config)] for config in configs}
...
def get_config_fixtest_filenames(
    ...
    def fixtest_matches_target(target: Path, fixtest: Path) -> bool:
        correct_suffix = is_config_fixtest_suffix(fixtest) and final_suffix_matches(target, fixtest)
        ...
        return correct_suffix and relatively_eq(original_target_directory, target, original_target_directory, fixtest)
...
def config_contains_fix_key(config: Path) -> bool:
    with open(config) as file:
        ...
        if rules.get("rules"):
            return any(("fix" in rule or "fix-regex" in rule) for rule in rules["rules"])
        else:
            return False
...
def checkid_passed(matches_for_checkid: Dict[str, Any]) -> bool:
    for _filename, expected_and_reported_lines in matches_for_checkid.items():
        if not expected_and_reported_lines["expected_lines"] == expected_and_reported_lines["reported_lines"]:
            return False
    return True


def fixed_file_comparison(fixed_testfile: Path, expected_fixed_testfile: str) -> List[str]:
    diff = []
...
def generate_test_results(
    *,
    ...
    config_fixtest_filenames: Dict[Path, List[Tuple[Path, Path]]] = get_config_fixtest_filenames(
        target, config_test_filenames
    )
    ...
    config_with_errors_output = [
        {"filename": str(filename), "error": error, "output": output} for filename, error, output in config_with_errors
    ]
    ...
    config_with_fixtests, config_without_fixtests = partition(fixtest_filenames.items(), lambda c: bool(c[1]))

    configs_missing_fixtests = [str(c) for c, _fixtest in config_without_fixtests if config_contains_fix_key(c)]
    ...
    configs_with_fixtests = {
        config: [(target, fixtest) for target, fixtest in testfiles if os.path.abspath(target) in passed_test_filenames]
    ...
    config_with_tempfiles = [
        (config, [temp_copies[target] for target, _fixtest in testfiles]) for config, testfiles in config_with_fixtests
    ]
    ...
    fixtest_comparisons = {
        temp_copies[target]: fixtest for _config, testfiles in config_with_fixtests for target, fixtest in testfiles
    }
    ...
    any_fixtest_failures = any(
        not fixtest_file_results["passed"] for fixtest_file_results in fixtest_results_output.values()
    )
    ...
    for _filename, rr in results_output.items():
        for check_id, check_results in sorted(rr["checks"].items()):
            ...
            if not check_results["passed"]:
                check_output_lines += _generate_check_output_line(check_id, check_results)
            else:
                num_tests_passed += 1
    ...
    for target_filename, f_results in fixtest_results.items():
        ...
        fixtest = f_results[1]
        if len(filediff) > 0:
            fixtest_file_diffs += _generate_fixcheck_output_line(target_filename, filediff, fixtest)
        else:
            num_fixtests_passed += 1

    if num_tests == 0:
        print("No unit tests found. See https://semgrep.dev/docs/writing-rules/testing-rules")
    elif num_tests == num_tests_passed:
        print(f"{num_tests_passed}/{num_tests}: ✓ All tests passed")
    else:
        print(f"{num_tests_passed}/{num_tests}: {num_tests - num_tests_passed} unit tests did not pass:")
        print(BREAK_LINE)
    ...
    if config_with_errors_output:
        ...
        print("\t" + "\n\t".join(f"{c['filename']}: {c['error']}" for c in config_with_errors_output))
