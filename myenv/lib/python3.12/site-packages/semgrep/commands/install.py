...
from semgrep.state import SemgrepState, get_state
from semgrep.util import abort, sub_check_output
...
def determine_semgrep_pro_path() -> Path:
    ...
    if core_path is None:
        logger.info("Could not find `semgrep-core` executable so not sure where to install semgrep-core-proprietary")
...
def download_semgrep_pro(state: SemgrepState, platform_kind: str, destination: Path) -> None:
    ...
    with state.app_session.get(url, timeout=180, stream=True) as r:
        ...
        if r.status_code == 403:
            logger.warning("Logged in deployment does not have access to Semgrep Pro Engine")
            # FIXME: Needs to be updated before launch Feb 2023
            logger.warning("Visit https://semgrep.dev/products/pro-engine/ for more information.")
            sys.exit(FATAL_EXIT_CODE)
        r.raise_for_status()
        ...
        with (
            ...
            progress.wrap_file(raw_response, total=file_size, description="Downloading...") as r_raw,
        ):
...
def run_install_semgrep_pro() -> None:
    ...
    if sys.platform.startswith("darwin"):
        ...
        platform_kind = "manylinux"
        logger.info("Running on potentially unsupported platform. Installing linux compatible binary")
    ...
    os.chmod(
        ...
        os.stat(semgrep_pro_path_tmp).st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH,
    )
    ...
    try:
        ...
        abort("Downloaded binary failed version check, try again or contact support@semgrep.com")
