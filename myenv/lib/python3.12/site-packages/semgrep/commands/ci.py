...
from semgrep.error import (
    FATAL_EXIT_CODE,
    INVALID_API_KEY_EXIT_CODE,
    MISSING_CONFIG_EXIT_CODE,
    SemgrepError,
)
from semgrep.git import git_check_output, is_git_repo_empty, is_git_repo_root_approx
...
def get_exclude_paths(
    ...
    patterns = {
        product: (
            [pattern.strip() for pattern in requested_patterns.get(product, []) if is_valid_pattern(pattern)]
    ...
    *,
    ...
    with tracing.TRACER.start_as_current_span("semgrep.commands.ci"):
        ...
        if (dump_n_rule_partitions and not dump_rule_partitions_dir) or (
            ...
            logger.info("Both or none of --x-dump-rule-partitions and --x-dump-rule-partitions-dir must be specified.")
            sys.exit(FATAL_EXIT_CODE)

        if partial_config and not partial_output:
            logger.info("When --x-partial-config is specified, --x-partial-output must also be specified.")
            sys.exit(FATAL_EXIT_CODE)
        ...
        if is_git_repo_empty():
            ...
            project_metadata = generate_meta_from_environment(None, subdir).to_project_metadata()
        ...
        try:
            ...
            if scan_handler:
                ...
                # Build scan_metadata
                if code:
                    scan_handler.scan_metadata.requested_products.append(out.Product(out.SAST()))
                if supply_chain:
                    scan_handler.scan_metadata.requested_products.append(out.Product(out.SCA()))
                if run_secrets_flag:
                    scan_handler.scan_metadata.requested_products.append(out.Product(out.Secrets()))
                ...
                with Progress(
                    ...
                    progress_bar.update(start_scan_task, completed=100, description=start_scan_desc)

                    product_names = [PRODUCT_NAMES_MAP.get(p) or p for p in scan_handler.enabled_products]
                    products_str = ", ".join(product_names) or "None"
                    products_task = progress_bar.add_task(f"Enabled products: [bold]{products_str}[/bold]")
                    progress_bar.update(products_task, completed=100)

                if scan_handler.rules == '{"rules":[]}' and set(scan_handler.enabled_products).issubset(
                    {"sast", "secrets"}
                ):
                    console.print(
        ...
        per_product_excludes = {product: [*exclude] if exclude else [] for product in ALL_PRODUCTS}
        excludes_from_app = (
            {
        ...
        run_scan_args = {
            ...
            "resolve_all_deps_in_diff_scan": scan_handler.resolve_all_deps_in_diff_scan if scan_handler else False,
        }
        ...
        try:
            ...
            if scan_handler:
                ...
                if dump_n_rule_partitions and scan_handler.enabled_products != ["sast"]:
                    raise SemgrepError("--x-dump-rule-partitions is only compatible with SAST.")
        ...
        if run_historical_secrets_scan and metadata.merge_base_ref:
            logger.info(f"Historical scanning was enabled, but is not yet supported on diff scans.")
            ...
            try:
                ...
                output_handler.handle_semgrep_errors([e])
                logger.info(f"Encountered error when running rules for historical scan: {e}")
        ...
        if scan_handler:
            ...
            if complete_result.app_block_override and len(complete_result.app_blocking_match_based_ids) > 0:
                num_blocking_findings = 0
                num_nonblocking_findings = 0
                app_blocked_mids = set([x.value for x in complete_result.app_blocking_match_based_ids])
            ...
            if internal_ci_scan_results:
                ...
                print(
                    json.dumps(
                        (scan_handler.ci_scan_results.to_json() if scan_handler.ci_scan_results else {}),
                        sort_keys=True,
        ...
        app_policy_block = (
            complete_result and complete_result.app_block_override and complete_result.app_blocking_match_based_ids
        )

        # The app is saying that there is some other reason for blocking (currently only ever used for SCA license scanning)
        app_other_block = complete_result and complete_result.app_block_override and complete_result.app_block_reason
        ...
        if cli_suggested_exit_code == 1 or app_policy_block:
            ...
            if app_other_block and not audit_mode:
                ...
                logger.info(f"  semgrep.dev is suggesting a non-zero exit code ({complete_result.app_block_reason})")
                exit_code = 1
            else:
