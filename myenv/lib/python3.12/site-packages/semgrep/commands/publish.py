...
from semgrep.test import get_config_filenames, get_config_test_filenames
...
def publish(target: str, visibility: VisibilityState, registry_id: Optional[str]) -> None:
    """
    ...
    click.echo(f'Found {len(config_filenames)} configs to publish with visibility "{visibility}"')

    for config_filename in config_filenames:
        test_cases = config_test_filenames.get(config_filename, [])
        click.echo(f"--> Uploading {config_filename} (test cases: {[str(t) for t in test_cases]})")
...
def _upload_rule(
    ...
    response = state.app_session.post(f"{state.env.semgrep_url}/api/registry/rules", json=request_json)
    ...
    if not response.ok:
        ...
        if visibility == VisibilityState.PUBLIC:
            click.echo(f"    Pull request created for this public rule at: {created_rule['pr_url']}")
        elif visibility == VisibilityState.UNLISTED:
            click.echo(f"    Published {visibility} rule at {state.env.semgrep_url}/s/{created_rule['id']}")
        else:
            click.echo(f"    Published {visibility} rule at {state.env.semgrep_url}/r/{created_rule['path']}")
