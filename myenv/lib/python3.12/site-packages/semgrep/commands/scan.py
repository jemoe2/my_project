...
from semgrep import __VERSION__, bytesize, tracing
from semgrep.app.version import (
    TOO_MANY_FINDINGS_THRESHOLD,
    get_no_findings_msg,
    get_too_many_findings_msg,
)
...
from semgrep.constants import (
    ...
    DEFAULT_MAX_LINES_PER_FINDING,
    DEFAULT_MAX_LOG_LIST_ENTRIES,
...
from semgrep.target_manager import ALL_PRODUCTS, write_pipes_to_disk
from semgrep.util import abort, is_truthy, with_color
...
_scan_options: List[Callable] = [
    ...
    optgroup.option("--gitlab-sast-output", "outputs_gitlab_sast", multiple=True, default=[]),
    optgroup.option("--gitlab-secrets-output", "outputs_gitlab_secrets", multiple=True, default=[]),
    optgroup.option("--junit-xml-output", "outputs_junit_xml", multiple=True, default=[]),
    ...
    optgroup.option("--pro-path-sensitive", "path_sensitive", is_flag=True, default=False),
    optgroup.option(
        "--oss-only",
        "requested_engine",
        type=EngineType,
...
def scan(
    *,
    ...
    if run_secrets_flag and requested_engine is EngineType.OSS:
        abort("Cannot run secrets scan with OSS engine (--oss specified). Semgrep Secrets is a proprietary extension.")
    ...
    with tracing.TRACER.start_as_current_span("semgrep.commands.scan"):
        ...
        if config and "auto" in config and metrics == MetricsState.OFF:
            abort("Cannot create auto config when metrics are off. Please allow metrics or run with a specific config.")
        ...
        with tempfile.TemporaryDirectory() as pipes_dir:
            ...
            return_data: Optional[Tuple[RuleMatchMap, List[SemgrepError], List[Rule], Set[Path]]] = None

            if validate:
                if not (pattern or lang or config):
                    logger.error(f"Nothing to validate, use the --config or --pattern flag to specify a rule")
                else:
                    (
                        resolved_configs,
                        config_errors,
