...
class GroupedOption(click.Option):
    ...
    def __init__(self, param_decls: Optional[Sequence[str]] = None, *, group: "OptionGroup", **attrs: Any):
        ...
        for attr in group.forbidden_option_attrs:
            if attr in attrs:
                raise TypeError(f"'{attr}' attribute is not allowed for '{type(group).__name__}' option `{self.name}'.")
...
class _GroupTitleFakeOption(click.Option):
    ...
    def __init__(self, param_decls: Optional[Sequence[str]] = None, *, group: "OptionGroup", **attrs: Any) -> None:
        self.__group = group
...
class OptionGroup:
    ...
    def __init__(self, name: Optional[str] = None, *, hidden: bool = False, help: Optional[str] = None) -> None:
...
class RequiredAnyOptionGroup(OptionGroup):
    ...
    def handle_parse_result(self, option: GroupedOption, ctx: click.Context, opts: Mapping[str, Any]) -> None:
        ...
        if all(o.hidden for o in self.get_options(ctx).values()):
            ...
            raise TypeError(f"Need at least one non-hidden option in {group_name} option group ({cls_name}).")
...
class RequiredAllOptionGroup(OptionGroup):
    ...
    def handle_parse_result(self, option: GroupedOption, ctx: click.Context, opts: Mapping[str, Any]) -> None:
        ...
        if not option_names.issubset(opts):
            ...
            raise click.UsageError(f"Missing required options from {group_name} option group:\n{option_info}", ctx=ctx)
