...
import pytest
from glom import (  # backwards compat
    OMIT,
    SKIP,
    STOP,
    A,
...
def test_stop():
    ...
    target = ["a", " b", " c ", "   ", "  done"]
    assert glom(target, [(lambda x: x.strip(), lambda x: x if x else STOP, lambda x: x[0])]) == ["a", "b", "c", "", "d"]
    return
...
def test_invoke():
    ...
    spec = Invoke(test).star(args="args").constants(3, b="b").specs(c="c").star(args="args2", kwargs="kwargs")
    repr(spec)  # no exceptions
    assert repr(Invoke(len).specs(T)) == "Invoke(len).specs(T)"
    assert (
        repr(Invoke.specfunc(next).constants(len).constants(1)) == "Invoke.specfunc(next).constants(len).constants(1)"
    )
    ...
    assert args == [(), {}]
    repr_spec = (
        Invoke.specfunc(T).star(args="args").constants(3, b="b").specs(c="c").star(args="args2", kwargs="kwargs")
    )
...
def test_spec_and_recursion():
    ...
    assert glom(["a", "b", "c"], Call(list, args=(Spec(Call(reversed, args=(Spec(T),))),))) == ["c", "b", "a"]
    assert glom(["cat", {"cat": 1}], T[1][T[0]]) == 1
    assert glom([["ab", "cd", "ef"], "".join], Call(T[1], args=(Spec((T[0], [T[1:]])),))) == "bdf"
...
def test_ref():
    ...
    etree2dicts = Ref(
        "ElementTree", {"tag": "tag", "text": "text", "attrib": "attrib", "children": (iter, [Ref("ElementTree")])}
    )
