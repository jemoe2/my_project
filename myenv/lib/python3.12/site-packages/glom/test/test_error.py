...
import pytest
from glom import Coalesce, GlomError, Or, Path, S, Switch, T, glom
...
def _norm_stack(formatted_stack, exc):
    ...
    if _PY2:
        ...
        formatted_stack = formatted_stack.decode("utf8").replace(r"\xc3\xa9", "é")
...
"""
    ...
    actual = _make_stack(Match(Switch([(1, 1), ("a", "a"), ([None], Switch([(1, 1), ("a", "a"), ([None], T.a)]))])))
    expected = """\
Traceback (most recent call last):
  File "test_error.py", line ___, in _make_stack
    glom(target, spec)
...
def test_all_public_errors():
    ...
    err_types = [
        t for t in [getattr(glom, name) for name in dir(glom)] if isinstance(t, type) and issubclass(t, Exception)
    ]
    ...
    err_types = sorted([t for t in err_types if not t is glom.GlomError], key=lambda t: t.__name__)
