...
def test_cruddy_json():
    ...
    # embed a build
    squished_json = Match({"smooshed_json": Auto((json.loads, Match({"sub": Auto((json.loads, M == 1))})))})
    glom({"smooshed_json": json.dumps({"sub": json.dumps(1)})}, squished_json)


def test_pattern_matching():
    pattern_matcher = Or(And(Match(1), Val("one")), And(Match(2), Val("two")), And(Match(float), Val("float")))
    ...
    factorial = (
        lambda t: t + 1,
        Ref("fact", (lambda t: t - 1, (M == 0) & Fill(1) | (S(r=Ref("fact")), S, lambda s: s["r"] * s[T]))),
    )
...
def test_json_ref():
    assert glom(
        ...
        Ref(
            "json",
            Match(Or(And(dict, {Ref("json"): Ref("json")}), And(list, [Ref("json")]), And(0, Val(None)), object)),
        ),
    ) == {"a": {"b": [None, 1]}}
...
def test_nested_struct():
    ...
    _str_json = Ref(
        "json",
        Match(Or(And(dict, {Ref("json"): Ref("json")}), And(list, [Ref("json")]), And(type(""), Auto(str)), object)),
    )
    ...
    rule_spec = Match(
        {
            ...
            "rule_data": _json(
                [  # list of condition-objects
                    {
                        Optional("value", "null"): _json(Or(None, int, float, str, [int, float, str])),
    ...
    rule = dict(
        ...
        rule_data=json.dumps(
            [{"value": json.dumps([1, 2]), "field": 2, "operator": ">"}, {"field": 2, "operator": "=="}]
        ),
    )
