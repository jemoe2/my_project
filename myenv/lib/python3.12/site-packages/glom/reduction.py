...
class Fold(object):
    ...
    def __init__(self, subspec, init, op=operator.iadd):
        ...
        self.op = op
        if not callable(op):
            raise TypeError("expected callable for %s op param, not: %r" % (self.__class__.__name__, op))
        if not callable(init):
            raise TypeError("expected callable for %s init param, not: %r" % (self.__class__.__name__, init))
...
class Count(Fold):
    ...
    def __init__(self):
        super(Count, self).__init__(subspec=T, init=int, op=lambda cur, val: cur + 1)
...
class Merge(Fold):
    ...
    def __init__(self, subspec=T, init=dict, op=None):
        ...
        if not callable(op):
            raise ValueError(
                'expected callable "op" arg or an "init" with an .update()' " method not %r and %r" % (op, init)
            )
