...
def glom_cli(target, spec, indent, debug, inspect):
    ...
    if debug or inspect:
        ...
        spec = Inspect(
            spec, echo=inspect, recursive=inspect, breakpoint=inspect and stdin_open, post_mortem=debug and stdin_open
        )
...
def get_command():
    ...
    cmd.add("--target-format", str, missing="json", doc="format of the source data (json or python)")
    cmd.add("--spec-file", str, missing=None, doc="path to glom spec definition")
    cmd.add(
        "--spec-format", str, missing="python", doc="format of the glom spec definition (json, python, python-full)"
    )

    cmd.add("--indent", int, missing=2, doc="number of spaces to indent the result, 0 to disable pretty-printing")
