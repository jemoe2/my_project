...
class TypeMatchError(MatchError, TypeError):
    ...
    def __init__(self, actual, expected):
        super(TypeMatchError, self).__init__("expected type {0.__name__}, not {1.__name__}", expected, actual)
...
_RE_FUNC_ERROR = ValueError(
    "'func' must be one of %s" % (", ".join(sorted(e and e.__name__ or "None" for e in _RE_VALID_FUNCS)))
)
...
class Regex(object):
    ...
    def glomit(self, target, scope):
        if type(target) not in _RE_TYPES:
            raise MatchError("{0!r} not valid as a Regex target -- expected {1!r}", type(target), _RE_TYPES)
...
class _Bool(object):
    def __init__(self, *children, **kw):
        ...
        if not children:
            raise ValueError("need at least one operand for {}".format(self.__class__.__name__))
...
def _handle_dict(target, spec, scope):
    ...
    required = {
        key for key in spec_keys if _precedence(key) == 0 and type(key) is not Optional or type(key) is Required
    }
    result = {  # pre-load result with defaults
        key.key: key.default for key in spec_keys if type(key) is Optional and key.default is not _MISSING
    }
...
def _glom_match(target, spec, scope):
    if isinstance(spec, type):
        ...
        try:
            ...
            raise MatchError("{0}({1!r}) did not validate (got exception {2!r})", spec.__name__, target, e)
...
class Switch(object):
    ...
    def __init__(self, cases, default=_MISSING):
        ...
        self.default = default
        if not cases:
            raise ValueError("expected at least one case in %s, got: %r" % (self.__class__.__name__, self.cases))
        return
    ...
    def __init__(self, spec=T, **kwargs):
        ...
        def _get_arg_val(name, cond, func, val, can_be_empty=True):
            ...
            if not is_iterable(val):
                ...
                raise ValueError("expected %r argument to contain at least one value," " not: %r" % (name, val))
            for v in val:
                if not func(v):
        ...
        self.instance_of = _get_arg_val("instance_of", "a type", lambda x: isinstance(x, type), instance_of, False)
        ...
        if equal_to is not _MISSING:
            ...
            if one_of is not _MISSING:
                raise TypeError('expected "one_of" argument to be unset when' ' "equal_to" argument is passed')
            ...
            if not one_of:
                raise ValueError('expected "one_of" to contain at least' " one value, not: %r" % (one_of,))
            self.vals = one_of
        else:
            self.vals = ()
        return
    ...
    def glomit(self, target, scope):
        ...
        if self.types and type(target) not in self.types:
            ...
            errs.append(
                ...
                % (
                    self.types[0].__name__ if len(self.types) == 1 else tuple([t.__name__ for t in self.types]),
        ...
        if self.validators:
            for i, validator in enumerate(self.validators):
                try:
                    ...
                    msg = "expected %r check to validate target" % getattr(validator, "__name__", None) or ("#%s" % i)
