...
class PathAccessError(GlomError, AttributeError, KeyError, IndexError):
    ...
    def get_message(self):
        ...
        return "could not access %r, part %r of %r, got error: %r" % (path_part, self.part_idx, self.path, self.exc)
...
class PathAssignError(GlomError):
    ...
    def get_message(self):
        return "could not assign %r on object at %r, got error: %r" % (self.dest_name, self.path, self.exc)
...
class CoalesceError(GlomError):
    ...
    def get_message(self):
        ...
        skipped_vals = [
            v.__class__.__name__ if isinstance(v, self.coal_obj.skip_exc) else "<skipped %s>" % v.__class__.__name__
            for v in self.skipped
        ]
        msg = "no valid values found. Tried %r and got (%s)" % (missed_specs, ", ".join(skipped_vals))
...
class UnregisteredTarget(GlomError):
    ...
    def __repr__(self):
        ...
        return "%s(%r, <type %r>, %r, %r)" % (cn, self.op, self.target_type.__name__, self.type_map, self.path)
...
_BUILTIN_ID_NAME_MAP = dict([(id(v), k) for k, v in __builtins__.items()])
...
class Path(object):
    ...
    def from_text(cls, text):
        ...
        def create():
            segs = text.split(".")
            if PATH_STAR:
                segs = [_T_STAR if seg == "*" else _T_STARSTAR if seg == "**" else seg for seg in segs]
...
def _format_path(t_path):
    ...
    if path_parts or not cur_t_path:
        return "Path(%s)" % ", ".join([_format_t(part) if type(part) is list else repr(part) for part in path_parts])
    ...
    def glomit(self, target, scope):
        ...
        for i in range(len(self._args) // 3):
            ...
            if op == "C":
                ...
                all_kwargs.update({k: v for k, v in kwargs.items() if self._cur_kwargs[k] is kwargs})
            elif op == "S":
                all_args.extend([recurse(arg) for arg in args])
                all_kwargs.update({k: recurse(v) for k, v in kwargs.items() if self._cur_kwargs[k] is kwargs})
            elif op == "*":
                if args is not None:
...
def _t_eval(target, _t, scope):
    ...
    if root is T:
        ...
        if fetch_till > 1 and t_path[1] in (".", "P"):
            ...
            _, kwargs = t_path[2]
            scope.update({k: scope[glom](target, v, scope) for k, v in kwargs.items()})
            return target
...
class Vars(object):
    ...
    def __repr__(self):
        ret = format_invocation(
            self.__class__.__name__, args=(self.base,) if self.base else (), kwargs=self.defaults, repr=bbrepr
        )
        return ret
    ...
    def register(self, target_type, **kwargs):
        ...
        for op_name in sorted(set(self._op_auto_map.keys()) | set(new_op_map.keys())):
            ...
            if handler is not False and not callable(handler):
                raise TypeError('expected handler for op "%s" to be' " callable or False, not: %r" % (op_name, handler))
...
def AUTO(target, spec, scope):
    ...
    raise TypeError(
        "expected spec to be dict, list, tuple, callable, string," " or other Spec-like type, not: %r" % (spec,)
    )
