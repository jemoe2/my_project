...
class LockfileManifestMatcher(SubprojectMatcher):
    ...
    @abstractmethod
    def _lockfile_to_manifest(self, lockfile_path: Path, candidates: FrozenSet[Path]) -> Optional[Path]:
        """
    ...
    def _get_subproject_root(
        self,
        manifest_lockfile_paths: Union[Tuple[Path, None], Tuple[None, Path], Tuple[Path, Path]],
    ) -> Path:
        """
    ...
    def _filter_manifest_lockfiles(self, dep_source_files: FrozenSet[Path]) -> Tuple[Set[Path], Set[Path]]:
        """
    ...
    def make_subprojects(self, dep_source_files: FrozenSet[Path]) -> Tuple[List[Subproject], FrozenSet[Path]]:
        ...
        for lockfile_path in lockfiles:
            matching_manifest_path = self._lockfile_to_manifest(lockfile_path, dep_source_files)
            if matching_manifest_path is not None:
                root_dir = self._get_subproject_root((matching_manifest_path, lockfile_path))
            ...
            dep_source: Union[ManifestLockfileDependencySource, LockfileOnlyDependencySource]
...
class ManifestOnlyMatcher(SubprojectMatcher):
    ...
    def _filter_matching_manifests(self, dep_source_files: FrozenSet[Path]) -> FrozenSet[Path]:
        """
    ...
    def make_subprojects(self, dep_source_files: FrozenSet[Path]) -> Tuple[List[Subproject], FrozenSet[Path]]:
        ...
        for manifest_path in manifests:
            ...
            manifest_dep_source = ManifestOnlyDependencySource(
                manifest=out.Manifest(kind=self.manifest_kind, path=out.Fpath(str(manifest_path))),
            )
            subprojects.append(
                Subproject(
                    root_dir=root_dir,
