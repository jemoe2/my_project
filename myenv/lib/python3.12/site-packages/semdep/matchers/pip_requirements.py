...
class PipRequirementsMatcher(SubprojectMatcher):
    ...
    manifest_kind: out.ManifestKind = field(default_factory=lambda: out.ManifestKind(value=out.RequirementsIn()))
    lockfile_kind: out.LockfileKind = field(default_factory=lambda: out.LockfileKind(value=out.PipRequirementsTxt()))
    ...
    def _is_manifest_match(self, path: Path) -> bool:
        return (
            fnmatch(str(path), f"{self.base_file_pattern}.{self.manifest_file_extension}")
            or str(path) == f"{self.default_manifest_file_base}.{self.manifest_file_extension}"
        )

    def is_match(self, path: Path) -> bool:
        return self._is_manifest_match(path) or self._is_requirements_match(path)

    def _lockfile_to_manifest(self, requirements_path: Path, candidates: FrozenSet[Path]) -> Optional[Path]:
        """
    ...
    def _filter_manifest_requirements(self, dep_source_files: FrozenSet[Path]) -> Tuple[Set[Path], Set[Path]]:
        """
    ...
    def make_subprojects(self, dep_source_files: FrozenSet[Path]) -> Tuple[List[Subproject], FrozenSet[Path]]:
        # find all manifests and requirements files that we will use to build subprojects
        manifests, requirements_files = self._filter_manifest_requirements(dep_source_files)
        ...
        for (
            ...
            lockfile_sources: List[Union[LockfileOnlyDependencySource, ManifestLockfileDependencySource]] = []
            for req_path in sorted(local_requirements_paths):  # sorting so that there is a deterministic order in tests
                matching_manifest_path = self._lockfile_to_manifest(req_path, dep_source_files)
                ...
                lockfile = out.Lockfile(kind=self.lockfile_kind, path=out.Fpath(str(req_path)))

                if manifest is not None:
                    lockfile_sources.append(ManifestLockfileDependencySource(manifest, lockfile))
                else:
