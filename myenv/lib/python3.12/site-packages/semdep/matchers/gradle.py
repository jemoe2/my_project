...
class GradleMatcher(SubprojectMatcher):
    ...
    def _lockfile_to_settings_and_build(
        ...
        possible_settings_paths = [lockfile_path.parent / x for x in self.SETTINGS_FILENAMES]
    ...
    def _sort_source_files(self, dep_source_files: FrozenSet[Path]) -> Tuple[Set[Path], Set[Path], Set[Path]]:
        """
    ...
    def make_subprojects(self, dep_source_files: FrozenSet[Path]) -> Tuple[List[Subproject], FrozenSet[Path]]:
        settings_files, build_files, lockfiles = self._sort_source_files(dep_source_files)
        ...
        for lockfile_path in lockfiles:
            ...
            settings_path, build_path = self._lockfile_to_settings_and_build(lockfile_path, dep_source_files)
            ...
            if manifest is not None:
                subprojects.append(
                    Subproject(
                        ...
                        dependency_source=ManifestLockfileDependencySource(manifest=manifest, lockfile=lockfile),
        ...
        for settings_path in settings_files:
            ...
            build_paths = dep_source_files.intersection(settings_path.parent / x for x in self.BUILD_FILENAMES)
        ...
        return subprojects, frozenset(lockfiles | used_settings_paths | used_build_paths)
