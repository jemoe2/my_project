...
def try_parse_golang_pseudo(
    ...
    try:
        ...
        return GolangPseudoVersion(timestamp=parsed_timestamp, commit_hash=split_raw_version[2])
    except ValueError:
        return None
...
def parse_golang_version(raw_version: str) -> ParsedGolangVersion:
    ...
    if len(split_raw_version) > 1:  # Check if anything exists after the core version spec
        if len(split_raw_version) == 3:
            ...
            parsed_pre_release = parse_golang_pre_release(split_raw_version)
    return ParsedGolangVersion(core=parsed_core, pseudo=parsed_pseudo, pre_release=parsed_pre_release)


def cmp_core(parsed_version_core: GolangVersionCore, specifier_version_core: GolangVersionCore) -> int:
    """
...
def cmp_pre_release(
    ...
    specifier_num_identifiers = len(specifier_version_pre_release.pre_release_identifiers)
...
def cmp_golang_versions(parsed_version: ParsedGolangVersion, specifier_version: ParsedGolangVersion) -> int:
    """
    ...
    if parsed_version.pre_release and specifier_version.pre_release:
        ...
        pre_release_diff = cmp_pre_release(parsed_version.pre_release, specifier_version.pre_release)
...
def compare_golang_specifier(specifier: str, version: str) -> bool:
    ...
    """
    specifier_regex = re.compile(r"""(?P<operator>(==|!=|<=|>=|<|>))\s*(?P<version>.*)""")
