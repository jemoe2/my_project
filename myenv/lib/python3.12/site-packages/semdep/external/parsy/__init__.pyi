...
class Parser(Generic[T]):
    ...
    def times(self: Parser[A], min: int, max: Optional[int | float] = None) -> Parser[List[A]]:
        """
    ...
    def sep_by(self: Parser[A], sep: Parser[B], *, min: int = 0, max: int | float = float("inf")) -> Parser[List[A]]:
        """
...
def string(expected_string: str, transform: Callable[[str], str] = lambda x: x) -> Parser[str]:
    """
...
@overload
def regex(exp: str | bytes | Pattern, flags: int = 0, group: int | str = 0) -> Parser[str]: ...
@overload
def regex(exp: str | bytes | Pattern, flags: int, group: tuple[int, ...] | tuple[str, ...]) -> Parser[tuple[str, ...]]:
    """
...
def string_from(*strings: str, transform: Callable[[str], str] = lambda x: x) -> Parser[str]:
    """
...
def from_enum(enum_cls: type[E], transform: Callable[[str], str] = lambda x: x) -> Parser[E]:
    """
