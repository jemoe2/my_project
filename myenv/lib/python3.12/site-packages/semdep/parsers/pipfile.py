...
from semdep.parsers.poetry import (
    key,
    list_value,
...
def parse_pipfile(
    ...
    sanitized_manifest_deps = (
        {dep.lower().replace("-", "_") for dep in parsed_manifest} if parsed_manifest else parsed_manifest
    )
    ...
    for package, dep_json in deps.items():
        ...
        output.append(
            FoundDependency(
                ...
                resolved_url=None,
                allowed_hashes=(
                    extract_pipfile_hashes([h.as_str() for h in fields["hashes"].as_list()])
                    if "hashes" in fields
                    else {}
                ),
                transitivity=transitivity(sanitized_manifest_deps, [package.lower().replace("-", "_")]),
