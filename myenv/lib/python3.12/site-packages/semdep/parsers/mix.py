...
from semdep.external.parsy import Parser, any_char, regex, string, success
...
inner_dependency_block = lbrace >> atom >> comma >> quoted_str >> comma >> any_array >> rbrace
...
def git_tag(package: str) -> Parser[tuple[int, tuple[str, str]]]:
    return string("tag") >> colon >> version_block.bind(lambda version: mark_line(success((package, version))))
...
package_key_value_block = whitespace >> quoted_str.bind(
    ...
    >> (package_entry_hex_value_block(package) | package_entry_git_value_block(package) | consume_line)
)

many_package_blocks = package_key_value_block.sep_by(whitespace)

lockfile_parser = whitespace >> string("%") >> lbrace >> many_package_blocks << whitespace << rbrace

# Manifest

# {:ehttpc, github: "emqx/ehttpc", tag: "0.4.13", override: true}
manifest_package = whitespace >> lbrace >> atom << any_char.until(rbrace << string(",").optional(), consume_other=True)

# {:ehttpc, github: "emqx/ehttpc", tag: "0.4.13", override: true},
# {:gproc, github: "emqx/gproc", tag: "0.9.0.1", override: true},
# {:rocksdb, github: "emqx/erlang-rocksdb", tag: "1.8.0-emqx-2", override: true},
many_manifest_packages = whitespace >> (comment | (mark_line(manifest_package) << comment.optional())).sep_by(new_lines)
...
inline_manifest_deps = whitespace >> inline_deps >> whitespace >> lbrack >> many_manifest_packages << any_char.many()

manifest_parser = (
    (any_char.until(manifest_deps_declaration) >> manifest_deps) | (any_char.until(inline_deps) >> inline_manifest_deps)
) << any_char.many()
...
def parse_mix(
    ...
    parsed_lockfile, parsed_manifest, errors = safe_parse_lockfile_and_manifest(
        DependencyFileToParse(lockfile_path, lockfile_parser, ScaParserName(MixLock_())),
        (DependencyFileToParse(manifest_path, manifest_parser, ScaParserName(MixLock_())) if manifest_path else None),
    )

    if not parsed_lockfile or not parsed_manifest:
        return [], errors

    direct_deps = _parse_manifest_deps([x for x in parsed_manifest if isinstance(x, tuple)])
