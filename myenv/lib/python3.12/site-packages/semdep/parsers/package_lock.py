    ...
    for package, dep_json in deps.items():
        ...
        output.append(
            FoundDependency(
                ...
                allowed_hashes=extract_npm_lockfile_hash(integrity) if integrity else {},
                resolved_url=resolved_url,
                transitivity=Transitivity(Transitive()) if nested else transitivity(manifest_deps, [package]),
                line_number=dep_json.line_number,
                lockfile_path=Fpath(str(lockfile_path)),
                manifest_path=Fpath(str(manifest_path)) if manifest_path else None,
            )
...
def parse_package_lock(
    ...
    if lockfile_version == 3:
        ...
        if not parsed_manifest:
            ...
            manifest_deps = (
                set(manifest_json["dependencies"].as_dict().keys()) if "dependencies" in manifest_json else set()
            )

        return (
            parse_dependencies_field(lockfile_path, deps.as_dict(), manifest_deps, False, manifest_path),
            errors,
        )
