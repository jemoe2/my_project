...
from semdep.external.parsy import string, string_from, success, whitespace
...
version_specifier = string_from("===", "==", ">=", "<=", ">", "<", "~=", "!=").bind(
    ...
    >> upto(" ", ";", ",", "\n").bind(
        lambda version: success((operator, version) if operator == "==" and "*" not in version else None)
    )
)
...
dep = package.bind(
    ...
    >> (
        ...
        >> version_specifier.sep_by(string(",") >> whitespace.optional()).bind(
            lambda version_specifiers: upto("\n").optional() >> success((package, [x for x in version_specifiers if x]))
        )
    )
)
