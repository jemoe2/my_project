...
from semdep.external.parsy import any_char, eof, regex, string, success
...
#   bar
# ]
list_value = string("[") >> any_char.until(string("]") << (string("\n") | eof)).result("") << string("]")

# Examples:
# {version = "*", optional = true, markers = "python_full_version <= \"3.11.0a6\" and extra == \"toml\""}
object_value = string("{") >> any_char.until(string("}") << (string("\n") | eof)).result("") << string("}")
...
multi_line_quoted_value = (
    ...
    >> any_char.until((string("'''") | string('"""')) << string("\n").optional()).result("")
    << (string("'''\n") | string('"""\n'))
)
...
poetry_dep = mark_line(
    string("[[package]]\n")
    >> key_value_list.map(
        lambda x: {key_val[0]: ValueLineWrapper(line_number, key_val[1]) for line_number, key_val in x}
    )
)
...
dependencies_parser = new_lines.many() >> string("[package.dependencies]\n").optional().bind(
    lambda title: (
        key_value_list.map(lambda child_info: {"children": [dep[0] for _, dep in child_info]})
        if title
...
poetry_dep_ptt_parser = poetry_dep.bind(
    lambda poetry_dep_info: dependencies_parser.map(lambda deps: dict_to_dependency_dict(poetry_dep_info, deps))
)
...
# secondary = False
poetry_source_extra = (string("[[") >> upto("]") << string("]]\n") << new_lines.optional() >> key_value_list).map(
    lambda _: None
)
...
# python = "<3.10"
# version = ">=4.0"
poetry_dep_extra = (string("[") >> upto("]") << string("]\n")).at_least(1) >> key_value_list.map(lambda _: None)
...
# faker = "^13.11.0"
manifest_deps = (
    string("[tool.poetry.dependencies]\n") << new_lines.optional() >> key_value.map(lambda x: x[0]).sep_by(new_lines)
)
...
def parse_poetry(
    ...
    sanitized_manifest_deps = (
        {dep.lower().replace("-", "_") for dep in (parsed_manifest if parsed_manifest else set())}
        if parsed_manifest
    ...
    for _line_number, dep in parsed_lockfile:
        if "name" not in dep or "version" not in dep:
            continue
        dep_version_map[dep["name"].value.lower().replace("_", "-")] = dep["version"].value
    for _line_number, dep in parsed_lockfile:
        if "name" not in dep or "version" not in dep:
            continue
        output.append(
