...
from semdep.external.parsy import any_char, regex, string
...
dependencies_block = regex(r"dependencies:\s*\[") >> whitespace >> multiple_package_blocks << consume_line.many()

package_swift_parser = any_char.until(regex(r"dependencies\s*:")) >> dependencies_block << any_char.many()
...
def parse_package_resolved(
    ...
    parsed_lockfile, parsed_manifest, errors = safe_parse_lockfile_and_manifest(
        ...
        (
            DependencyFileToParse(manifest_path, package_swift_parser, ScaParserName(PackageSwift_()))
            if manifest_path
            else None
        ),
    )
    ...
    if lockfile_version_int == 1:
        all_deps = parse_swiftpm_v1(lockfile_path, lockfile_json, direct_deps, manifest_path)
    elif lockfile_version_int == 2 or lockfile_version_int == 3:
        all_deps = parse_swiftpm_v2_v3(lockfile_path, lockfile_json, direct_deps, manifest_path)
    else:
        errors.append(
            DependencyParserError(
                out.Fpath(str(lockfile_path)),
