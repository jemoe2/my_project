...
from typing import List, Optional, Set, Tuple, TypedDict, TypeVar, Union
...
def create_dependency_dict(dependency_info: list[tuple[str, str]], children: list[tuple[str, str]]) -> DependencyDict:
    raw_dict = {}
    ...
    .bind(
        lambda title: (
            key_value1.sep_by(string("\n")).map(
                lambda child_info: [
                    dep_version_pair(dep.strip('"').strip("@"), version) for dep, version in child_info
                ],
            )
            if title
            else success([])
    ...
    .bind(
        lambda title: (
            key_value2.sep_by(string("\n    ")).map(
                lambda child_info: [dep_version_pair(dep.strip('"'), version) for dep, version in child_info],
            )
            if title
            else success([])
        )
    ...
    pair(
        ...
        key_value2.sep_by(string("\n")).bind(
            lambda dep_info: dependencies2.map(
                lambda deps: create_dependency_dict(dependency_info=[x for x in dep_info], children=deps)
            )
            << (string("\n  ").optional() >> line.sep_by(string("\n  ")).map(lambda _: None))
        ),
    ),
...
def parse_yarn(
    ...
    parsed_lockfile, parsed_manifest, errors = safe_parse_lockfile_and_manifest(
        ...
        DependencyFileToParse(manifest_path, json_doc, ScaParserName(Jsondoc())) if manifest_path else None,
    )
    ...
    for _line_number, (sources, fields) in parsed_lockfile:
        if len(sources) < 1 or "version" not in fields:
            continue
        sources = [dep_version_pair(package, version_req) for package, version_req in sources]
