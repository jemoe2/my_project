...
from semdep.external.parsy import (
    ParseError,
    Parser,
    alt,
    fail,
...
LegacySemgrepParser = Callable[[Path, str, Optional[str]], Generator[FoundDependency, None, None]]
...
def to_parser(parser: LegacySemgrepParser, parser_type: ScaParserName) -> SemgrepParser:
    ...
    def wrapped_parser(
        ...
        try:
            ...
            return (
                [],
                [DependencyParserError(out.Fpath(str(lockfile_path)), parser_type, str(e))],
            )
...
def upto(
    *s: str,
    ...
    if not allow_newline:
        s = (*s, "\n")
    if include_other:
        return not_any(*s).bind(lambda x: alt(*(string(x) for x in s)).map(lambda y: x + y))
    elif consume_other:
...
def parse_error_to_str(e: ParseError) -> str:
    ...
    if len(expected_list) == 1:
        ...
        return (
            f"expected {expected_list[0] if expected_list[0].startswith('Any char not in') else repr(expected_list[0])}"
        )
    else:
...
def parse_dependency_file(
    ...
    if not text.strip():
        ...
        return DependencyParserError(out.Fpath(str(file_to_parse.path)), file_to_parse.parser_name, reason)
    ...
    try:
        ...
        return DependencyParserError(out.Fpath(file_to_parse.path.name), file_to_parse.parser_name, str(e))
        ...
        if line < len(text_lines):
            ...
            reason = f"Parser failed at the beginning of {file_to_parse.path}.\n{error_str}"
            return DependencyParserError(out.Fpath(str(file_to_parse.path)), file_to_parse.parser_name, reason)
        else:
...
class ParsedDependency:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.package == other.package and self.version == other.version and self.transitivity == other.transitivity
        )
