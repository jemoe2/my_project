...
manifest = (
    ...
    >> (manifest_line | success(None)).sep_by(string("\n"), min=1).map(lambda xs: {x for x in xs if x})
    << any_char.many()
)

gradle = (
    comment_line.many()  # Optionally parse comments at the beginning
    >> (dep | (regex("empty=[^\n]*").result(None))).sep_by(string("\n")).map(lambda xs: [x for x in xs if x])
    << string("\n").optional()
)
...
def parse_gradle(
    ...
    parsed_lockfile, parsed_manifest, errors = safe_parse_lockfile_and_manifest(
        ...
        DependencyFileToParse(manifest_path, manifest, ScaParserName(GradleLockfile_())) if manifest_path else None,
    )
