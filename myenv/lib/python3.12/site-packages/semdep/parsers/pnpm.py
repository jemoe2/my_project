...
SNAPSHOT_KEY_PATTERN = re.compile(r"^(?P<package>.+?)@(?P<version>[^\(]+)(?P<contexts>(\(.+?\))*)$")
...
def validate_lockfile_dependencies_completeness(
    ...
    """
    all_children = {(child.package, child.version) for children in adjacency_map.values() for child in children}
    ...
    error_str = (
        "The following packages are referenced as dependencies of other packages but are not resolved in the lockfile:"
    )
...
def build_package_key_mapping_post_v9(
    ...
    for key, _value in snapshots.items():
        ...
        canonical_package_key = f"{package_key_match.group('package')}@{package_key_match.group('version')}"
...
def parse_dependencies(
    ...
    return [
        DependencyChild(package=package.value, version=version.value) for package, version in dependencies.value.items()
    ]
...
def parse_dep_children_post_v9(
    ...
    all_dependencies: List[DependencyChild] = parse_dependencies(snapshot.value["dependencies"])
    ...
    return [sanitize_dependency_post_v9(dep) for dep in all_dependencies if dep.package not in peer_dependencies]
...
def parse_pnpm(
    ...
    parsed_lockfile, _, errors = safe_parse_lockfile_and_manifest(
        DependencyFileToParse(
            lockfile_path,
            lambda text: parse_yaml_preserve_spans(text, str(lockfile_path), allow_null=True),
    ...
    if lockfile_version <= 5.4:
        ...
        canonical_package_key_mapping = build_package_key_mapping_post_v9(parsed_lockfile)
    ...
    try:
        ...
        if not package_map or not isinstance(package_map, YamlTree) or not package_map.value:
            return [], errors
