...
from . import _wcmatch, _wcparse, util
...
class _GlobSplit(Generic[AnyStr]):
    ...
    def split(self) -> list[_GlobPart]:
        ...
        if (self.extmatchbase and not parts[0].is_drive) or (
...
class Glob(Generic[AnyStr]):
    ...
    def __init__(
        ...
        if not isinstance(temp, bytes if ptype else str):
            raise TypeError(
                "Pattern and root_dir should be of the same type, not {} and {}".format(type(pats[0]), type(temp))
            )
    ...
    def _iter_patterns(self, patterns: Sequence[AnyStr], force_negate: bool = False) -> Iterator[tuple[bool, AnyStr]]:
        ...
        try:
            ...
            raise _wcparse.PatternLimitException("Pattern limit exceeded the limit of {:d}".format(self.limit)) from e
    ...
    def _parse_patterns(self, patterns: Sequence[AnyStr], force_negate: bool = False) -> None:
        ...
        if (
            ...
            and not self.flags & NODOTDIR
            and not self.nounique
            and not (self.pathlib and self.scandotdir)
        ):
            self.nounique = True
    ...
    def _glob_dir(
        self, curdir: AnyStr, matcher: Callable[..., Any] | None, dir_only: bool = False, deep: bool = False
