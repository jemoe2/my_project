...
import os
import re
from typing import Any, AnyStr, Generic, Iterator
...
class WcMatch(Generic[AnyStr]):
        ...
        if self.file_check is None:
            if not file_pattern:
                self.file_check = _wcmatch.WcRegexp(
                    (re.compile(rb"^.*$" if isinstance(file_pattern, bytes) else r"^.*$", re.DOTALL),)
                )
            else:
    ...
    def _valid_file(self, base: AnyStr, name: AnyStr) -> bool:
        ...
        if self.file_check is not None and self.compare_file(
            fullpath[self._base_len :] if self.file_pathname else name
        ):
            valid = True
    ...
    def _valid_folder(self, base: AnyStr, name: AnyStr) -> bool:
        ...
        valid = True
        fullpath = os.path.join(base, name)
        if not self.recursive or (
            self.folder_exclude_check
            and not self.compare_directory(fullpath[self._base_len :] if self.dir_pathname else name)
        ):
            valid = False
        if valid and (not self.show_hidden and util.is_hidden(fullpath)):
            valid = False
