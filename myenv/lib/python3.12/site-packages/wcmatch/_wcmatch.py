...
import re
import stat
from typing import Any, AnyStr, Generic, Pattern, cast
...
RE_WIN_MOUNT = (re.compile(r"\\|[a-z]:(?:\\|$)", re.I), re.compile(rb"\\|[a-z]:(?:\\|$)", re.I))
RE_MOUNT = (re.compile(r"/"), re.compile(rb"/"))
...
class _Match(Generic[AnyStr]):
    ...
    def _match_real(self, symlinks: dict[tuple[int | None, AnyStr], bool], root: AnyStr, dir_fd: int | None) -> bool:
...
class WcRegexp(util.Immutable, Generic[AnyStr]):
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            isinstance(other, WcRegexp)
            and self._include == other._include
            and self._exclude == other._exclude
    ...
    def __ne__(self, other: Any) -> bool:
        ...
        return (
            not isinstance(other, WcRegexp)
            or self._include != other._include
            or self._exclude != other._exclude
            or self._real != other._real
            or self._path != other._path
            or self._follow != other._follow
        )

    def match(self, filename: AnyStr, root_dir: AnyStr | None = None, dir_fd: int | None = None) -> bool:
        """Match filename."""

        return _Match(filename, self._include, self._exclude, self._real, self._path, self._follow).match(
            root_dir=root_dir, dir_fd=dir_fd
        )
