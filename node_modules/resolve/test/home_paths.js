...
test("homedir module paths", function (t) {
  ...
  makeTempDir(t, hnm, function (err) {
    ...
    t.equal(
      require.resolve("baz"),
      bazHNMmain,
      "sanity check: require.resolve finds HNM `baz`",
    );
    var dotMainMain = path.join(dotMainDir, "index.js");
    t.equal(
      require.resolve("dot_main"),
      dotMainMain,
      "sanity check: require.resolve finds `dot_main`",
    );
    ...
    makeTempDir(t, hnl, function (err) {
      ...
      copyDir.sync(
        path.join(__dirname, "resolver/dot_slash_main"),
        dotSlashMainDir,
      );
      ...
      t.equal(
        require.resolve("dot_slash_main"),
        dotSlashMainMain,
        "sanity check: require.resolve finds HNL `dot_slash_main`",
      );
      ...
      t.test("with temp dirs", function (st) {
        ...
        st.test("just in `$HOME/.node_modules`", function (s2t) {
          ...
          resolve("dot_main", function (err, res, pkg) {
            ...
            s2t.equal(
              res,
              dotMainMain,
              "`dot_main` resolves in `$HOME/.node_modules`",
            );
        ...
        st.test("just in `$HOME/.node_libraries`", function (s2t) {
          ...
          resolve("dot_slash_main", function (err, res, pkg) {
            ...
            s2t.equal(
              res,
              dotSlashMainMain,
              "`dot_slash_main` resolves in `$HOME/.node_libraries`",
            );
        ...
        st.test(
          "in `$HOME/.node_libraries` and `$HOME/.node_modules`",
          function (s2t) {
            s2t.plan(3);
            ...
            resolve("baz", function (err, res, pkg) {
              ...
              s2t.equal(
                res,
                bazHNMmain,
                "`baz` resolves in `$HOME/.node_modules` when in both",
              );
