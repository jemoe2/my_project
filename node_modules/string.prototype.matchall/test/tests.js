...
var groups = function groups(matchObject) {
  return hasGroups
    ? assign(matchObject, { groups: matchObject.groups }, matchObject)
    : matchObject;
};
...
var testResults = function (t, iterator, expectedResults, item) {
  ...
  t.test(prefix + "actual vs expected result lengths", function (st) {
    st.equal(
      results.length,
      expecteds.length,
      "actual and expected result counts are the same",
    );
    st.end();
  });
  t.test(
    prefix + "actual vs expected results",
          ...
          function (s2t) {
            if (expected.done) {
              s2t.equal(
                result.value,
                undefined,
                "result " + (index + 1) + " value is undefined",
              );
            } else {
              s2t.equal(
                Array.isArray(result.value),
                true,
                "result " + (index + 1) + " value is an array",
              );
              s2t.deepEqual(
                entries(result.value || {}),
                entries(expected.value || {}),
                "result " + (index + 1) + " has the same entries",
              );
              s2t.deepEqual(
                result.value,
                expected.value,
                "result " + (index + 1) + " value is expected value",
              );
            }
            s2t.end();
          },
...
module.exports = function (matchAll, regexMatchAll, t) {
  ...
  t.test("#flags", function (st) {
    st.test("without a flags property", function (s2t) {
      ...
      s2t.equal(
        regex.flags,
        undefined,
        'regex has an undefined "flags" property',
      );
      s2t["throws"](function () {
        matchAll(str, regex);
    ...
    st.test("with a static flags property", function (s2t) {
      ...
      try {
        ...
        s2t.comment(
          "# SKIP in node < 6, `global` is not configurable on regexes",
        );
        return s2t.end();
      }
  ...
  t.test("returns an iterator", function (st) {
    ...
    st.equal(
      has.call(iterator, "next"),
      false,
      'iterator does not have own property "next"',
    );
  ...
  t.test("zero-width matches", function (st) {
    ...
    st.test("sticky", { skip: !hasSticky }, function (s2t) {
      var expectedResults = [{ value: undefined, done: true }];
