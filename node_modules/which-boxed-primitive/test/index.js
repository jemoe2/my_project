...
var objects = [/a/g, new Date(), function () {}, [], {}].concat(v.objects);
  ...
  t.test("unboxed primitives", function (st) {
    forEach(v.primitives, function (primitive) {
      st.equal(
        ...
        inspect(primitive) + " is a primitive, but not a boxed primitive",
      );
    });
    st.end();
  });
    ...
    forEach(v.primitives, function (primitive) {
      if (primitive != null) {
        ...
        st.equal(
          ...
          expected,
          inspect(boxed) + " is a boxed primitive: " + expected,
        );
      }
    });
    st.end();
  ...
  t.test("non-primitive objects", function (st) {
    forEach(objects, function (object) {
      st.equal(
        ...
        inspect(object) + " is not a primitive, boxed or otherwise",
      );
    });
    st.end();
  });
