...
test("dataViewByteOffset", function (t) {
  ...
  t.test("DataView", { skip: typeof DataView !== "function" }, function (st) {
    ...
    st.equal(
      dataViewByteOffset(dv),
      2,
      inspect(dv) +
        " has the same byteOffset originally passed to the DataView",
    );
    st.equal(
      dataViewByteOffset(dv),
      dv.byteOffset,
      inspect(dv) + " has the same byteOffset as the buffer’s byteOffset",
    );
