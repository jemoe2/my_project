...
module.exports = function TypedArrayCreateFromConstructor(
  constructor,
  argumentList,
) {
  ...
  if (availableTypedArrays.length === 0) {
    throw new $SyntaxError(
      "Assertion failed: Typed Arrays are not supported in this environment",
    );
  }
  ...
  if (argumentList.length === 1 && typeof argumentList[0] === "number") {
    ...
    if (length < argumentList[0]) {
      throw new $TypeError(
        "`argumentList[0]` must be <= `newTypedArray.length`",
      ); // step 3.c
    }
  }
