...
module.exports = function IsViewOutOfBounds(viewRecord) {
  if (!isDataViewWithBufferWitnessRecord(viewRecord)) {
    throw new $TypeError(
      "Assertion failed: `viewRecord` must be a DataView With Buffer Witness Record",
    );
  }
  ...
  if (
    IsDetachedBuffer(dataViewBuffer(view)) !==
    (bufferByteLength === "DETACHED")
  ) {
    // step 3
    throw new $TypeError(
      "Assertion failed: `IsDetachedBuffer(dataViewBuffer(view))` must be true if and only if `bufferByteLength === ~DETACHED~",
    );
  }
  ...
  var byteOffsetEnd =
    dvByteLength === "AUTO" ? bufferByteLength : byteOffsetStart + dvByteLength; // steps 6 - 7
