...
module.exports = function SetValueInBuffer(
  ...
  if (!isArrayBuffer(arrayBuffer) && !isSAB) {
    throw new $TypeError(
      "Assertion failed: `arrayBuffer` must be an ArrayBuffer or a SharedArrayBuffer",
    );
  }

  if (!isInteger(byteIndex) || byteIndex < 0) {
    throw new $TypeError(
      "Assertion failed: `byteIndex` must be a non-negative integer",
    );
  }

  if (typeof type !== "string" || !hasOwn(tableTAO.size, "$" + type)) {
    throw new $TypeError(
      "Assertion failed: `type` must be a Typed Array Element Type",
    );
  }

  if (typeof value !== "number" && typeof value !== "bigint") {
    throw new $TypeError(
      "Assertion failed: `value` must be a Number or a BigInt",
    );
  }
  ...
  if (order !== "SEQ-CST" && order !== "UNORDERED" && order !== "INIT") {
    throw new $TypeError(
      'Assertion failed: `order` must be `"SEQ-CST"`, `"UNORDERED"`, or `"INIT"`',
    );
  }

  if (arguments.length > 6 && typeof arguments[6] !== "boolean") {
    throw new $TypeError(
      "Assertion failed: `isLittleEndian` must be a boolean, if present",
    );
  }
  ...
  if (
    ...
    // step 3
    throw new $TypeError(
      "Assertion failed: `value` must be a BigInt if type is ~BIGINT64~ or ~BIGUINT64~, otherwise a Number",
    );
  }
  ...
  var isLittleEndian =
    arguments.length > 6 ? arguments[6] : defaultEndianness === "little"; // step 6
  ...
  if (isSAB) {
    ...
    throw new $SyntaxError(
      "SharedArrayBuffer is not supported by this implementation",
    );
  } else {
