...
module.exports = function IsTypedArrayOutOfBounds(taRecord) {
  if (!isTypedArrayWithBufferWitnessRecord(taRecord)) {
    throw new $TypeError(
      "Assertion failed: `taRecord` must be a TypedArray With Buffer Witness Record",
    );
  }
  ...
  if (
    IsDetachedBuffer(typedArrayBuffer(O)) &&
    bufferByteLength !== "DETACHED"
  ) {
    throw new $TypeError(
      "Assertion failed: typed array is detached only if the byte length is ~DETACHED~",
    ); // step 3
  }
