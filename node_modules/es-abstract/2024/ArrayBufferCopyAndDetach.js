...
var $abSlice =
  callBound("%ArrayBuffer.prototype.slice%", true) ||
  function slice(ab, a, b) {
    // in node < 0.11, slice is an own nonconfigurable property
...
module.exports = function ArrayBufferCopyAndDetach(
  arrayBuffer,
  newLength,
  preserveResizability,
) {
  if (
    preserveResizability !== "PRESERVE-RESIZABILITY" &&
    preserveResizability !== "FIXED-LENGTH"
  ) {
    throw new $TypeError(
      "`preserveResizability` must be ~PRESERVE-RESIZABILITY~ or ~FIXED-LENGTH~",
    );
  }
  ...
  var newBuffer =
    newMaxByteLength === "EMPTY"
      ? new $ArrayBuffer(newByteLength)
      : new $ArrayBuffer(newByteLength, { maxByteLength: newMaxByteLength });
