...
module.exports = function SetTypedArrayFromTypedArray(
  target,
  targetOffset,
  source,
) {
  ...
  if (!whichTarget) {
    throw new $TypeError(
      "Assertion failed: target must be a TypedArray instance",
    );
  }

  if (
    targetOffset !== Infinity &&
    (!isInteger(targetOffset) || targetOffset < 0)
  ) {
    throw new $TypeError(
      "Assertion failed: targetOffset must be a non-negative integer or +Infinity",
    );
  }
  ...
  if (!whichSource) {
    throw new $TypeError(
      "Assertion failed: source must be a TypedArray instance",
    );
  }
  ...
  if (targetOffset === Infinity) {
    throw new $RangeError(
      "targetOffset must be a non-negative integer or +Infinity",
    ); // step 13
  }

  if (srcLength + targetOffset > targetLength) {
    throw new $RangeError(
      "targetOffset + source.length must not be greater than target.length",
    ); // step 14
  }

  var targetContentType =
    whichTarget === "BigInt64Array" || whichTarget === "BigUint64Array"
      ? "BigInt"
      : "Number";
  var sourceContentType =
    whichSource === "BigInt64Array" || whichSource === "BigUint64Array"
      ? "BigInt"
      : "Number";
