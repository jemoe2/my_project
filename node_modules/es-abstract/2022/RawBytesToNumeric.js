...
module.exports = function RawBytesToNumeric(type, rawBytes, isLittleEndian) {
  if (!hasOwnProperty(TypeToSizes, type)) {
    throw new $TypeError(
      "Assertion failed: `type` must be a TypedArray element type: " +
        keys(TypeToSizes),
    );
  }
  if (!IsArray(rawBytes) || !every(rawBytes, isByteValue)) {
    throw new $TypeError(
      "Assertion failed: `rawBytes` must be an Array of bytes",
    );
  }
  if (typeof isLittleEndian !== "boolean") {
    throw new $TypeError(
      "Assertion failed: `isLittleEndian` must be a Boolean",
    );
  }
  ...
  if (rawBytes.length !== elementSize) {
    ...
    throw new $RangeError(
      "Assertion failed: `rawBytes` must have a length of " +
        elementSize +
        " for type " +
        type,
    );
