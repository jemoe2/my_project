...
module.exports = function GetSubstitution(
  ...
  if (!isInteger(position) || position < 0) {
    throw new $TypeError(
      "Assertion failed: `position` must be a nonnegative integer, got " +
        inspect(position),
    );
  }
  ...
  if (!IsArray(captures) || !every(captures, isStringOrUndefined)) {
    throw new $TypeError(
      "Assertion failed: `captures` must be a possibly-empty List of Strings or `undefined`, got " +
        inspect(captures),
    );
  }
  ...
  if (typeof namedCaptures !== "undefined" && !isObject(namedCaptures)) {
    throw new $TypeError(
      "Assertion failed: `namedCaptures` must be `undefined` or an Object",
    );
  }

  if (typeof replacementTemplate !== "string") {
    throw new $TypeError(
      "Assertion failed: `replacementTemplate` must be a String",
    );
  }
  ...
  if (position > stringLength) {
    throw new $TypeError(
      "Assertion failed: position > stringLength, got " + inspect(position),
    ); // step 2
  }
    ...
    if (isPrefixOf("$$", templateRemainder)) {
      ...
      for (var d = 2; d > 0; d -= 1) {
        ...
        if (!found) {
          ...
          if (index < 0 || index > 99) {
            throw new $TypeError(
              "Assertion failed: `index` must be >= 0 and <= 99",
            ); // step 5.f.ii.1.e
          }
          if (index === 0) {
            // step 5.f.ii.1.f
      ...
      if (gtPos === -1 || typeof namedCaptures === "undefined") {
        ...
        if (!isObject(namedCaptures)) {
          throw new $TypeError(
            "Assertion failed: Type(namedCaptures) is not Object",
          ); // step 5.g.iii.3
        }
