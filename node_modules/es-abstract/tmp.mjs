...
const typesPackagesPresent = Object.entries(devDependencies).filter(([name]) =>
  name.startsWith("@types/"),
);

console.log(`Found ${typesPackagesPresent.length} \`@types/\` packages...`);

const typesPackagesToRemove = Promise.all(
  typesPackagesPresent
    .filter(([x]) => x !== "@types/node")
    .map(async ([name, version]) => {
      ...
      try {
        actualVersion = JSON.parse(
          readFileSync(
            join(process.cwd(), "node_modules", actualName, "package.json"),
          ),
        ).version;
      } catch (e) {
...
typesPackagesToRemove.then((x) => {
  ...
  if (x.length > 0) {
    execSync(
      `npm uninstall --save ${x.map(([name, version]) => `"${name}@${version}"`).join(" ")}`,
      { cwd: process.cwd() },
    );
  }
});
