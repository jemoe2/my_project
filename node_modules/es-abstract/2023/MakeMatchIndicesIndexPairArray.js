...
module.exports = function MakeMatchIndicesIndexPairArray(
  S,
  indices,
  groupNames,
  hasGroups,
  ...
  if (!IsArray(indices) || !every(indices, isMatchRecordOrUndefined)) {
    throw new $TypeError(
      "Assertion failed: `indices` must be a List of either Match Records or `undefined`",
    );
  }
  if (!IsArray(groupNames) || !every(groupNames, isStringOrUndefined)) {
    throw new $TypeError(
      "Assertion failed: `groupNames` must be a List of either Strings or `undefined`",
    );
  }
  ...
  if (!(n < MAX_ARRAY_LENGTH)) {
    throw new $TypeError(
      "Assertion failed: `indices` length must be less than the max array size, 2**32 - 1",
    );
  }
  if (groupNames.length !== n - 1) {
    throw new $TypeError(
      "Assertion failed: `groupNames` must have exactly one fewer item than `indices`",
    );
  }
