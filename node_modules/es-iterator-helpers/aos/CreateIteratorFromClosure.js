...
module.exports = function CreateIteratorFromClosure(
  closure,
  generatorBrand,
  proto,
) {
  ...
  if (arguments.length > 3) {
    if (!IsArray(extraSlots) || !every(extraSlots, isString)) {
      throw new $TypeError(
        "`extraSlots` must be a List of String internal slot names",
      );
    }
  }
  var internalSlotsList = safeConcat(extraSlots, [
    "[[GeneratorContext]]",
    "[[GeneratorBrand]]",
    "[[GeneratorState]]",
  ]); // step 3
  ...
  SLOT.set(
    generator,
    "[[CloseIfAbrupt]]",
    SLOT.get(closure, "[[CloseIfAbrupt]]"),
  ); // our second userland slot
