  ...
  tests: function (zipKeyed, name, t) {
    ...
    forEach(v.primitives, function (primitive) {
      ...
      if (primitive != null) {
        t["throws"](
          function () {
            zipKeyed({ a: primitive });
          },
          TypeError,
          'key "a" on iterables object is ' +
            debug(primitive) +
            " which is not an iterable Object",
        );
      }
    });
    ...
    forEach(v.objects, function (nonIterator) {
      t.doesNotThrow(function () {
        zipKeyed({ a: nonIterator });
      }, "does not throw until `.next()`");

      t["throws"](
        function () {
          zipKeyed({ a: nonIterator }).next();
        },
        TypeError,
        'key "a" on iterables object is ' +
          debug(nonIterator) +
    ...
    t.test("actual iteration", { skip: !hasSymbols }, function (st) {
      forEach(v.nonFunctions, function (nonFunction) {
        if (nonFunction != null) {
          ...
          st["throws"](
            function () {
              zipKeyed({ a: [], b: badIterable, c: [] }).next();
            },
            TypeError,
            'key "b" on iterables object is ' +
              debug(badIterable) +
              " is not a function",
          );
        }
  ...
  shimmed: function () {
    test("Iterator.zipKeyed: shimmed", function (t) {
      t.test("Function name", { skip: !functionsHaveNames }, function (st) {
        st.equal(
          Iterator.zipKeyed.name,
          "zipKeyed",
          'Iterator.zipKeyed has name "zipKeyed"',
        );
        st.end();
      });
      ...
      t.test(
        ...
        function (et) {
          et.equal(
            false,
            isEnumerable.call(Iterator, "zipKeyed"),
            "Iterator.zipKeyed is not enumerable",
          );
          et.end();
        },
      );
      ...
      module.exports.tests(
        callBind(Iterator.zipKeyed, Iterator),
        "Iterator.zipKeyed",
        t,
      );
