      ...
      CallExpression(node) {
          ...
          if ("name" in node.callee && node.callee.name === method.methodName) {
            ...
            while (current.type !== "Program") {
              if (current.type === "MethodDefinition") {
                methods.push({
                  methodName:
                    "name" in current.key ? current.key.name : undefined,
                  node: method.node,
                });
                break;
              }
        ...
        while (current.type !== "Program") {
          if (isFirstArgumentInSetStateCall(current, node)) {
            const methodName =
              "name" in node.callee ? node.callee.name : undefined;
            methods.forEach((method) => {
        ...
        if (
          ...
          while (current.type !== "Program") {
            ...
            if (current.type === "MethodDefinition") {
              ...
              methods.push({
                methodName:
                  "name" in current.parent.key
                    ? current.parent.key.name
                    : undefined,
                node,
              });
              break;
            ...
            if (current.type === "VariableDeclarator") {
              vars.push({
                ...
                variableName:
                  "name" in current.id ? current.id.name : undefined,
              });
              break;
            }
