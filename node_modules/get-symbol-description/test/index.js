...
test("getSymbolDescription", function (t) {
  ...
  t.test("with symbols", { skip: !v.hasSymbols }, function (st) {
    forEach(
      ...
      function (pair) {
        ...
        st.equal(
          ...
          desc,
          debug(sym) + " description is " + debug(desc),
        );
      },
    );

    st.test(
      "only possible when inference or native `Symbol.prototype.description` is supported",
      {
    ...
    st.test(
      "only possible when global symbols are supported",
      {
      ...
      function (s2t) {
        ...
        s2t.equal(
          getSymbolDescription(Symbol["for"]("")),
          "",
          'Symbol.for("") description is ""',
        );
        s2t.end();
      },
    );
