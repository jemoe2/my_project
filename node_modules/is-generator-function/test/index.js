...
test("returns false for non-generator functions", function (t) {
  ...
  t.notOk(
    ...
    "anonymous function is not an generator function",
  );

  var namedFunc = function foo() {};
  t.notOk(
    isGeneratorFunction(namedFunc),
    "named function is not an generator function",
  );
  ...
  if (typeof window === "undefined") {
    ...
    t.notOk(
      isGeneratorFunction(window.alert),
      "window.alert is not an generator function",
    );
  }
  t.end();
});
...
test("returns false for non-generator function with faked toString", function (t) {
  ...
  t.notOk(
    isGeneratorFunction(func),
    "anonymous function with faked toString is not a generator function",
  );
  t.end();
});
...
test(
  ...
  { skip: !hasToStringTag || generatorFuncs.length === 0 },
  function (t) {
    ...
    t.notOk(
      isGeneratorFunction(fakeGenFunction),
      'fake GeneratorFunction with @@toStringTag "GeneratorFunction" is not a generator function',
    );
    t.end();
  },
);
...
test("returns true for generator functions", function (t) {
  if (generatorFuncs.length > 0) {
    forEach(generatorFuncs, function (generatorFunc) {
      t.ok(
        isGeneratorFunction(generatorFunc),
        generatorFunc + " is generator function",
      );
    });
  } else {
    t.skip(
      "generator function is generator function - this environment does not support ES6 generator functions. Please run `node --harmony`, or use a supporting browser.",
    );
  }
  t.end();
});
